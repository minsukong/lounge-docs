...
- 애플리케이션 소스는 `apps/` 아래에서 관리합니다. Flutter Native 앱은 `apps/mobile/`, WebView Front-end는 `apps/app-webview/`의 실제 package, 설정과 source를 기준으로 판단합니다.
- Front-end 작업의 기본 구현 범위는 `apps/app-webview/`입니다. `apps/mobile/`은 별도 앱 개발자가 담당하므로 사용자가 Flutter 작업을 명시적으로 요청하지 않은 한 수정하거나 Native 구현·Build·배포 기준을 대신 결정하지 않습니다.
- 화면 코드는 React 19와 TypeScript로 작성합니다.
- 스타일은 Tailwind CSS 4와 `src/app/globals.css`의 의미 기반 토큰을 사용합니다.
- 디자인 토큰은 Figma의 Tokens Studio에서만 생성·수정·삭제하며, 저장소의 토큰 파일은 직접 수정하지 않습니다.
...
- AI의 사용자 응답, 설명, 완료 보고와 저장소의 한국어 문서·주석·사용자 노출 문구는 표준 한국어를 사용합니다. 한국어 단어 안이나 문장 사이에 한자 또는 중국어 문자를 섞지 않습니다. 한국어와 중국어 문자가 결합된 혼합 표현은 자연스러운 한국어로 고칩니다.
- 사용자가 명시적으로 요청한 중국어·일본어 번역, 다국어 예시, 외부 고유명사, 인용문, 코드 식별자와 테스트 데이터는 예외입니다. 예외가 아닌 한자나 중국어 문자를 발견하면 자연스러운 한국어 표현으로 고칩니다.
- 깨진 문자, 유니코드 대체 문자 `U+FFFD`, 잘못 디코딩된 문자열, 의도하지 않은 전각 기호·제어 문자·보이지 않는 문자를 생성하거나 남기지 않습니다. 문자 인코딩이 의심되면 임의로 저장하지 말고 원본 인코딩과 실제 표시 결과를 먼저 확인합니다.
- AI 또는 로컬 LLM이 만든 텍스트는 그대로 신뢰하지 않습니다. 완료 전에 이번 작업에서 생성하거나 수정한 한국어 텍스트를 다시 읽고 한·중 혼용, 문맥에 맞지 않는 외국 문자, 깨진 문자가 없는지 확인합니다.
- 문자 검사는 한자·중국어 Unicode 범위와 `U+FFFD`를 검색하되, 검색 결과를 기계적으로 일괄 치환하지 않습니다. 번역 예시와 코드처럼 의도된 사용인지 문맥을 확인하고 의도하지 않은 항목만 수정합니다.
- 검색 색인처럼 원본에서 생성되는 파일에 이상 문자가 복제되었으면 원본을 먼저 수정하고 공식 생성 절차로 파생 파일을 다시 만듭니다. 생성 파일만 직접 고쳐 원본과 불일치하게 만들지 않습니다.
- 예상하지 않은 문자가 하나라도 남아 있으면 작업이 완료된 것으로 보고하지 않습니다. 직접 수정할 수 없는 외부 원문이나 도구 출력이면 위치, 문자와 수정하지 못한 이유를 완료 보고에 명시합니다.
...
- 문서 변경 기록 여부와 `docs/CHANGELOG.md` 작성 기준은 `docs/AGENTS.md`의 「변경 기록」을 따릅니다.
...
- 애플리케이션 Build와 배포 범위는 `apps/mobile/`과 `apps/app-webview/`에서 담당자별로 각각 관리하며, Front-end 작업에서는 `apps/app-webview/`만 기본 검증 대상으로 봅니다. `docs/`는 제품 Bundle, 정적 자산, Build 입력과 배포 Artifact에 포함하거나 애플리케이션의 `public` 경로로 복사하지 않습니다.
...
이 절은 Codex, Cline, Kilo Code, Claude Code, Copilot 등 이 저장소에서 작업하는 AI 도구의 공통 기준입니다. 특정 도구의 별도 규칙 파일이 있더라도 여기의 기준을 중복 정의하거나 다르게 해석하지 않습니다.

### 컨텍스트 확인

- 가장 가까운 `AGENTS.md`, 사용자가 지정한 파일과 실제 Source부터 확인하고 현재 작업과 무관한 문서를 연쇄적으로 읽지 않습니다.
- 구현 기준이 더 필요하면 작업과 직접 일치하는 `docs/ai` 요약 하나를 먼저 확인하고, 부족할 때만 상세 가이드나 `docs/common-source`를 추가로 확인합니다.
- 같은 내용의 Markdown과 HTML이 함께 있으면 Markdown만 읽습니다.
- 브리핑, 교육 자료, 변경 기록, 초안과 작업자용 일정은 해당 자료 자체가 작업 대상일 때만 확인합니다.
- 사용자가 파일, selector, 함수 또는 문구처럼 정확한 대상을 지정하면 전체 파일보다 검색 결과와 필요한 주변 범위를 우선 확인합니다.

### 작업 분할과 출력 제한

- 한 번에 하나의 명확하고 검증 가능한 작업 단위를 완료합니다. 여러 파일이나 서로 다른 목적이 섞인 큰 작업은 구현 전에 작은 단계로 나누고 순서대로 진행합니다.
- 단계 분할은 긴 답변을 여러 조각으로만 나누는 방식이 아니라 조사, 구현, 검증처럼 각각 독립적으로 완료 여부를 확인할 수 있는 작업 단위를 기준으로 합니다.
- 설명은 현재 판단과 다음 행동을 이해하는 데 필요한 만큼만 작성하고 실제 파일 수정과 검증을 우선합니다. 이미 전달한 배경, 계획, 코드와 결과를 반복하지 않습니다.
- 전체 파일, 긴 로그, 대량의 검색 결과를 응답에 그대로 출력하지 않습니다. 필요한 위치와 핵심 내용만 요약하고 사용자가 명시적으로 요청한 경우에만 원문 범위를 넓힙니다.
- 응답이 출력 한도에 가까워질 것으로 예상되면 새 범위를 시작하지 않습니다. 현재 단계의 완료 항목, 변경 파일, 검증 결과와 남은 작업을 짧게 정리한 뒤 다음 단계로 나눕니다.
- 출력 한도 때문에 응답이 중단된 뒤 재개할 때는 같은 내용을 처음부터 다시 생성하지 않습니다. 실제 파일 상태를 먼저 확인하고 완료되지 않은 부분부터 이어갑니다.
- 도구 사용이 필요하면 불필요한 설명을 길게 생성한 뒤 호출하지 말고, 필요한 인자와 형식만 구성해 바로 호출합니다.
- 도구 호출 형식이 깨졌거나 도구 호출용 내용이 일반 텍스트로 출력되거나 같은 출력이 반복되면 생성을 계속하지 않습니다. 현재 상태를 확인하고 더 작은 작업 단위와 올바른 호출 형식으로 한 번만 다시 시도합니다.
- 같은 출력 제한 또는 도구 호출 오류가 다시 발생하면 출력 한도를 임의로 늘려 우회하지 않습니다. 완료된 내용과 오류 지점을 보고하고 새 작업으로 분리하거나 사용자 확인이 필요한 상태로 남깁니다.

### 편집 전략

- 첫 수정 전에 현재 도구 목록을 확인합니다. `apply_patch`가 있으면 같은 파일의 여러 위치를 수동으로 수정하는 작업에 반드시 우선 사용하고, 해당 변경을 여러 `editor` 호출로 나누지 않습니다.
- `editor`는 도구 schema가 한 호출에 여러 replacement를 명시적으로 지원하는 경우에만 일괄 편집 도구로 간주합니다. 그 외에는 `apply_patch`나 실제 multi-edit가 없을 때 사용하는 단건 편집 fallback으로 취급합니다.
- 관련 범위를 먼저 충분히 확인하고, 같은 파일의 관련 변경은 가능한 한 하나의 호출에 여러 patch hunk 또는 replacement로 묶습니다.
- 일괄 편집 도구가 없거나 정상적으로 구성한 일괄 호출이 실패한 경우에만 단건 편집 도구로 전환하고 호출 수를 최소화합니다.
- 기존 파일을 전체 내용으로 다시 쓰지 말고 변경된 범위만 수정합니다. 기존 인코딩, 줄바꿈, 들여쓰기와 관련 없는 서식을 보존합니다.
- 작은 수정마다 같은 파일을 다시 읽지 않습니다. 관련 변경을 마친 뒤 영향 범위와 간결한 diff를 한 번 검증합니다.
- 전체 파일 formatter는 사용자가 명시적으로 요청한 경우에만 실행합니다.

### 중단과 편집 실패 복구

- 중단, 재시도 또는 작업 재개 후에는 현재 대상 파일을 먼저 확인하고 실제 파일 상태를 기준으로 완료된 부분과 남은 부분을 구분합니다.
- 같은 인자와 전략으로 실패한 편집을 반복하지 않습니다. 대상 문자열을 찾지 못했거나 여러 번 일치하면 관련 범위만 한 번 확인한 뒤 현재 텍스트와 고유한 anchor로 다시 구성합니다.
- 같은 대상에 두 번의 편집이 실패하면 반복을 멈추고 정확한 오류와 안전하게 시도할 수 있는 다른 방법을 보고합니다.
- 읽기와 편집을 번갈아 무한 반복하지 말고, 완료된 범위는 이후 변경의 영향을 받지 않는 한 다시 확인하지 않습니다.

### Windows PowerShell 호환성

- 활성 Shell과 버전을 확인하지 못했다면 Windows PowerShell 5.1 호환 문법을 사용합니다.
- 명령 연결에 `&&` 또는 `||`를 사용하지 않습니다. 명령을 별도로 실행하거나 조건부 실행이 필요하면 명시적인 PowerShell `if` 문을 사용합니다.
- CMD의 `2>nul` 대신 PowerShell cmdlet의 `-ErrorAction SilentlyContinue` 또는 필요한 경우 `2>$null`을 사용합니다.
- `명령 || powershell -Command ...`처럼 CMD, Bash와 PowerShell 문법을 한 명령에서 섞지 않습니다.
- 알고 있는 Windows 경로에는 `-LiteralPath`를 우선 사용합니다.

### 변경 범위 검증

- 편집 후 `git diff --numstat` 또는 동등한 요약으로 변경량이 요청 범위와 맞는지 확인합니다.
- 기존 파일 대부분이 변경된 것으로 보이면 추가 쓰기나 formatter 실행을 중단하고 줄바꿈, 인코딩 또는 서식 변동 가능성을 보고합니다.
- 완료 전에 `git diff --check`를 실행하고 현재 작업에서 만든 관련 없는 변경이 없는지 확인합니다.
- 문서 변경 기록 여부는 `docs/AGENTS.md`의 「변경 기록」을 따르며 AI 도구별 별도 기준을 만들지 않습니다.
