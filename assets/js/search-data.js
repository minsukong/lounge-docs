window.LOUNGE_GUIDE_SEARCH_INDEX = [
  ...
  },
  {
    ...
    "excerpt": "Figma는 컴포넌트와 화면의 시각적 기준 및 디자인 의도를 제공하고 Storybook은 실제 코드로 구현된 컴포넌트와 화면의 렌더링 상태를 제공합니다. Figma Main Component의 Description에는 코드 컴포넌트 이름, import 경로와 주요 Props를 기록합니다. 실제 코드 또는 Storybook에 접근할 수 있는 URL이 있으면 Main Component의 Dev…",
    "content": "8. Figma와 Storybook의 역할 Figma는 컴포넌트와 화면의 시각적 기준 및 디자인 의도를 제공하고 Storybook은 실제 코드로 구현된 컴포넌트와 화면의 렌더링 상태를 제공합니다. Figma Main Component의 Description에는 코드 컴포넌트 이름, import 경로와 주요 Props를 기록합니다. 실제 코드 또는 Storybook에 접근할 수 있는 URL이 있으면 Main Component의 Dev resource로 연결합니다. 외부 Storybook URL이 없으면 컴포넌트 이름과 Story title을 기록하고 실제 소스 링크를 우선 연결합니다. Figma Description, Story와 실제 코드가 다르면 실제 코드와 변경 의도를 확인한 뒤 함께 갱신합니다. Figma의 주요 Screen에는 연결되는 Story 이름을 기록하고, 접근 가능한 Storybook URL이 있으면 Dev resource로 연결합니다. Storybook은 Code Connect를 대체하지 않지만 AI 코딩 도구와 개발자가 기존 컴포넌트의 API와 전체 화면 상태를 찾는 실행 가능한 UI 기준으로 사용합니다.",
