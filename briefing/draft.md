...
폴더 구조는 현재 구축 기준이며 실제 저장소가 생성된 뒤 Package 경계와 Build 구성이 확인되면 조정할 수 있습니다. 구조를 바꿀 때는 한 기능의 변경이 여러 Application과 Package에 불필요하게 퍼지지 않는지 확인합니다.

상세한 Application·Package 경계와 공통화 기준은 [Front-End Monorepo 공통 기준](../monorepo/index.html)에서 확인합니다.
...
파일 배치, 기술 스택, 상태와 품질의 전체 기준은 [Front-End 개발 가이드](../frontend_guide/index.html)에서 확인합니다.
...
전체 상태 소유 기준은 [Front-End 개발 가이드](../frontend_guide/index.html), Zustand의 Store와 Persist 기준은 [Zustand UI 상태 관리 가이드](../ui_guide/zustand.html)에서 확인합니다.
...
Loading, Timeout, Retry, Offline과 측정 방법은 [네트워크 지연 대응 및 성능 검증 가이드](../performance_guide/index.html)에서 확인합니다.
...
Native·WebView 책임은 [APP 개발 표준](../app/app.html), Bridge 계약과 구현은 [WebView 개발 가이드](../app/webView/webView_guide.html)에서 확인합니다.
...
Browser 점유율과 대표 너비는 [반응형 웹 Browser 지원 가이드](../browser_support_guide/index.html), App WebView 최소 OS는 [WebView 지원 환경 개요](../app/webView/webView_overview.html)에서 확인합니다.
...
Lingui 카탈로그, 로컬 LLM 번역과 Flutter 기기 번역은 [Front-End 다국어 및 로컬 LLM 번역 가이드](../i18n_guide/index.html)에서 확인합니다.
...
XSS, 인증·권한, 저장소, 개인정보, CSP, Bridge와 공급망은 [Front-End 보안과 개인정보 가이드](../security_guide/index.html)에서 확인합니다.
...
Type 경계는 [TypeScript 가이드](../typescript_guide/index.html), 정적 검사는 [Lint 가이드](../lint_guide/index.html), Test 범위는 [테스트 가이드](../test_guide/index.html)에서 확인합니다.

## 13. 가이드 저장소 구조와 문서 계층

현재 `lounge-docs`는 실제 서비스 Source 저장소가 아니라 Front-end 구축 기준을 정리하고, 이후 프로젝트에 적용할 문서와 참고 Source를 준비하는 가이드 저장소입니다.

```text
lounge-docs/
├── AGENTS.md                         # 이 가이드 저장소를 작성·수정하는 규칙
├── README.md                         # 전체 문서 진입점
├── search/                           # 사람용 가이드 중앙 검색
├── monorepo/                         # 저장소·Application·Package 경계 원본
├── frontend_guide/                   # Front-end 구현 기준 원본
├── app/                              # Native App·WebView 기준 원본
├── ui_guide/                         # Figma, Token, 상태 관리와 UI 구현 원본
├── storybook_guide/                  # Storybook 운영 기준 원본
├── security_guide/                   # 보안과 개인정보 기준 원본
├── *_guide/                          # TypeScript, Test, 성능, 다국어 등 주제별 원본
├── briefing/                         # 구축 기준 공유용 브리핑과 발표 자료
└── reference/
    └── front-end/                    # 실제 프로젝트에 가져가 사용할 문서 묶음
        ├── AGENTS.md                 # 실제 Front-end 업무에서 AI가 먼저 읽는 규칙
        └── docs/
            ├── ai/                   # 작업별 짧은 AI 요약
...
`monorepo`, `frontend_guide`, `security_guide`, `storybook_guide`와 각 주제별 `*_guide`는 사람이 읽고 기술 기준과 판단 근거를 확인하는 원본입니다. 배경, 적용 조건, 예외, 미확정 항목과 Review 기준을 포함합니다.

각 폴더의 `draft.md`는 내용을 검토하고 조정하는 초안입니다. 사용자가 HTML 반영을 요청한 뒤 생성하거나 갱신한 `index.html` 또는 주제별 HTML이 사람이 보는 배포 문서입니다. 초안만 수정한 상태를 HTML까지 반영된 것으로 간주하지 않습니다.
...
사람용 원본은 [가이드 검색](../search/index.html)에서 찾고, 실제 프로젝트 문서 계층은 [프로젝트 구조 AI 요약](../reference/front-end/docs/ai/project.html)에서 확인합니다.
