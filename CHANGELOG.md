...
- `reference/front-end`를 최종 구조로 설명하던 브리핑 내용을 통합 전 준비 영역으로 바로잡고, Root `AGENTS.md`, `docs/AGENTS.md`, `docs/guides`, `docs/assets`, `docs/ai`, `docs/common-source`와 `apps/app-webview`의 최종 책임을 반영했습니다.
- 실제 통합 시 문서, 공통 자산과 내부 링크를 한 변경에서 함께 이동하고 기존 루트 가이드 경로, `reference/front-end`와 별도 `common_docs`를 중간 구조로 남기지 않는 기준을 명시했습니다.
- 일반 브리핑과 PC 발표 페이지를 함께 갱신하고 문서 구조, 내부 링크와 발표 장면 구성을 확인했습니다.
