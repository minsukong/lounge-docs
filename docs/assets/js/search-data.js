window.LOUNGE_GUIDE_SEARCH_INDEX = [
  ...
  },
  {
    ...
    "content": "사람과 AI가 바뀌어도 같은 기준으로 구축합니다. BUILD 구축 구조 프로젝트 · Component · 상태 · API · WebView · Storybook · Test · Build VERIFY 사용 환경 OS · Browser · 반응형 · 다국어 · 접근성 · 보안 · 개인정보 OPERATE 가이드 운영 사람용 원본 · AI용 요약 · 실제 프로젝트 · 변경 동기화 실제 Application이 생성된 뒤에는 설치 Package, 기존 Source와 승인된 프로젝트 계약을 일반 예시보다 우선합니다. 02 · REPOSITORY",
  ...
  },
  {
    ...
    "excerpt": "실행과 배포 목적이 다른 Application은 분리하고, 공통 UI는 실제 재사용이 검증된 뒤 이동합니다. apps/ ├─ website/ 회사 웹사이트 ├─ mobile/ Flutter Native App ├─ app-webview/ WebView 서비스 화면 ├─ card-inapp/ 제휴사별 In-app └─ backoffice/ 운영자 화면 packages/ └─ ui/ 검증된 공통…",
    "content": "Application 가까이에서 작게 시작합니다. 실행과 배포 목적이 다른 Application은 분리하고, 공통 UI는 실제 재사용이 검증된 뒤 이동합니다. apps/ ├─ website/ 회사 웹사이트 ├─ mobile/ Flutter Native App ├─ app-webview/ WebView 서비스 화면 ├─ card-inapp/ 제휴사별 In-app └─ backoffice/ 운영자 화면 packages/ └─ ui/ 검증된 공통 UI packages/ui 에는 둘 이상의 Web Application에서 의미와 변경 이유가 같은 구현만 둡니다. 03 · CORE WORKFLOW",
  ...
  },
  {
    ...
    "excerpt": "WebView 서비스 화면은 apps/app-webview 에 구현할 예정입니다. 웹사이트, 제휴사별 In-app 화면과 Backoffice처럼 실행과 배포 목적이 다른 Application은 각각의 apps/* 영역으로 분리합니다. apps/ ├── website/ # 회사 웹사이트 ├── mobile/ # Flutter Native App ├── app-webview/ # App에서 사용…",
    "content": "2. 구축 대상과 저장소 구조 WebView 서비스 화면은 apps/app-webview 에 구현할 예정입니다. 웹사이트, 제휴사별 In-app 화면과 Backoffice처럼 실행과 배포 목적이 다른 Application은 각각의 apps/* 영역으로 분리합니다. apps/ ├── website/ # 회사 웹사이트 ├── mobile/ # Flutter Native App ├── app-webview/ # App에서 사용하는 WebView 서비스 화면 ├── card-inapp/ # 제휴사별 In-app 화면 └── backoffice/ # 운영자 화면 packages/ └── ui/ # 둘 이상의 Web Application에서 검증된 공통 UI Application 가까이에서 시작합니다 apps/app-webview 안에서는 Routing과 화면 진입점, 기능 코드와 WebView 전용 UI를 가까운 위치에서 관리합니다. App-WebView 한 곳에서만 사용하는 shadcn/ui 원형과 Wrapper는 apps/app-webview/src/components/ui 에 둡니다. packages/ui 는 검증된 공통 구현만 관리합니다 둘 이상의 Web Application에서 실제로 재사용되고 의미와 변경 이유가 같은 UI만 이동합니다. 한 화면에서만 사용하는 Wrapper, 공통화 가능성을 예상한 Type과 Adapter는 미리 만들지 않습니다. apps/ 와 docs/ 의 최상위 분리는 실제 프로젝트 기준입니다. 앱 내부 Package 경계와 Build 구성은 앱 초기화 후 실제 사용처를 확인하며 조정할 수 있습니다. 상세 기준: Application과 Package 경계, 공통화와 Release 단위는 Front-End Monorepo 공통 기준 에서 확인합니다.",
  ...
  },
  {
    ...
    "excerpt": "이 저장소는 WebView 애플리케이션과 구현 가이드를 함께 관리합니다. 실제 Source는 apps/app-webview , 모든 가이드는 docs 에 두며 별도 가이드 저장소나 복제된 reference 문서를 운영하지 않습니다. lounge-project/ ├── AGENTS.md # 저장소 전체 AI 구현 규칙 ├── README.md # 프로젝트 진입점 ├── apps/ │ └── a…",
    "content": "13. 실제 프로젝트 구조와 문서 계층 이 저장소는 WebView 애플리케이션과 구현 가이드를 함께 관리합니다. 실제 Source는 apps/app-webview , 모든 가이드는 docs 에 두며 별도 가이드 저장소나 복제된 reference 문서를 운영하지 않습니다. lounge-project/ ├── AGENTS.md # 저장소 전체 AI 구현 규칙 ├── README.md # 프로젝트 진입점 ├── apps/ │ └── app-webview/ │ ├── AGENTS.md # WebView 앱 전용 규칙 │ ├── .storybook/ # 앱 초기화 후 Storybook 설정 │ └── src/ # 실제 Application Source └── docs/ ├── AGENTS.md # 문서 작성과 검증 규칙 ├── README.md # 전체 가이드 목록 ├── index.html # 사람이 보는 문서 진입점 ├── search/ # 상세 가이드 중앙 검색 ├── assets/ # 문서 공통 Style·Script·Image ├── templates/ # HTML 가이드 템플릿 ├── guides/ # 주제별 상세 HTML 가이드와 초안 ├── ai/ # 작업별 짧은 AI 요약 └── common-source/ # 조건부 참고 구현 문서별 역할 Root AGENTS.md 는 실제 Application 구현 규칙과 작업별 문서 선택을 담당합니다. apps/app-webview/AGENTS.md 는 앱 초기화 상태와 WebView 앱에만 필요한 추가 규칙을 관리합니다. docs/guides 는 사람이 기술 기준, 적용 조건, 예외와 Review 근거를 확인하는 원본입니다. docs/ai 는 작업별 요약이고 docs/common-source 는 실제 사용처, 설치 Package와 승인된 계약을 확인한 뒤 필요한 부분만 비교·병합하는 참고 구현입니다. 현재 앱 상태 apps/app-webview 는 package와 Source를 초기화하기 전의 빈 골격입니다. Next.js, Tailwind CSS, shadcn/ui, Storybook과 Test 설정은 실제 설치 결과를 확인하며 생성하고, 승인된 계약이 없는 기능은 미리 만들지 않습니다. flowchart TD accTitle: 실제 프로젝트에서 문서와 소스를 사용하는 흐름 accDescr: 저장소 루트 지침에서 시작해 앱의 실제 설정과 작업별 AI 요약을 확인하고, 필요한 상세 가이드와 공통 소스 적용 기준을 읽은 뒤 구현과 검증을 수행하며 변경된 기준을 문서에 동기화합니다. A[\"Root AGENTS.md\"] --> B[\"apps/app-webview AGENTS · Package · Source\"] B --> C[\"docs/ai 작업별 요약\"] C --> D[\"docs/guides 또는 docs/common-source 필요한 세부 기준\"] D --> E[\"Source 구현과 Storybook · 품질 검증\"] E --> F[\"구조·규칙 변경 시 관련 docs 동기화\"] 실제 프로젝트에서 지침과 가이드를 확인하고 Source와 문서를 함께 관리하는 흐름 문서 확인 우선순위 flowchart TD accTitle: 실제 업무에서 문서를 확인하는 우선순위 accDescr: 작업자는 실제 프로젝트 AGENTS 문서에서 시작해 작업별 AI 요약과 기존 소스를 확인하고, 세부 판단에는 사람용 원본 가이드와 승인된 담당자 계약을 사용합니다. A[\"실제 프로젝트 AGENTS.md\"] --> B[\"작업별 docs/ai 요약\"] B --> C[\"package.json · 설정 · 기존 Source\"] C --> D[\"사람용 원본 가이드\"] D --> E[\"승인된 기획 · Backend · Native · 배포 계약\"] 실제 프로젝트 규칙에서 원본 가이드와 승인된 계약까지 확인하는 흐름 검색과 동기화 중앙 검색은 docs/guides 의 사람이 읽는 상세 가이드를 대상으로 합니다. docs/ai 와 docs/common-source 는 중복 검색 결과를 막기 위해 상세 가이드 검색 대상과 분리합니다. 문서 진입점: 필요한 원본 문서는 가이드 검색 에서 찾고, 실제 프로젝트에 적용할 AI 문서 계층은 프로젝트 구조 AI 요약 에서 확인합니다.",
