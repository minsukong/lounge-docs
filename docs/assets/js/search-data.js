window.LOUNGE_GUIDE_SEARCH_INDEX = [
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "검색 자동완성에 Debounce, 취소, 최신 응답 판별과 Cleanup을 적용합니다. 빠른 입력, 느린 응답, 화면 이탈, 네트워크 오류를 각각 재현합니다. 실행 순서 타임라인, 실패 원인, 선택한 해결 방식과 대안의 차이를 기록합니다. Timer, Listener와 요청이 남지 않는지 테스트합니다. 통과 기준: 정상 결과만 보여주는 것으로 부족합니다. 오래된 결과가 화면을 덮는 정확한 순…",
    content:
      "7. 마스터 실습과 증거 검색 자동완성에 Debounce, 취소, 최신 응답 판별과 Cleanup을 적용합니다. 빠른 입력, 느린 응답, 화면 이탈, 네트워크 오류를 각각 재현합니다. 실행 순서 타임라인, 실패 원인, 선택한 해결 방식과 대안의 차이를 기록합니다. Timer, Listener와 요청이 남지 않는지 테스트합니다. 통과 기준: 정상 결과만 보여주는 것으로 부족합니다. 오래된 결과가 화면을 덮는 정확한 순서를 설명하고, 취소와 실패를 구분하며, 중첩 참조 공유와 자원 누수를 코드에서 찾아낼 수 있어야 합니다. 교육 과정 · 다음: TypeScript 기본 1. JavaScript 기초",
  ...
  },
  {
    ...
    excerpt:
      "Call Stack, Task와 Microtask JavaScript는 현재 Call Stack의 동기 코드를 먼저 끝낸 뒤 Promise 반응인 Microtask를 처리하고, 그 다음 Timer 같은 Task를 처리합니다. 따라서 코드의 줄 순서와 완료 순서는 같지 않을 수 있습니다. 직접 실행 순서를 종이에 예측한 뒤 Console 결과와 비교하고, 예측이 틀렸다면 어떤 Queue를 놓쳤…",
    content:
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "map , filter 와 find 의 차이를 설명할 수 있습니다. 기존 객체를 직접 수정한 코드를 찾을 수 있습니다. async 함수의 성공과 실패 경로를 구분할 수 있습니다. 비동기 응답 순서가 달라질 때 오래된 결과가 화면을 덮는 문제를 발견할 수 있습니다. Closure, 얕은 복사와 정리되지 않은 외부 작업이 만드는 버그를 설명할 수 있습니다.",
    content:
      "5. 완료 확인 map , filter 와 find 의 차이를 설명할 수 있습니다. 기존 객체를 직접 수정한 코드를 찾을 수 있습니다. async 함수의 성공과 실패 경로를 구분할 수 있습니다. 비동기 응답 순서가 달라질 때 오래된 결과가 화면을 덮는 문제를 발견할 수 있습니다. Closure, 얕은 복사와 정리되지 않은 외부 작업이 만드는 버그를 설명할 수 있습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'const availableTickets = tickets. filter ( ( ticket ) => ticket.status === "available" , ); async function loadTickets ( ) { const response = await fetch ( "/api/tickets" ); if (!response.ok) throw new Error ( "이용권을 불러오지…',
    content:
      '3. 코드 읽기 const availableTickets = tickets. filter ( ( ticket ) => ticket.status === "available" , ); async function loadTickets ( ) { const response = await fetch ( "/api/tickets" ); if (!response.ok) throw new Error ( "이용권을 불러오지 못했습니다." ); return response. json (); } availableTickets 는 별도 상태가 아니라 원본 배열에서 계산한 값입니다. 요청 함수는 실패를 숨기지 않고 호출자가 처리할 수 있게 전달합니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "교육 과정 · 다음: TypeScript 기본 const 와 let 의 값 변경 범위를 구분합니다. 객체와 배열을 직접 변경하지 않고 새 값으로 변환합니다. 함수의 입력, 출력과 비동기 실패 경로를 설명합니다.",
    content:
      "1. 학습 목표 교육 과정 · 다음: TypeScript 기본 const 와 let 의 값 변경 범위를 구분합니다. 객체와 배열을 직접 변경하지 않고 새 값으로 변환합니다. 함수의 입력, 출력과 비동기 실패 경로를 설명합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "AI에게 배열 변환과 비동기 요청이 포함된 짧은 예제를 만들게 합니다. 각 함수의 입력, 반환값과 실패 가능성을 먼저 설명해 달라고 요청합니다. 원본 객체를 직접 변경하는 부분과 처리되지 않은 Promise를 찾아 달라고 요청합니다. 직접 요구를 하나 바꾸고 AI가 수정한 Diff를 확인합니다. 이 JavaScript 코드에서 값의 입력과 출력, 배열 변환, 비동기 실패 경로를 설명해줘. 바…",
    content:
      "4. AI와 함께 연습 AI에게 배열 변환과 비동기 요청이 포함된 짧은 예제를 만들게 합니다. 각 함수의 입력, 반환값과 실패 가능성을 먼저 설명해 달라고 요청합니다. 원본 객체를 직접 변경하는 부분과 처리되지 않은 Promise를 찾아 달라고 요청합니다. 직접 요구를 하나 바꾸고 AI가 수정한 Diff를 확인합니다. 이 JavaScript 코드에서 값의 입력과 출력, 배열 변환, 비동기 실패 경로를 설명해줘. 바로 수정하지 말고 원본 값을 직접 변경하는 부분과 처리되지 않은 Promise부터 찾아줘.",
  ...
  },
  {
    ...
    section: "계약과 보안 질문",
    excerpt:
      "지원 Method, 입력, 성공 결과와 오류 형식은 무엇인가? 사용자 취소, 권한 거부, 미지원과 Timeout을 어떻게 구분하는가? 최소 App Version과 하위 호환 범위는 무엇인가? 로그에 남길 진단 코드와 제거할 민감정보는 무엇인가? 로그아웃과 사용자 전환 때 Cookie, Query Cache와 UI 상태를 어떻게 정리하는가? 이 Native 기능에 필요한 Bridge 계약 질문…",
    content:
      "4. 계약과 보안 질문 지원 Method, 입력, 성공 결과와 오류 형식은 무엇인가? 사용자 취소, 권한 거부, 미지원과 Timeout을 어떻게 구분하는가? 최소 App Version과 하위 호환 범위는 무엇인가? 로그에 남길 진단 코드와 제거할 민감정보는 무엇인가? 로그아웃과 사용자 전환 때 Cookie, Query Cache와 UI 상태를 어떻게 정리하는가? 이 Native 기능에 필요한 Bridge 계약 질문을 정리해줘. 화면·Adapter·Native·Backend 책임을 나누고 취소·권한 거부·미지원·Timeout을 구분해줘. 실제 계약이 없는 필드명과 전역 객체명은 확정하지 마.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "카메라 또는 파일 선택 Capability를 타입 안전 Adapter로 구현합니다. Bridge 없음, 지원하지 않는 Version, 잘못된 Payload, Timeout, 중복 응답과 사용자 취소를 주입합니다. Web fallback과 App 담당자에게 확인할 계약 질문을 함께 제출합니다. 통과 기준: 화면과 Native 구현의 결합을 Adapter에서 차단하고, 입력·응답·오류·Versi…",
    content:
      "7. 마스터 실습과 증거 카메라 또는 파일 선택 Capability를 타입 안전 Adapter로 구현합니다. Bridge 없음, 지원하지 않는 Version, 잘못된 Payload, Timeout, 중복 응답과 사용자 취소를 주입합니다. Web fallback과 App 담당자에게 확인할 계약 질문을 함께 제출합니다. 통과 기준: 화면과 Native 구현의 결합을 Adapter에서 차단하고, 입력·응답·오류·Version·요청 수명을 계약으로 설명할 수 있어야 합니다. 이전: 서버·클라이언트 상태 · 교육 과정 · 다음: 테스트와 디버깅 10. WebView Bridge와 보안",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "Native 기능이 필요한 이유와 Web 구현 범위를 설명할 수 있습니다. Bridge 미지원 환경을 안전한 실패 상태로 처리할 수 있습니다. URL, Storage, Bridge와 로그에 포함하면 안 되는 값을 구분할 수 있습니다. Bridge 응답 검증, 요청 식별, Timeout과 중복 실행 정책을 계약 질문으로 만들 수 있습니다. 화면이 Native 전역 객체와 범용 실행 통로에 직접…",
    content:
      "5. 완료 확인 Native 기능이 필요한 이유와 Web 구현 범위를 설명할 수 있습니다. Bridge 미지원 환경을 안전한 실패 상태로 처리할 수 있습니다. URL, Storage, Bridge와 로그에 포함하면 안 되는 값을 구분할 수 있습니다. Bridge 응답 검증, 요청 식별, Timeout과 중복 실행 정책을 계약 질문으로 만들 수 있습니다. 화면이 Native 전역 객체와 범용 실행 통로에 직접 의존하는 코드를 발견할 수 있습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'type NativeCapability = "location" | "qrScan" ; interface NativeBridge { isAvailable(capability: NativeCapability): boolean ; request(capability: NativeCapability, params?: unknown ): Promise < unknown >; } 화면은 Android·i…',
    content:
      '3. 코드 읽기 type NativeCapability = "location" | "qrScan" ; interface NativeBridge { isAvailable(capability: NativeCapability): boolean ; request(capability: NativeCapability, params?: unknown ): Promise < unknown >; } 화면은 Android·iOS 전역 객체 대신 Adapter만 사용합니다. 외부 응답은 unknown 으로 받고 기능별 Parser가 확인하며, 호출자가 지정한 Generic 타입으로 검증을 건너뛰지 않습니다. 실제 채널명, Method, Version, Timeout과 오류 형식은 App 담당자와 확정한 계약으로 교체합니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: 서버·클라이언트 상태 · 교육 과정 · 다음: 테스트와 디버깅 WebView, Native, Bridge와 Backend의 책임을 구분합니다. 화면 컴포넌트에서 플랫폼 전역 객체를 직접 호출하지 않습니다. 인증 토큰, 카드정보와 개인정보를 URL·Bridge·로그에 포함하지 않습니다.",
    content:
      "1. 학습 목표 이전: 서버·클라이언트 상태 · 교육 과정 · 다음: 테스트와 디버깅 WebView, Native, Bridge와 Backend의 책임을 구분합니다. 화면 컴포넌트에서 플랫폼 전역 객체를 직접 호출하지 않습니다. 인증 토큰, 카드정보와 개인정보를 URL·Bridge·로그에 포함하지 않습니다.",
  ...
  },
  {
    ...
    section: "디버깅과 AI 협업",
    excerpt:
      "재현 조건, 기대 결과와 실제 결과를 분리합니다. 오류 메시지를 없애기 전에 처음 잘못된 데이터나 상태가 생긴 지점을 찾습니다. AI에게 원인 후보와 확인 순서를 요청하고 하나씩 검증합니다. 버그 수정 후 위험이 크면 회귀 Test나 명시적 검수 항목을 남깁니다. 이 오류를 바로 수정하지 말고 재현 조건, 기대 결과와 실제 결과를 정리해줘. 원인 후보를 가능성 순으로 제시하고 각 후보를 확인…",
    content:
      "4. 디버깅과 AI 협업 재현 조건, 기대 결과와 실제 결과를 분리합니다. 오류 메시지를 없애기 전에 처음 잘못된 데이터나 상태가 생긴 지점을 찾습니다. AI에게 원인 후보와 확인 순서를 요청하고 하나씩 검증합니다. 버그 수정 후 위험이 크면 회귀 Test나 명시적 검수 항목을 남깁니다. 이 오류를 바로 수정하지 말고 재현 조건, 기대 결과와 실제 결과를 정리해줘. 원인 후보를 가능성 순으로 제시하고 각 후보를 확인할 가장 작은 방법을 알려줘. 수정 후 필요한 회귀 테스트도 사용자 동작 기준으로 제안해줘.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "간헐적으로 이전 사용자 데이터가 보이는 결함을 진단합니다. 최소 재현, 시간순 증거, 세 가지 가설과 반증, Root Cause, 수정 전후 테스트를 제출합니다. 테스트가 잡지 못하는 실제 환경 위험도 별도로 적습니다. 통과 기준: 테스트 개수가 아니라 위험과 검증 경계의 연결을 설명하고, 증상 패치가 아니라 원인을 좁혀 재발 방지까지 제안할 수 있어야 합니다. 이전: WebView Brid…",
    content:
      "7. 마스터 실습과 증거 간헐적으로 이전 사용자 데이터가 보이는 결함을 진단합니다. 최소 재현, 시간순 증거, 세 가지 가설과 반증, Root Cause, 수정 전후 테스트를 제출합니다. 테스트가 잡지 못하는 실제 환경 위험도 별도로 적습니다. 통과 기준: 테스트 개수가 아니라 위험과 검증 경계의 연결을 설명하고, 증상 패치가 아니라 원인을 좁혀 재발 방지까지 제안할 수 있어야 합니다. 이전: WebView Bridge와 보안 · 교육 과정 · 다음: 기능 통합과 리뷰 11. 테스트와 디버깅",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "검사 하나가 통과해도 다른 검사를 대체하지 못하는 이유를 설명할 수 있습니다. Storybook 대상과 제외 대상을 근거로 구분하고 실제 코드와 Story의 불일치를 찾을 수 있습니다. 위험도가 높은 로직과 사용자 흐름을 우선 테스트할 수 있습니다. Lint Disable, Test Skip과 무조건 재시도로 오류를 숨기지 않습니다. Parser·컴포넌트·요청 경계 중 어떤 수준을 테스트해야…",
    content:
      "5. 완료 확인 검사 하나가 통과해도 다른 검사를 대체하지 못하는 이유를 설명할 수 있습니다. Storybook 대상과 제외 대상을 근거로 구분하고 실제 코드와 Story의 불일치를 찾을 수 있습니다. 위험도가 높은 로직과 사용자 흐름을 우선 테스트할 수 있습니다. Lint Disable, Test Skip과 무조건 재시도로 오류를 숨기지 않습니다. Parser·컴포넌트·요청 경계 중 어떤 수준을 테스트해야 하는지 선택할 수 있습니다. 실제 Backend 환경을 우선하고 필요한 경우에만 Front-end 범위에서 선택한 Mock 도구의 범위와 한계를 설명할 수 있습니다. 입력부터 화면 결과까지 증거를 따라가며 AI의 원인 추측을 검증할 수 있습니다.",
  ...
  },
  {
    ...
    excerpt:
      "테스트 수준 선택 Pure Function은 Unit, 컴포넌트 상호작용은 Integration, 실제 사용자 핵심 경로는 End-to-End (E2E) Test 가 적합할 수 있습니다. 모든 테스트를 같은 비율로 만들지 말고 실패 가능성, 사용자 영향과 탐지 난이도로 우선순위를 정합니다. Mock의 신뢰 한계 Mock은 Front-end 분기를 재현하지만 실제 API, 브라우저 Layout…",
    content:
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'const user = userEvent. setup (); render ( < ReservationForm /> ); await user. type ( screen. getByLabelText ( "예약자 이름" ), "홍길동" , ); await user. click ( screen. getByRole ( "button" , { name: "예약하기" }), ); 역할과 Label로 요소…',
    content:
      '3. 코드 읽기 const user = userEvent. setup (); render ( < ReservationForm /> ); await user. type ( screen. getByLabelText ( "예약자 이름" ), "홍길동" , ); await user. click ( screen. getByRole ( "button" , { name: "예약하기" }), ); 역할과 Label로 요소를 찾으면 접근 가능한 사용 방식과 가까운 테스트가 됩니다. 내부 State, Tailwind 클래스와 라이브러리 자체 동작은 기본 검증 대상이 아닙니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: WebView Bridge와 보안 · 교육 과정 · 다음: 기능 통합과 리뷰 Typecheck, Lint, Test와 Build의 역할을 구분합니다. Storybook의 독립 UI 검수와 정적 Build가 실제 애플리케이션 검증을 대신하지 않는 이유를 설명합니다. 구현 세부사항보다 사용자 동작과 결과를 테스트합니다. AI에게 바로 수정시키기 전에 오류 원인과 영향 범위를 확인합니다.",
    content:
      "1. 학습 목표 이전: WebView Bridge와 보안 · 교육 과정 · 다음: 기능 통합과 리뷰 Typecheck, Lint, Test와 Build의 역할을 구분합니다. Storybook의 독립 UI 검수와 정적 Build가 실제 애플리케이션 검증을 대신하지 않는 이유를 설명합니다. 구현 세부사항보다 사용자 동작과 결과를 테스트합니다. AI에게 바로 수정시키기 전에 오류 원인과 영향 범위를 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "다음 내용을 AI의 도움을 받아도 좋지만 최종적으로 개발자가 설명하고 승인할 수 있어야 합니다. 이 기능의 사용자 목표와 실패 경로 컴포넌트와 파일을 나눈 이유 각 State의 소유자와 선택한 도구 외부 입력을 검증하는 경계 담당자와 확인한 계약과 아직 미확정인 항목 자동 검사와 직접 확인한 결과 다음 변경에서 주의해야 할 회귀 위험 API 미확정 상태에서 정리한 Backend 질문과 남아…",
    content:
      "5. 기능 통합 단계 완료 기준 다음 내용을 AI의 도움을 받아도 좋지만 최종적으로 개발자가 설명하고 승인할 수 있어야 합니다. 이 기능의 사용자 목표와 실패 경로 컴포넌트와 파일을 나눈 이유 각 State의 소유자와 선택한 도구 외부 입력을 검증하는 경계 담당자와 확인한 계약과 아직 미확정인 항목 자동 검사와 직접 확인한 결과 다음 변경에서 주의해야 할 회귀 위험 API 미확정 상태에서 정리한 Backend 질문과 남아 있는 TBD 로그아웃·사용자 전환 때 제거해야 할 사용자 범위 상태 자동 검사와 실제 화면·Network·WebView 검증이 각각 보장하는 범위 기능 통합 단계 완료는 AI 없이 모든 코드를 작성하는 상태가 아닙니다. AI에게 더 정확한 맥락을 제공하고 잘못된 가정과 구현을 발견하며 제품과 기술 결정에 책임질 수 있는 상태입니다.",
  ...
  },
  {
    ...
    section: "마스터 통과 심사",
    excerpt:
      "학습자는 30분 동안 설계를 설명하고, 리뷰어가 넣은 새로운 요구와 실패 조건 하나를 작은 변경으로 반영합니다. 이어서 알 수 없는 오류 하나를 증거로 진단하고 Rollback 또는 안전한 배포 범위를 제안합니다. 통과 기준: 기능 동작뿐 아니라 결정 이유, 위험, 미확정 사항, 검증 한계를 설명하고 AI가 만든 잘못된 가정과 과도한 구조를 스스로 수정해야 합니다. 이전: 테스트와 디버깅 ·…",
    content:
      "7. 마스터 통과 심사 학습자는 30분 동안 설계를 설명하고, 리뷰어가 넣은 새로운 요구와 실패 조건 하나를 작은 변경으로 반영합니다. 이어서 알 수 없는 오류 하나를 증거로 진단하고 Rollback 또는 안전한 배포 범위를 제안합니다. 통과 기준: 기능 동작뿐 아니라 결정 이유, 위험, 미확정 사항, 검증 한계를 설명하고 AI가 만든 잘못된 가정과 과도한 구조를 스스로 수정해야 합니다. 이전: 테스트와 디버깅 · 교육 과정 · 다음: Front-End 보안과 개인정보 12. 기능 통합과 리뷰",
  ...
  },
  {
    ...
    section: "병합 전 리뷰",
    excerpt:
      "요청하지 않은 기능, 상태, package와 공통화를 추가하지 않았는가? 타입 오류를 any , 근거 없는 as , ! 와 검사 비활성화로 숨기지 않았는가? 서버, 폼과 UI 상태의 소유자가 구분되어 있는가? API·Bridge 계약의 미확정 사항을 코드로 고정하지 않았는가? 로딩, 빈 값, 오류, 취소와 중복 제출을 필요한 범위에서 처리했는가? 의미 있는 HTML, 토큰, 키보드와 Focu…",
    content:
  ...
  },
  {
    ...
    section: "통합 목표",
    excerpt:
      "이전: 테스트와 디버깅 · 교육 과정 · 다음: Front-End 보안과 개인정보 사용자 행동과 완료 조건을 구현 전에 정의합니다. 상태, 타입, API·Bridge 계약과 실패 경로를 연결합니다. AI의 Diff와 실제 검사 결과를 검토해 병합 여부를 판단합니다.",
    content:
      "1. 통합 목표 이전: 테스트와 디버깅 · 교육 과정 · 다음: Front-End 보안과 개인정보 사용자 행동과 완료 조건을 구현 전에 정의합니다. 상태, 타입, API·Bridge 계약과 실패 경로를 연결합니다. AI의 Diff와 실제 검사 결과를 검토해 병합 여부를 판단합니다.",
  ...
  },
  {
    ...
    section: "AI 작업 요청",
    excerpt:
      "이 기능을 프로젝트 가이드와 기존 소스를 기준으로 조사해줘. 바로 구현하지 말고 사용자 행동, 화면 상태, 상태 소유자, 필요한 타입, API·Bridge 계약 질문, 재사용 코드와 작은 구현 단계를 먼저 정리해줘. 확인되지 않은 요구는 추측하지 말고 별도로 표시해줘. 합의 후에는 한 단계씩 구현하고 각 단계마다 변경 이유와 검사 결과를 설명해줘. AI의 설명은 실제 명령 결과나 브라우저 검…",
    content:
      "3. AI 작업 요청 이 기능을 프로젝트 가이드와 기존 소스를 기준으로 조사해줘. 바로 구현하지 말고 사용자 행동, 화면 상태, 상태 소유자, 필요한 타입, API·Bridge 계약 질문, 재사용 코드와 작은 구현 단계를 먼저 정리해줘. 확인되지 않은 요구는 추측하지 말고 별도로 표시해줘. 합의 후에는 한 단계씩 구현하고 각 단계마다 변경 이유와 검사 결과를 설명해줘. AI의 설명은 실제 명령 결과나 브라우저 검증을 대신하지 않습니다. 확인하지 못한 항목은 완료 결과에 남깁니다.",
  ...
  },
  {
    ...
    section: "과정 완료 기준",
    excerpt:
      "신뢰하지 않는 입력과 위험한 출력 지점을 코드에서 찾을 수 있습니다. 인증, 권한과 UI 제한의 책임을 구분할 수 있습니다. 자격 증명과 개인정보의 저장·전달·삭제 경로를 설명할 수 있습니다. Client 검증, 자동 Test와 서버·기기 통합 검증의 한계를 설명할 수 있습니다. React·Next.js의 공식 보안 공지를 기준으로 실제 설치·배포 Version의 영향 여부와 필요한 대응을…",
    content:
      "6. 과정 완료 기준 신뢰하지 않는 입력과 위험한 출력 지점을 코드에서 찾을 수 있습니다. 인증, 권한과 UI 제한의 책임을 구분할 수 있습니다. 자격 증명과 개인정보의 저장·전달·삭제 경로를 설명할 수 있습니다. Client 검증, 자동 Test와 서버·기기 통합 검증의 한계를 설명할 수 있습니다. React·Next.js의 공식 보안 공지를 기준으로 실제 설치·배포 Version의 영향 여부와 필요한 대응을 판단할 수 있습니다. AI의 “안전하다”는 설명을 근거 없이 승인하지 않고 확인할 파일과 설정을 지정할 수 있습니다. 확인된 문제, 미확정 계약과 검증하지 못한 위험을 구분해 병합 여부를 판단할 수 있습니다.",
  ...
  },
  {
    ...
    excerpt:
      "12단계에서 완성한 기능의 데이터 흐름도를 만들고 XSS, CSRF, 권한, 사용자 간 캐시 격리, Storage, Bridge, 로그와 Dependency 위험을 검토합니다. Front-end에서 해결할 항목과 Backend·App·배포 담당자에게 확인할 항목을 분리합니다. 즉시 미통과: 사용자 간 데이터 노출, Client만의 권한 검증, 인증정보의 URL·로그 저장, 신뢰하지 않은 HT…",
    content:
      "8. 보안 마스터 실습과 Fail Gate 12단계에서 완성한 기능의 데이터 흐름도를 만들고 XSS, CSRF, 권한, 사용자 간 캐시 격리, Storage, Bridge, 로그와 Dependency 위험을 검토합니다. Front-end에서 해결할 항목과 Backend·App·배포 담당자에게 확인할 항목을 분리합니다. 즉시 미통과: 사용자 간 데이터 노출, Client만의 권한 검증, 인증정보의 URL·로그 저장, 신뢰하지 않은 HTML 실행, 계약이 없는 보안 정책의 임의 확정이 하나라도 남으면 다른 점수와 관계없이 과정을 완료할 수 없습니다. 최종 통과 기준: 위협을 과장하거나 축소하지 않고 실제 데이터 흐름과 공격 조건을 근거로 우선순위를 정하며, 방어의 한계와 담당자별 책임을 설명할 수 있어야 합니다. 이전: 기능 통합과 리뷰 · 교육 과정 · 보안 기준 원본 13. Front-End 보안과 개인정보",
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: 기능 통합과 리뷰 · 교육 과정 · 보안 기준 원본 Browser와 WebView Client가 신뢰 경계가 아닌 이유를 설명합니다. 외부 입력에서 화면, URL, 저장소, 로그와 Bridge까지 데이터 흐름을 추적합니다. AI가 만든 코드의 XSS, 권한 오해, Secret·개인정보 노출과 사용자 전환 위험을 발견합니다. Front-end가 처리할 항목과 Backend·App·배포…",
    content:
      "1. 학습 목표 이전: 기능 통합과 리뷰 · 교육 과정 · 보안 기준 원본 Browser와 WebView Client가 신뢰 경계가 아닌 이유를 설명합니다. 외부 입력에서 화면, URL, 저장소, 로그와 Bridge까지 데이터 흐름을 추적합니다. AI가 만든 코드의 XSS, 권한 오해, Secret·개인정보 노출과 사용자 전환 위험을 발견합니다. Front-end가 처리할 항목과 Backend·App·배포 계약이 필요한 항목을 구분합니다.",
  ...
  },
  {
    ...
    section: "꼭 알아야 할 내용",
    excerpt:
      "추론 우선 초기값만으로 명확한 지역 변수와 간단한 반환 타입은 TypeScript에 맡깁니다. Props, 함수 입력과 모듈 경계처럼 사용법을 결정하는 값은 타입으로 표현합니다. 가능한 값 제한 상태가 몇 가지로 정해져 있으면 넓은 string 보다 Literal Union을 사용합니다. 불가능한 값이 컴파일 단계에서 들어오지 못하게 하는 것이 목적입니다. null 처리 null 과 unde…",
    content:
      "2. 꼭 알아야 할 내용 추론 우선 초기값만으로 명확한 지역 변수와 간단한 반환 타입은 TypeScript에 맡깁니다. Props, 함수 입력과 모듈 경계처럼 사용법을 결정하는 값은 타입으로 표현합니다. 가능한 값 제한 상태가 몇 가지로 정해져 있으면 넓은 string 보다 Literal Union을 사용합니다. 불가능한 값이 컴파일 단계에서 들어오지 못하게 하는 것이 목적입니다. null 처리 null 과 undefined 가능성을 조건문으로 확인합니다. ! 로 검사를 건너뛰면 실제 실행 오류 가능성은 사라지지 않습니다. 상태를 판별 가능한 Union으로 표현 isLoading , isError , data? 처럼 서로 모순될 수 있는 속성을 늘어놓기보다 상태를 구분하는 Literal 속성을 둡니다. 각 분기에서 필요한 값만 가지게 하여 로딩인데 데이터가 있는 것 같은 불가능한 조합을 타입 단계에서 제거합니다. 누락 없는 분기 switch 의 마지막에서 never 를 확인하면 새 상태가 추가됐을 때 빠진 UI 처리를 발견할 수 있습니다. 기본값으로 조용히 흡수하면 제품 상태가 추가되어도 기존 화면으로 잘못 표시될 수 있습니다. 타입 보존 도구의 목적 satisfies 는 값의 구체적인 추론을 유지하면서 요구 형태를 검사할 때 사용합니다. Generic은 입력과 출력의 실제 관계가 있을 때만 사용하며, 타입을 고급스럽게 보이게 하려는 Wrapper에는 추가하지 않습니다.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "결제 또는 예약 상태를 최소 여섯 상태의 판별 가능한 Union으로 모델링합니다. 새 상태 하나를 추가해 누락된 UI, Analytics와 버튼 정책이 모두 컴파일 단계에서 발견되는지 확인합니다. 잘못된 as , ! , 선택 속성 남용이 숨기는 버그도 한 개씩 재현합니다. 통과 기준: 타입 오류를 없애는 것이 아니라 도메인의 가능한 값과 전이를 정확히 표현하고, 타입 검사와 실행 중 검증의…",
    content:
      "7. 마스터 실습과 증거 결제 또는 예약 상태를 최소 여섯 상태의 판별 가능한 Union으로 모델링합니다. 새 상태 하나를 추가해 누락된 UI, Analytics와 버튼 정책이 모두 컴파일 단계에서 발견되는지 확인합니다. 잘못된 as , ! , 선택 속성 남용이 숨기는 버그도 한 개씩 재현합니다. 통과 기준: 타입 오류를 없애는 것이 아니라 도메인의 가능한 값과 전이를 정확히 표현하고, 타입 검사와 실행 중 검증의 책임 차이를 자신의 말로 설명할 수 있어야 합니다. 이전: JavaScript 기초 · 교육 과정 · 다음: TypeScript 경계 2. TypeScript 기본",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "추론 가능한 값에 타입을 반복하지 않는 이유를 설명할 수 있습니다. string 과 Literal Union 중 적절한 방식을 선택할 수 있습니다. any , as 와 ! 가 오류를 숨길 수 있음을 설명할 수 있습니다. 판별 가능한 Union과 누락 없는 분기로 불가능한 상태를 제거할 수 있습니다. satisfies , Generic과 타입 단언을 각각 어떤 근거로 선택하는지 설명할 수 있습…",
    content:
      "5. 완료 확인 추론 가능한 값에 타입을 반복하지 않는 이유를 설명할 수 있습니다. string 과 Literal Union 중 적절한 방식을 선택할 수 있습니다. any , as 와 ! 가 오류를 숨길 수 있음을 설명할 수 있습니다. 판별 가능한 Union과 누락 없는 분기로 불가능한 상태를 제거할 수 있습니다. satisfies , Generic과 타입 단언을 각각 어떤 근거로 선택하는지 설명할 수 있습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'type TicketStatus = | "available" | "used" | "expired" ; function getStatusLabel ( status: TicketStatus ) { if (status === "available" ) return "사용 가능" ; if (status === "used" ) return "사용 완료" ; return "기간 만료" ; } 함수 안에서…',
    content:
      '3. 코드 읽기 type TicketStatus = | "available" | "used" | "expired" ; function getStatusLabel ( status: TicketStatus ) { if (status === "available" ) return "사용 가능" ; if (status === "used" ) return "사용 완료" ; return "기간 만료" ; } 함수 안에서 조건을 확인할 때 TypeScript가 가능한 타입을 좁힙니다. 문자열 오타와 처리하지 않은 새 상태를 더 일찍 발견할 수 있습니다.',
  ...
  },
  {
    ...
    excerpt:
      "불가능한 상태를 표현하지 않기 isLoading , error , data 를 독립 속성으로 두면 로딩 중인데 데이터와 오류가 동시에 존재하는 조합이 생깁니다. status 를 판별자로 갖는 Union으로 상태별 필수 값을 묶고, UI가 허용된 상태만 받게 합니다. 누락 없는 분기 function assertNever(value: never): never { throw new Error(`처…",
    content:
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: JavaScript 기초 · 교육 과정 · 다음: TypeScript 경계 TypeScript의 추론에 맡길 타입과 직접 표현할 계약을 구분합니다. 정해진 값은 Literal Union으로 제한합니다. 조건문으로 Union을 안전하게 Narrowing합니다.",
    content:
      "1. 학습 목표 이전: JavaScript 기초 · 교육 과정 · 다음: TypeScript 경계 TypeScript의 추론에 맡길 타입과 직접 표현할 계약을 구분합니다. 정해진 값은 Literal Union으로 제한합니다. 조건문으로 Union을 안전하게 Narrowing합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "AI에게 Boolean 여러 개로 만든 상태를 판별 가능한 Union으로 바꾸게 합니다. 추론 가능한 중복 타입과 지나치게 넓은 string 을 찾게 합니다. any , 근거 없는 as 와 ! 를 사용하지 않는 대안을 설명하게 합니다. 이 타입이 실제 가능한 상태만 표현하는지 검토해줘. 중복 Boolean과 Optional 속성 때문에 불가능한 조합이 생기는지, Union과 Narrowing…",
    content:
      "4. AI와 함께 연습 AI에게 Boolean 여러 개로 만든 상태를 판별 가능한 Union으로 바꾸게 합니다. 추론 가능한 중복 타입과 지나치게 넓은 string 을 찾게 합니다. any , 근거 없는 as 와 ! 를 사용하지 않는 대안을 설명하게 합니다. 이 타입이 실제 가능한 상태만 표현하는지 검토해줘. 중복 Boolean과 Optional 속성 때문에 불가능한 조합이 생기는지, Union과 Narrowing으로 더 단순하게 만들 수 있는지 먼저 설명해줘.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "승인된 정상 응답, 필드 누락, 알 수 없는 Enum, 잘못된 날짜, 빈 본문, JSON이 아닌 본문을 Fixture로 만들고 Parser를 검증합니다. API 계약이 승인되지 않았다면 Parser를 만들지 말고 요청·응답·오류 질문서와 TBD 만 제출합니다. 통과 기준: 외부 unknown 이 안전한 내부 모델이 되는 단일 경계를 찾을 수 있고, 형태 검증·의미 검증·정규화·오류 표시의 책…",
    content:
      "7. 마스터 실습과 증거 승인된 정상 응답, 필드 누락, 알 수 없는 Enum, 잘못된 날짜, 빈 본문, JSON이 아닌 본문을 Fixture로 만들고 Parser를 검증합니다. API 계약이 승인되지 않았다면 Parser를 만들지 말고 요청·응답·오류 질문서와 TBD 만 제출합니다. 통과 기준: 외부 unknown 이 안전한 내부 모델이 되는 단일 경계를 찾을 수 있고, 형태 검증·의미 검증·정규화·오류 표시의 책임을 분리할 수 있어야 합니다. 이전: TypeScript 기본 · 교육 과정 · 다음: React 컴포넌트와 상태 3. TypeScript 경계",
  ...
  },
  {
    ...
    section: "신뢰 경계 설계",
    excerpt:
      "형태, 의미와 정규화의 순서 외부 값은 먼저 객체인지, 필요한 필드가 있는지, 필드 타입이 맞는지 확인합니다. 그 다음 날짜 범위, 허용 상태, 필드 사이의 관계 같은 도메인 의미를 검증합니다. 마지막으로 화면이 사용할 내부 모델로 정규화합니다. 검증과 변환이 여러 컴포넌트에 흩어지면 서로 다른 규칙이 생깁니다. 오류도 계약의 일부 Parser는 단순히 true 나 false 만 반환하지 않…",
    content:
      "6. 신뢰 경계 설계 형태, 의미와 정규화의 순서 외부 값은 먼저 객체인지, 필요한 필드가 있는지, 필드 타입이 맞는지 확인합니다. 그 다음 날짜 범위, 허용 상태, 필드 사이의 관계 같은 도메인 의미를 검증합니다. 마지막으로 화면이 사용할 내부 모델로 정규화합니다. 검증과 변환이 여러 컴포넌트에 흩어지면 서로 다른 규칙이 생깁니다. 오류도 계약의 일부 Parser는 단순히 true 나 false 만 반환하지 않고 필요한 경우 어느 경계에서 어떤 입력이 거부되었는지 구조화된 오류를 제공합니다. 단, 원본 개인정보나 전체 응답을 사용자 메시지와 로그에 그대로 노출하지 않습니다. 생성 타입이 보장하지 않는 것 OpenAPI 같은 명세에서 생성한 타입은 명세와 코드의 정적 연결을 돕지만 실제 서버가 명세를 지킨다는 사실, 오래된 앱의 Bridge 응답, Local Storage 값과 URL 입력을 보장하지 않습니다. 경계별 실제 신뢰 수준을 표로 작성합니다.",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "Props 타입이 컴포넌트 사용법을 표현하는지 검토할 수 있습니다. 타입 선언만으로 외부 응답이 안전해지지 않는 이유를 설명할 수 있습니다. Runtime Guard가 필요한 경계를 선택할 수 있습니다. 형태가 잘못된 응답과 업무상 빈 응답을 서로 다른 실패로 처리할 수 있습니다. 생성된 타입이 Runtime 검증을 대신하지 못하는 이유를 설명할 수 있습니다.",
    content:
      "5. 완료 확인 Props 타입이 컴포넌트 사용법을 표현하는지 검토할 수 있습니다. 타입 선언만으로 외부 응답이 안전해지지 않는 이유를 설명할 수 있습니다. Runtime Guard가 필요한 경계를 선택할 수 있습니다. 형태가 잘못된 응답과 업무상 빈 응답을 서로 다른 실패로 처리할 수 있습니다. 생성된 타입이 Runtime 검증을 대신하지 못하는 이유를 설명할 수 있습니다.",
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: TypeScript 기본 · 교육 과정 · 다음: React 컴포넌트와 상태 Props와 함수 입력을 사용자의 의도가 드러나는 타입으로 표현합니다. TypeScript 타입 선언과 Runtime 검증의 차이를 설명합니다. 외부 입력의 unknown 을 확인한 뒤 사용합니다.",
    content:
      "1. 학습 목표 이전: TypeScript 기본 · 교육 과정 · 다음: React 컴포넌트와 상태 Props와 함수 입력을 사용자의 의도가 드러나는 타입으로 표현합니다. TypeScript 타입 선언과 Runtime 검증의 차이를 설명합니다. 외부 입력의 unknown 을 확인한 뒤 사용합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "화면 하나의 Props와 외부 응답 경계를 AI에게 찾게 합니다. 타입 선언만 있고 Runtime 검증이 필요한 입력을 구분하게 합니다. 기능 전용 타입을 공통 폴더로 너무 일찍 이동하지 않았는지 검토하게 합니다. 이 코드에서 모듈 경계를 통과하는 값을 찾아줘. TypeScript 타입만 있으면 충분한 값과 Runtime 검증이 필요한 값을 구분하고, 근거 없는 타입 단언 없이 처리하는 방법…",
    content:
      "4. AI와 함께 연습 화면 하나의 Props와 외부 응답 경계를 AI에게 찾게 합니다. 타입 선언만 있고 Runtime 검증이 필요한 입력을 구분하게 합니다. 기능 전용 타입을 공통 폴더로 너무 일찍 이동하지 않았는지 검토하게 합니다. 이 코드에서 모듈 경계를 통과하는 값을 찾아줘. TypeScript 타입만 있으면 충분한 값과 Runtime 검증이 필요한 값을 구분하고, 근거 없는 타입 단언 없이 처리하는 방법을 설명해줘.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "검색·필터·선택·인라인 편집이 있는 목록을 구현합니다. 먼저 값별 소유자와 수명을 표로 작성하고, Props 복사·Index Key·중복 파생 State가 있는 결함 버전을 리뷰합니다. React DevTools로 불필요한 렌더를 관찰하되 측정 없이 Memoization을 추가하지 않습니다. 통과 기준: 각 State가 왜 그 컴포넌트에 있으며 언제 보존·초기화되는지 설명하고, Context…",
    content:
      "7. 마스터 실습과 증거 검색·필터·선택·인라인 편집이 있는 목록을 구현합니다. 먼저 값별 소유자와 수명을 표로 작성하고, Props 복사·Index Key·중복 파생 State가 있는 결함 버전을 리뷰합니다. React DevTools로 불필요한 렌더를 관찰하되 측정 없이 Memoization을 추가하지 않습니다. 통과 기준: 각 State가 왜 그 컴포넌트에 있으며 언제 보존·초기화되는지 설명하고, Context나 전역 Store가 필요한 실제 공유 범위를 증명할 수 있어야 합니다. 이전: TypeScript 경계 · 교육 과정 · 다음: React Effect와 폼 4. React 컴포넌트와 상태",
  ...
  },
  {
    ...
    excerpt:
      "상태 인벤토리 구현 전에 모든 값을 원본 상태, 파생 값, 서버 상태, URL 상태, 폼 초안과 일시적 UI 상태로 분류합니다. 다른 값에서 계산할 수 있다면 State에 저장하지 않습니다. URL로 공유하거나 새로고침 뒤 복원해야 하는 선택값은 지역 State보다 URL이 적합할 수 있습니다. 상태 보존은 위치와 Key로 결정된다 조건문 모양이 아니라 렌더 트리의 같은 위치와 같은 Key인…",
    content:
      "6. 상태와 책임을 설계하기 상태 인벤토리 구현 전에 모든 값을 원본 상태, 파생 값, 서버 상태, URL 상태, 폼 초안과 일시적 UI 상태로 분류합니다. 다른 값에서 계산할 수 있다면 State에 저장하지 않습니다. URL로 공유하거나 새로고침 뒤 복원해야 하는 선택값은 지역 State보다 URL이 적합할 수 있습니다. 상태 보존은 위치와 Key로 결정된다 조건문 모양이 아니라 렌더 트리의 같은 위치와 같은 Key인지가 State 보존을 결정합니다. 사용자나 편집 대상이 바뀔 때 State가 유지되어야 하는지 초기화되어야 하는지 먼저 정하고 Key를 그 정책에 맞게 사용합니다. 컴포넌트 분리의 판단 기준 줄 수가 아니라 변경 이유, 상태 소유권, 재사용되는 행동과 독립 검증 가능성으로 분리합니다. 실제 반복이 없는 범용 Wrapper와 지나치게 많은 Boolean Props는 책임이 불분명하다는 신호입니다.",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "State가 아닌 계산 가능한 값을 찾을 수 있습니다. 상태 소유자를 선택한 이유를 설명할 수 있습니다. 컴포넌트를 합치거나 나눌 근거를 책임으로 설명할 수 있습니다. key 와 트리 위치가 상태 보존에 미치는 영향을 설명할 수 있습니다. Props, 지역 State, Context와 전역 Store 중 상태의 제어 주체를 선택할 수 있습니다.",
    content:
      "5. 완료 확인 State가 아닌 계산 가능한 값을 찾을 수 있습니다. 상태 소유자를 선택한 이유를 설명할 수 있습니다. 컴포넌트를 합치거나 나눌 근거를 책임으로 설명할 수 있습니다. key 와 트리 위치가 상태 보존에 미치는 영향을 설명할 수 있습니다. Props, 지역 State, Context와 전역 Store 중 상태의 제어 주체를 선택할 수 있습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'function TicketList ( { tickets }: TicketListProps ) { const [filter, setFilter] = useState < TicketFilter >( "all" ); const visibleTickets = tickets. filter ( ( ticket ) => filter === "all" || ticket.status === filter,…',
    content:
      '3. 코드 읽기 function TicketList ( { tickets }: TicketListProps ) { const [filter, setFilter] = useState < TicketFilter >( "all" ); const visibleTickets = tickets. filter ( ( ticket ) => filter === "all" || ticket.status === filter, ); return < TicketResults tickets = {visibleTickets} /> ; } visibleTickets 는 기존 데이터와 필터에서 계산되므로 State가 아닙니다. 필터를 바꾸면 렌더링 중 다시 계산됩니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: TypeScript 경계 · 교육 과정 · 다음: React Effect와 폼 Props와 State의 책임을 구분합니다. 최소한의 State만 저장하고 파생 값은 계산합니다. 상태를 사용하는 가장 가까운 공통 부모를 소유자로 선택합니다.",
    content:
      "1. 학습 목표 이전: TypeScript 경계 · 교육 과정 · 다음: React Effect와 폼 Props와 State의 책임을 구분합니다. 최소한의 State만 저장하고 파생 값은 계산합니다. 상태를 사용하는 가장 가까운 공통 부모를 소유자로 선택합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "Figma 화면 하나에서 사용자 행동과 바뀌는 값만 먼저 나열하게 합니다. 각 State의 소유자와 파생 값을 AI에게 표로 정리하게 합니다. 컴포넌트 분리가 Figma 계층을 그대로 옮긴 것인지 검토하게 합니다. 이 화면의 값을 Props, State와 파생 값으로 나눠줘. 각 State를 누가 사용하고 변경하는지 확인해 가장 가까운 소유자를 제안하고, Figma 레이어 수가 아니라 책임으…",
    content:
      "4. AI와 함께 연습 Figma 화면 하나에서 사용자 행동과 바뀌는 값만 먼저 나열하게 합니다. 각 State의 소유자와 파생 값을 AI에게 표로 정리하게 합니다. 컴포넌트 분리가 Figma 계층을 그대로 옮긴 것인지 검토하게 합니다. 이 화면의 값을 Props, State와 파생 값으로 나눠줘. 각 State를 누가 사용하고 변경하는지 확인해 가장 가까운 소유자를 제안하고, Figma 레이어 수가 아니라 책임으로 컴포넌트를 나눠줘.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "회원 정보 수정 폼에 동기 검증, 서버 오류 매핑, 제출 취소와 중복 제출 방지를 구현합니다. 느린 응답 중 사용자를 전환하고, 이전 응답이 새 사용자의 폼을 덮지 않는지 테스트합니다. 각 Effect가 동기화하는 외부 시스템을 한 문장으로 설명합니다. 통과 기준: Event와 Effect를 구분하고, 오래된 Closure·누락 Dependency·경쟁 상태를 재현하며, UI 초안과 서버의…",
    content:
      "7. 마스터 실습과 증거 회원 정보 수정 폼에 동기 검증, 서버 오류 매핑, 제출 취소와 중복 제출 방지를 구현합니다. 느린 응답 중 사용자를 전환하고, 이전 응답이 새 사용자의 폼을 덮지 않는지 테스트합니다. 각 Effect가 동기화하는 외부 시스템을 한 문장으로 설명합니다. 통과 기준: Event와 Effect를 구분하고, 오래된 Closure·누락 Dependency·경쟁 상태를 재현하며, UI 초안과 서버의 최종 진실이 충돌할 때 복구 정책을 설명할 수 있어야 합니다. 이전: React 컴포넌트와 상태 · 교육 과정 · 다음: Next.js App Router 5. React Effect와 폼",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "Event와 Effect의 차이를 실제 코드로 설명할 수 있습니다. 폼 값과 서버 상태를 같은 Store에 넣지 않는 이유를 설명할 수 있습니다. 필드 오류, 업무 오류와 네트워크 오류를 구분할 수 있습니다. Effect의 시작과 정리 경로, 의존성과 경쟁 상태를 검토할 수 있습니다. 폼 상태와 Mutation 상태를 중복 저장하지 않고 성공·실패 UX를 연결할 수 있습니다.",
    content:
      "5. 완료 확인 Event와 Effect의 차이를 실제 코드로 설명할 수 있습니다. 폼 값과 서버 상태를 같은 Store에 넣지 않는 이유를 설명할 수 있습니다. 필드 오류, 업무 오류와 네트워크 오류를 구분할 수 있습니다. Effect의 시작과 정리 경로, 의존성과 경쟁 상태를 검토할 수 있습니다. 폼 상태와 Mutation 상태를 중복 저장하지 않고 성공·실패 UX를 연결할 수 있습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      "async function handleSubmit ( values: ReservationValues ) { if (isSubmitting) return ; setIsSubmitting ( true ); try { await saveReservation (values); } finally { setIsSubmitting ( false ); } } 제출은 사용자 Event에서 시작하며 중복 실행…",
    content:
      "3. 코드 읽기 async function handleSubmit ( values: ReservationValues ) { if (isSubmitting) return ; setIsSubmitting ( true ); try { await saveReservation (values); } finally { setIsSubmitting ( false ); } } 제출은 사용자 Event에서 시작하며 중복 실행을 막습니다. 실제 구현에서는 성공, 필드 오류, 업무 오류와 네트워크 오류에 맞는 화면 결과를 연결합니다.",
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: React 컴포넌트와 상태 · 교육 과정 · 다음: Next.js App Router 사용자 Event와 외부 시스템 동기화를 위한 Effect를 구분합니다. 입력 값, 필드 오류, 제출 상태와 서버 오류를 구분합니다. 제출 중 중복 요청을 막고 사용자에게 진행 상태를 제공합니다.",
    content:
      "1. 학습 목표 이전: React 컴포넌트와 상태 · 교육 과정 · 다음: Next.js App Router 사용자 Event와 외부 시스템 동기화를 위한 Effect를 구분합니다. 입력 값, 필드 오류, 제출 상태와 서버 오류를 구분합니다. 제출 중 중복 요청을 막고 사용자에게 진행 상태를 제공합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "기존 useEffect 마다 어떤 외부 시스템과 동기화하는지 AI에게 설명하게 합니다. 폼의 입력·제출·성공·실패 상태를 먼저 나열하게 합니다. 중복 제출과 서버 오류 원문 노출 여부를 검토하게 합니다. 이 컴포넌트의 useEffect마다 동기화하는 외부 시스템을 설명해줘. Event나 렌더링 중 계산으로 바꿀 수 있는 Effect를 구분하고, 폼 제출의 중복 실행과 오류 상태도 함께 검토해…",
    content:
      "4. AI와 함께 연습 기존 useEffect 마다 어떤 외부 시스템과 동기화하는지 AI에게 설명하게 합니다. 폼의 입력·제출·성공·실패 상태를 먼저 나열하게 합니다. 중복 제출과 서버 오류 원문 노출 여부를 검토하게 합니다. 이 컴포넌트의 useEffect마다 동기화하는 외부 시스템을 설명해줘. Event나 렌더링 중 계산으로 바꿀 수 있는 Effect를 구분하고, 폼 제출의 중복 실행과 오류 상태도 함께 검토해줘.",
  ...
  },
  {
    ...
    excerpt:
      "Effect 제거 검토 Effect를 작성하기 전에 렌더 중 계산, 사용자 Event, State 초기값, Key를 통한 초기화로 해결할 수 있는지 확인합니다. 다른 State를 맞추기 위한 Effect는 중간의 잘못된 화면과 추가 렌더를 만들기 쉽습니다. Setup과 Cleanup의 대칭 구독, Timer, Event Listener와 요청은 생성에 대응하는 정리 경로가 있어야 합니다. D…",
    content:
      "6. Effect와 폼의 수명 설계 Effect 제거 검토 Effect를 작성하기 전에 렌더 중 계산, 사용자 Event, State 초기값, Key를 통한 초기화로 해결할 수 있는지 확인합니다. 다른 State를 맞추기 위한 Effect는 중간의 잘못된 화면과 추가 렌더를 만들기 쉽습니다. Setup과 Cleanup의 대칭 구독, Timer, Event Listener와 요청은 생성에 대응하는 정리 경로가 있어야 합니다. Dependency가 바뀌는 순간 이전 Effect의 Cleanup이 먼저 실행된다는 점을 이용해 사용자 전환과 화면 이탈에서 오래된 작업을 종료합니다. 폼의 상태 전이 폼을 단순 Boolean 모음으로 다루지 말고 편집 전, 편집 중, 검증 실패, 제출 중, 성공, 서버 거부, 취소 상태로 모델링합니다. 제출 중 중복 실행, 서버 필드 오류, 전체 오류, 응답 전 이탈과 서버 값 변경을 포함합니다.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "공개 콘텐츠, 로그인 사용자 정보, 상호작용 폼이 함께 있는 Route를 설계합니다. 파일별 실행 위치, 직렬화 경계, 캐시·신선도 정책을 표로 제출하고 서버 전용 모듈이 Client Bundle에 들어가지 않는지 확인합니다. 통과 기준: 습관적으로 전체 화면에 use client 를 붙이지 않고, 로딩·오류·Not Found 책임과 사용자별 데이터 격리를 포함한 경계 선택을 설명할 수 있어…",
    content:
      "7. 마스터 실습과 증거 공개 콘텐츠, 로그인 사용자 정보, 상호작용 폼이 함께 있는 Route를 설계합니다. 파일별 실행 위치, 직렬화 경계, 캐시·신선도 정책을 표로 제출하고 서버 전용 모듈이 Client Bundle에 들어가지 않는지 확인합니다. 통과 기준: 습관적으로 전체 화면에 use client 를 붙이지 않고, 로딩·오류·Not Found 책임과 사용자별 데이터 격리를 포함한 경계 선택을 설명할 수 있어야 합니다. 이전: React Effect와 폼 · 교육 과정 · 다음: Tailwind UI와 접근성 6. Next.js App Router",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "Route와 Layout의 책임을 설명할 수 있습니다. Client Component가 필요한 이유를 구체적으로 말할 수 있습니다. 확정된 App Router 규칙과 미확정 렌더링 정책을 구분할 수 있습니다. Hydration 불일치와 Client Bundle로 서버 전용 코드가 유입되는 위험을 찾을 수 있습니다. URL, Route 경계와 컴포넌트 상태의 책임을 구분할 수 있습니다.",
    content:
      "5. 완료 확인 Route와 Layout의 책임을 설명할 수 있습니다. Client Component가 필요한 이유를 구체적으로 말할 수 있습니다. 확정된 App Router 규칙과 미확정 렌더링 정책을 구분할 수 있습니다. Hydration 불일치와 Client Bundle로 서버 전용 코드가 유입되는 위험을 찾을 수 있습니다. URL, Route 경계와 컴포넌트 상태의 책임을 구분할 수 있습니다.",
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: React Effect와 폼 · 교육 과정 · 다음: Tailwind UI와 접근성 page , layout , Loading과 Error 경계의 역할을 구분합니다. Server Component를 기본으로 두고 Client 전환 이유를 설명합니다. 브라우저 API와 Client State가 필요한 작은 경계만 분리합니다.",
    content:
      "1. 학습 목표 이전: React Effect와 폼 · 교육 과정 · 다음: Tailwind UI와 접근성 page , layout , Loading과 Error 경계의 역할을 구분합니다. Server Component를 기본으로 두고 Client 전환 이유를 설명합니다. 브라우저 API와 Client State가 필요한 작은 경계만 분리합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "화면의 각 컴포넌트가 상태, Event 또는 브라우저 API를 사용하는지 표시하게 합니다. Client 전환이 필요한 가장 작은 경계를 제안하게 합니다. 현재 프로젝트에서 확정되지 않은 캐싱과 데이터 패칭 가정을 따로 적게 합니다. 이 화면에서 Server Component를 기본으로 유지하고, 상태·Event·브라우저 API가 필요한 가장 작은 Client 경계를 찾아줘. 캐싱과 데이터 패…",
    content:
      "4. AI와 함께 연습 화면의 각 컴포넌트가 상태, Event 또는 브라우저 API를 사용하는지 표시하게 합니다. Client 전환이 필요한 가장 작은 경계를 제안하게 합니다. 현재 프로젝트에서 확정되지 않은 캐싱과 데이터 패칭 가정을 따로 적게 합니다. 이 화면에서 Server Component를 기본으로 유지하고, 상태·Event·브라우저 API가 필요한 가장 작은 Client 경계를 찾아줘. 캐싱과 데이터 패칭 정책은 추측하지 말고 확인 항목으로 분리해줘.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "복합 폼과 Dialog를 마우스 없이 완료합니다. 320px·200% 확대·긴 문구·고대비 모드에서 검수하고 자동 검사와 수동 키보드 결과를 별도로 기록합니다. 색상만 제거해도 상태가 구분되는지 확인합니다. 통과 기준: 특정 Figma 좌표를 복사하지 않고 Layout 제약을 설명하며, 키보드·Focus·이름·오류 관계를 실제 동작으로 증명해야 합니다. 이전: Next.js App Route…",
    content:
      "7. 마스터 실습과 증거 복합 폼과 Dialog를 마우스 없이 완료합니다. 320px·200% 확대·긴 문구·고대비 모드에서 검수하고 자동 검사와 수동 키보드 결과를 별도로 기록합니다. 색상만 제거해도 상태가 구분되는지 확인합니다. 통과 기준: 특정 Figma 좌표를 복사하지 않고 Layout 제약을 설명하며, 키보드·Focus·이름·오류 관계를 실제 동작으로 증명해야 합니다. 이전: Next.js App Router · 교육 과정 · 다음: API 계약과 오류 7. Tailwind UI와 접근성",
  ...
  },
  {
    ...
    excerpt:
      "반응형은 Breakpoint 목록이 아니다 320px 너비, 긴 한국어·영어 문구, 200% 확대, 모바일 키보드, Safe Area와 가로·세로 전환을 입력 조건으로 봅니다. 고정 높이와 무분별한 overflow-hidden 으로 문제를 감추지 않습니다. 접근 가능한 이름과 관계 Label, 설명, 오류와 입력의 관계를 의미 있는 HTML과 aria-describedby 로 연결합니다. A…",
    content:
      "6. 제약 기반 UI와 접근성 심화 반응형은 Breakpoint 목록이 아니다 320px 너비, 긴 한국어·영어 문구, 200% 확대, 모바일 키보드, Safe Area와 가로·세로 전환을 입력 조건으로 봅니다. 고정 높이와 무분별한 overflow-hidden 으로 문제를 감추지 않습니다. 접근 가능한 이름과 관계 Label, 설명, 오류와 입력의 관계를 의미 있는 HTML과 aria-describedby 로 연결합니다. ARIA는 올바른 기본 요소가 없을 때 보완하며, Button을 Div로 다시 만들기 위한 도구가 아닙니다. Web Content Accessibility Guidelines (WCAG) 를 추가 기준으로 사용합니다. Focus는 사용자 흐름이다 Dialog가 열리면 적절한 시작점으로 이동하고, 닫히면 호출한 요소로 돌아와야 합니다. 오류 발생 시 첫 오류로 무조건 이동시키기보다 현재 작업과 오류 요약 방식에 맞는 정책을 정합니다. 동적 완료·오류는 필요한 경우 Live Region으로 전달합니다. Storybook은 구현과 함께 갱신한다 공통 UI, 독립 검수 가치가 있는 Feature 또는 사용자에게 노출되는 Screen이 대상이면 같은 변경에서 기존 Story를 확인하고 필요한 상태와 Viewport를 갱신합니다. 실제 프로젝트에 정의된 Storybook 정적 Build와 검사를 실행하며, Script나 Addon이 설치된 것처럼 추측하지 않습니다.",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'export function TicketCard ( { title, onOpen }: TicketCardProps ) { return ( < article className = "rounded-lg border border-border bg-card p-4 text-card-foreground" > < h2 > {title} </ h2 > < Button type = "button" onCl…',
    content:
      '3. 코드 읽기 export function TicketCard ( { title, onOpen }: TicketCardProps ) { return ( < article className = "rounded-lg border border-border bg-card p-4 text-card-foreground" > < h2 > {title} </ h2 > < Button type = "button" onClick = {onOpen} > 상세 보기 </ Button > </ article > ); } 색상은 역할 기반 토큰을 사용하고 실행 동작은 Button으로 제공합니다. 화면이 맞아 보여도 긴 문구, 모바일 너비, 키보드와 Focus를 별도로 확인합니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: Next.js App Router · 교육 과정 · 다음: API 계약과 오류 Tailwind Utility가 표현하는 CSS Layout과 반응형 동작을 설명합니다. 원시 색상보다 의미 기반 토큰과 기존 shadcn/ui를 우선합니다. Figma의 의도를 실제 코드·Story와 연결하고 Safari 15 호환 범위를 확인합니다. 의미 있는 HTML, Label, 키보드와 Focus를…",
    content:
      "1. 학습 목표 이전: Next.js App Router · 교육 과정 · 다음: API 계약과 오류 Tailwind Utility가 표현하는 CSS Layout과 반응형 동작을 설명합니다. 원시 색상보다 의미 기반 토큰과 기존 shadcn/ui를 우선합니다. Figma의 의도를 실제 코드·Story와 연결하고 Safari 15 호환 범위를 확인합니다. 의미 있는 HTML, Label, 키보드와 Focus를 확인합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "AI에게 Figma 값과 기존 토큰·컴포넌트의 매핑을 먼저 조사하게 합니다. 원시 색상, 임의 값과 의미 없는 클릭 요소를 검토하게 합니다. 모바일 너비, 긴 문구, 오류 상태와 키보드 동작을 확인하게 합니다. 이 UI에서 기존 shadcn/ui와 의미 기반 토큰을 먼저 찾아줘. 원시 색상, 불필요한 임의 값, div 클릭, Label·Focus·접근 가능한 이름 누락을 검토하고 모바일 너비…",
    content:
      "4. AI와 함께 연습 AI에게 Figma 값과 기존 토큰·컴포넌트의 매핑을 먼저 조사하게 합니다. 원시 색상, 임의 값과 의미 없는 클릭 요소를 검토하게 합니다. 모바일 너비, 긴 문구, 오류 상태와 키보드 동작을 확인하게 합니다. 이 UI에서 기존 shadcn/ui와 의미 기반 토큰을 먼저 찾아줘. 원시 색상, 불필요한 임의 값, div 클릭, Label·Focus·접근 가능한 이름 누락을 검토하고 모바일 너비에서 확인할 항목도 정리해줘.",
  ...
  },
  {
    ...
    excerpt:
      "HTTP 응답은 Status와 본문의 조합 Hypertext Transfer Protocol (HTTP) 응답은 성공 여부만으로 JSON 본문을 보장하지 않습니다. 204, 빈 본문, JSON이 아닌 오류 본문과 잘못된 Content-Type을 구분합니다. Parsing 실패를 서버가 보낸 도메인 오류로 오인하지 않습니다. 재시도는 안전성 결정이다 Timeout과 네트워크 단절 뒤 서버가 요…",
    content:
      "6. 계약과 실패 모델 심화 HTTP 응답은 Status와 본문의 조합 Hypertext Transfer Protocol (HTTP) 응답은 성공 여부만으로 JSON 본문을 보장하지 않습니다. 204, 빈 본문, JSON이 아닌 오류 본문과 잘못된 Content-Type을 구분합니다. Parsing 실패를 서버가 보낸 도메인 오류로 오인하지 않습니다. 재시도는 안전성 결정이다 Timeout과 네트워크 단절 뒤 서버가 요청을 처리했는지 알 수 없을 수 있습니다. 조회와 멱등성이 보장된 요청, 결제·예약처럼 중복 실행 위험이 있는 Mutation의 재시도 정책을 다르게 정합니다. 지수 Backoff와 사용자 취소, Offline 복구도 계약에 포함합니다. 브라우저 보안 경계 Cross-Origin Resource Sharing (CORS) 는 서버가 허용한 다른 Origin의 응답을 브라우저가 읽을 수 있게 하는 HTTP 헤더 기반 메커니즘입니다. Front-end에서 우회 옵션을 넣는 문제가 아니며, 인증과 권한을 대신하지도 않습니다.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "승인된 계약 하나를 Status·본문·Parser·화면 상태·재시도 정책으로 연결합니다. 204, 400, 401, 403, 404, 409, 429, 500, Timeout, Offline과 취소를 주입하고 사용자 메시지, 로그, 재시도 가능 여부를 표로 제출합니다. 통과 기준: 네트워크 오류, HTTP 오류, Parsing 오류, 도메인 오류와 취소를 구분하며 미확정 계약을 코드로 추측하…",
    content:
      "7. 마스터 실습과 증거 승인된 계약 하나를 Status·본문·Parser·화면 상태·재시도 정책으로 연결합니다. 204, 400, 401, 403, 404, 409, 429, 500, Timeout, Offline과 취소를 주입하고 사용자 메시지, 로그, 재시도 가능 여부를 표로 제출합니다. 통과 기준: 네트워크 오류, HTTP 오류, Parsing 오류, 도메인 오류와 취소를 구분하며 미확정 계약을 코드로 추측하지 않아야 합니다. 이전: Tailwind UI와 접근성 · 교육 과정 · 다음: 서버·클라이언트 상태 8. API 계약과 오류",
  ...
  },
  {
    ...
    section: "코드 읽기",
    excerpt:
      'async function requestTickets ( signal: AbortSignal ) { const response = await fetch ( "/api/tickets" , { signal }); if (!response.ok) { throw new RequestFailedError (response.status); } const data: unknown = await respo…',
    content:
      '3. 코드 읽기 async function requestTickets ( signal: AbortSignal ) { const response = await fetch ( "/api/tickets" , { signal }); if (!response.ok) { throw new RequestFailedError (response.status); } const data: unknown = await response. json (); return parseTickets (data); } 요청 함수는 HTTP Status를 보존하고 외부 JSON을 Parser로 검증합니다. 401 이 세션 만료인지 이 함수에서 단정하지 않으며, 실제 오류 Class와 공통 응답 구조는 API 계약이 확정된 뒤 적용합니다.',
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: Tailwind UI와 접근성 · 교육 과정 · 다음: 서버·클라이언트 상태 요청·응답 필드, nullable, 날짜·금액과 오류 계약을 질문합니다. HTTP 오류, 업무 오류, 인증 만료와 네트워크 오류를 구분합니다. 서버 오류 원문과 민감정보를 사용자나 로그에 노출하지 않습니다.",
    content:
      "1. 학습 목표 이전: Tailwind UI와 접근성 · 교육 과정 · 다음: 서버·클라이언트 상태 요청·응답 필드, nullable, 날짜·금액과 오류 계약을 질문합니다. HTTP 오류, 업무 오류, 인증 만료와 네트워크 오류를 구분합니다. 서버 오류 원문과 민감정보를 사용자나 로그에 노출하지 않습니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "AI에게 화면에 필요한 데이터에서 Backend 질문 목록을 만들게 합니다. 확인된 계약과 가정한 필드를 분리하게 합니다. 성공, 빈 값, 업무 오류, 인증 만료, Timeout과 오프라인 화면을 제안하게 합니다. 이 화면을 구현하기 전에 Backend에 확인할 API 계약 질문을 만들어줘. 확인된 사실과 추측을 분리하고, 성공·빈 값·업무 오류·401·Timeout·오프라인을 화면에서 어떻…",
    content:
      "4. AI와 함께 연습 AI에게 화면에 필요한 데이터에서 Backend 질문 목록을 만들게 합니다. 확인된 계약과 가정한 필드를 분리하게 합니다. 성공, 빈 값, 업무 오류, 인증 만료, Timeout과 오프라인 화면을 제안하게 합니다. 이 화면을 구현하기 전에 Backend에 확인할 API 계약 질문을 만들어줘. 확인된 사실과 추측을 분리하고, 성공·빈 값·업무 오류·401·Timeout·오프라인을 화면에서 어떻게 구분할지 제안해줘. 계약은 임의로 확정하지 마.",
  ...
  },
  {
    ...
    section: "마스터 실습과 증거",
    excerpt:
      "두 사용자와 두 필터가 있는 조회·수정 흐름을 구현합니다. Query Key 충돌, 낙관적 갱신 실패, 사용자 전환 중 늦은 응답을 의도적으로 재현하고 수정합니다. 캐시 Inspector와 테스트로 이전 사용자 데이터가 남지 않음을 확인합니다. 통과 기준: 지역·폼·URL·서버·세션 상태를 구분하고, 데이터별 소유자·신선도·정리 시점을 설명하며 서버 데이터를 전역 Store에 불필요하게 복제…",
    content:
      "7. 마스터 실습과 증거 두 사용자와 두 필터가 있는 조회·수정 흐름을 구현합니다. Query Key 충돌, 낙관적 갱신 실패, 사용자 전환 중 늦은 응답을 의도적으로 재현하고 수정합니다. 캐시 Inspector와 테스트로 이전 사용자 데이터가 남지 않음을 확인합니다. 통과 기준: 지역·폼·URL·서버·세션 상태를 구분하고, 데이터별 소유자·신선도·정리 시점을 설명하며 서버 데이터를 전역 Store에 불필요하게 복제하지 않아야 합니다. 이전: API 계약과 오류 · 교육 과정 · 다음: WebView Bridge와 보안 9. 서버·클라이언트 상태",
  ...
  },
  {
    ...
    section: "완료 확인",
    excerpt:
      "상태 도구를 소유자와 수명으로 선택할 수 있습니다. TanStack Query 데이터와 Zustand 상태를 구분할 수 있습니다. Persist가 필요한 실제 요구와 보안 위험을 설명할 수 있습니다. 세션 확인 실패와 비로그인을 구분하고 회원 상세 데이터를 세션에 넣지 않을 수 있습니다. Query Key, 무효화, Cache 갱신과 낙관적 갱신의 선택 근거를 설명할 수 있습니다. 로그아웃·…",
    content:
      "5. 완료 확인 상태 도구를 소유자와 수명으로 선택할 수 있습니다. TanStack Query 데이터와 Zustand 상태를 구분할 수 있습니다. Persist가 필요한 실제 요구와 보안 위험을 설명할 수 있습니다. 세션 확인 실패와 비로그인을 구분하고 회원 상세 데이터를 세션에 넣지 않을 수 있습니다. Query Key, 무효화, Cache 갱신과 낙관적 갱신의 선택 근거를 설명할 수 있습니다. 로그아웃·사용자 전환 때 제거해야 할 사용자 범위 상태를 추적할 수 있습니다.",
  ...
  },
  {
    ...
    section: "학습 목표",
    excerpt:
      "이전: API 계약과 오류 · 교육 과정 · 다음: WebView Bridge와 보안 지역 UI, URL, 폼, 서버와 공유 클라이언트 상태를 구분합니다. 서버 데이터를 TanStack Query 밖의 Store에 복제하지 않습니다. React로 해결되지 않는 공유 UI 상태에만 Zustand를 검토합니다.",
    content:
      "1. 학습 목표 이전: API 계약과 오류 · 교육 과정 · 다음: WebView Bridge와 보안 지역 UI, URL, 폼, 서버와 공유 클라이언트 상태를 구분합니다. 서버 데이터를 TanStack Query 밖의 Store에 복제하지 않습니다. React로 해결되지 않는 공유 UI 상태에만 Zustand를 검토합니다.",
  ...
  },
  {
    ...
    section: "AI와 함께 연습",
    excerpt:
      "화면의 모든 상태를 소유자와 수명 기준으로 분류하게 합니다. 서버 데이터가 Zustand나 지역 State에 복제됐는지 찾게 합니다. 로그아웃과 사용자 전환 때 초기화할 캐시와 UI 상태를 구분하게 합니다. 이 기능의 상태를 지역 UI, URL, 폼, 서버, 공유 클라이언트 상태로 분류해줘. 현재 도구가 적절한 이유를 설명하고, 서버 데이터나 파생 값이 다른 Store에 중복 저장된 부분과…",
    content:
      "4. AI와 함께 연습 화면의 모든 상태를 소유자와 수명 기준으로 분류하게 합니다. 서버 데이터가 Zustand나 지역 State에 복제됐는지 찾게 합니다. 로그아웃과 사용자 전환 때 초기화할 캐시와 UI 상태를 구분하게 합니다. 이 기능의 상태를 지역 UI, URL, 폼, 서버, 공유 클라이언트 상태로 분류해줘. 현재 도구가 적절한 이유를 설명하고, 서버 데이터나 파생 값이 다른 Store에 중복 저장된 부분과 사용자 전환 시 초기화할 항목을 찾아줘.",
  ...
  },
  {
    ...
    excerpt:
      "이 문서는 앱 개발자와 APP 구축 일정을 검토하기 위한 산정 자료입니다. 특정 착수일이나 완료일을 전제로 하지 않으며, 실제 개발 범위와 책임이 현재 가정과 일치하는지 확인한 뒤 일정을 조정합니다. 개발자 구성: 미들급 Flutter 개발자 1명 AI 활용: 반복 구현, Test, 오류 분석과 문서화에 적극 활용 구축 조건: 기존 구현을 전제로 하지 않는 신규 구축 기간 단위: 1개월을 2…",
    content:
      "1. 문서 목적과 산정 기준 이 문서는 앱 개발자와 APP 구축 일정을 검토하기 위한 산정 자료입니다. 특정 착수일이나 완료일을 전제로 하지 않으며, 실제 개발 범위와 책임이 현재 가정과 일치하는지 확인한 뒤 일정을 조정합니다. 개발자 구성: 미들급 Flutter 개발자 1명 AI 활용: 반복 구현, Test, 오류 분석과 문서화에 적극 활용 구축 조건: 기존 구현을 전제로 하지 않는 신규 구축 기간 단위: 1개월을 20근무일로 환산 포함 범위: 구현, 연동, 실기기 QA와 출시 준비 주 5일 근무의 월평균 평일은 약 21.7일입니다. 공휴일, 연차, 회사 휴무일과 전사 일정 등을 고려해 과도하게 낙관적인 환산을 피하기 위해 1개월을 20근무일 로 계산합니다. 실제 착수일이 정해지면 해당 기간의 달력을 기준으로 다시 계산해야 합니다.",
  ...
  },
  {
    ...
    section: "산정 결과 요약",
    excerpt:
      "구분 예상 기간 포함 범위 기능 구현 완료 95~120근무일 Native 기능 및 주요 연동 구현 통합 QA·출시 준비 완료 120~155근무일 실기기 QA, 테스트 배포와 안정화 포함 외부 Store 심사 5~10달력일 이상 개발 근무일과 별도인 외부 대기 기본 산정값은 미들급 Flutter 개발자 1명이 AI를 적극적으로 활용하고, 기능 개발에 충분히 참여하는 조건에서 120~155근무일…",
    content:
      "14. 산정 결과 요약 구분 예상 기간 포함 범위 기능 구현 완료 95~120근무일 Native 기능 및 주요 연동 구현 통합 QA·출시 준비 완료 120~155근무일 실기기 QA, 테스트 배포와 안정화 포함 외부 Store 심사 5~10달력일 이상 개발 근무일과 별도인 외부 대기 기본 산정값은 미들급 Flutter 개발자 1명이 AI를 적극적으로 활용하고, 기능 개발에 충분히 참여하는 조건에서 120~155근무일, 약 6~7.75개월 입니다. WebView 화면 구현은 Front-end 일정에 포함되며 앱 일정에는 중복 계산하지 않습니다. 다만 전체 출시 시점은 Front-end의 WebView 제공, Backend API, Bridge 계약과 앱 통합 QA가 모두 완료되는 시점으로 결정됩니다. 더라운지 Flutter APP 구축 일정 산정 보고서",
  ...
  },
  {
    ...
    section: "산정 자료",
    excerpt:
      "더라운지 3.0 FO APP IA : Native와 WebView 화면 구분 및 기능 범위 APP 개발 표준 : Flutter Architecture, WebView Bridge, Native 기능, 품질과 배포 책임 IA의 화면 수만으로 일정을 계산하지 않고, 앱 Shell, 인증, WebView, 기기 기능, Android·iOS 검증과 배포 작업을 함께 반영했습니다.",
    content:
      "2. 산정 자료 더라운지 3.0 FO APP IA : Native와 WebView 화면 구분 및 기능 범위 APP 개발 표준 : Flutter Architecture, WebView Bridge, Native 기능, 품질과 배포 책임 IA의 화면 수만으로 일정을 계산하지 않고, 앱 Shell, 인증, WebView, 기기 기능, Android·iOS 검증과 배포 작업을 함께 반영했습니다.",
  ...
  },
  {
    ...
    excerpt:
      "WebView 166개 화면의 React·Next.js 구현 Backend API, Database와 인증 Server 구현 결제 승인, 검증, Webhook, 정산과 Server 취소 처리 Back Office와 운영자 화면 구축 UI/UX 기획 및 디자인 제작 자체 번역문과 운영 콘텐츠 제작 제외 범위도 앱 일정의 의존 항목입니다. WebView URL, Bridge 계약, 인증 정책,…",
    content:
      "5. 제외 및 별도 담당 범위 WebView 166개 화면의 React·Next.js 구현 Backend API, Database와 인증 Server 구현 결제 승인, 검증, Webhook, 정산과 Server 취소 처리 Back Office와 운영자 화면 구축 UI/UX 기획 및 디자인 제작 자체 번역문과 운영 콘텐츠 제작 제외 범위도 앱 일정의 의존 항목입니다. WebView URL, Bridge 계약, 인증 정책, API 오류 형식과 결제 복귀 규칙이 늦게 확정되면 Flutter 구현에 재작업이 발생합니다.",
  ...
  },
  {
    ...
    section: "AI 적극 활용 기준",
    excerpt:
      "다음 작업에서 AI를 일상적인 개발 도구로 사용하는 것을 전제로 합니다. Flutter Widget과 반복 화면 초안 생성 Model, DTO와 API Client 코드 초안 Riverpod 상태와 Form Validation 초안 Android·iOS 설정 확인과 오류 원인 분석 Unit·Widget Test Case와 Mock 초안 공통 Component 변환과 반복 Refactoring…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "Android·iOS 실제 기기에서 발생하는 플랫폼별 문제 소셜 로그인, 본인인증과 외부 앱 복귀 결제 실패, 중복 실행 방지와 주문 상태 재확인 WebView Origin, Redirect와 Bridge 보안 검증 Push, Deep Link, Camera, QR와 OS 권한 처리 Signing, 인증서, TestFlight와 Google Play 배포 기획·디자인·Backend·Front…",
    content:
      "8. AI로 크게 줄이기 어려운 작업 Android·iOS 실제 기기에서 발생하는 플랫폼별 문제 소셜 로그인, 본인인증과 외부 앱 복귀 결제 실패, 중복 실행 방지와 주문 상태 재확인 WebView Origin, Redirect와 Bridge 보안 검증 Push, Deep Link, Camera, QR와 OS 권한 처리 Signing, 인증서, TestFlight와 Google Play 배포 기획·디자인·Backend·Front-end 담당자와의 계약 협의 AI가 코드를 생성해도 실제 계정, 인증서, 외부 서비스와 실기기에서 성공·실패 흐름을 확인해야 합니다. 따라서 AI 적극 활용이 실환경 검증을 대체하지 않습니다.",
  ...
  },
  {
    ...
    excerpt:
      "Flutter 프로젝트, Architecture와 공통 Navigation 구조 개발·QA·운영 Flavor 및 환경 설정 Native 60개 화면 및 상태 로그인, 소셜 연결, 본인인증과 Session 상태 WebView Container, 허용 Origin과 Navigation 통제 WebView Bridge 요청·응답, 권한, 오류와 Version 처리 Push, Deep Link와 앱…",
    content:
      "4. Flutter 앱 개발 포함 범위 Flutter 프로젝트, Architecture와 공통 Navigation 구조 개발·QA·운영 Flavor 및 환경 설정 Native 60개 화면 및 상태 로그인, 소셜 연결, 본인인증과 Session 상태 WebView Container, 허용 Origin과 Navigation 통제 WebView Bridge 요청·응답, 권한, 오류와 Version 처리 Push, Deep Link와 앱 내부 Route 연결 Camera·QR·Barcode, 권한과 Native 저장소 연동 외부 인증·결제 화면 전환과 앱 복귀 Android·iOS 실기기 검증과 플랫폼별 수정 Unit·Widget·Integration Test와 주요 회귀 Test Signing, TestFlight와 Google Play 테스트 배포 준비",
  ...
  },
  {
    ...
    excerpt:
      "앱 IA와 웹사이트 IA에서 동일한 화면 ID가 212개 확인됩니다. 공통 영역 일부는 문서에서 화면 ID 체계가 다르게 부여되어 있어 실제 기능 수준의 중복도는 화면 ID 일치 비율보다 높습니다. 앱의 WebView 166개는 대부분 반응형 웹사이트 기능의 Mobile Web 표현과 같습니다. 하나의 공통 Front-end 코드베이스에서 반응형 웹과 WebView 모드를 함께 지원하면 다음…",
    content:
      "5. 두 구축 범위를 합산하지 않는 이유 앱 IA와 웹사이트 IA에서 동일한 화면 ID가 212개 확인됩니다. 공통 영역 일부는 문서에서 화면 ID 체계가 다르게 부여되어 있어 실제 기능 수준의 중복도는 화면 ID 일치 비율보다 높습니다. 앱의 WebView 166개는 대부분 반응형 웹사이트 기능의 Mobile Web 표현과 같습니다. 하나의 공통 Front-end 코드베이스에서 반응형 웹과 WebView 모드를 함께 지원하면 다음을 재사용할 수 있습니다. 도메인 모델과 TypeScript Type API Client와 Server 상태 처리 Form 검증과 오류 메시지 상품, 사용처, 주문과 회원 관련 UI Component Loading, Empty, Error와 권한 제한 상태 단위 테스트와 주요 사용자 흐름 테스트 따라서 166 + 225 = 391 개의 신규 화면으로 계산하지 않습니다. 약 225개의 공통 웹 화면 및 상태 를 구현하고, 그중 앱에 제공되는 166개 에 WebView 실행 환경 대응을 추가하는 방식으로 산정합니다. 반응형 웹과 앱 WebView를 별도 저장소와 별도 Component로 중복 구현하면 재사용 효과가 사라져 전체 일정이 약 20~30% 증가 할 수 있습니다.",
  ...
  },
  {
    ...
    section: "산정 자료와 대상",
    excerpt:
      "산정에는 다음 두 IA를 사용했습니다. 더라운지 3.0 FO APP IA : 앱에서 Native와 WebView로 제공할 화면 정의 더라운지 3.0 FO IA(PC WEB) : PC와 Mobile Web을 포함한 반응형 웹사이트 화면 정의 두 IA는 같은 Front Office 서비스를 서로 다른 실행 환경에서 정의합니다. 따라서 화면 수를 단순 합산하지 않고, 공통 기능의 재사용 여부와 앱…",
    content:
      "2. 산정 자료와 대상 산정에는 다음 두 IA를 사용했습니다. 더라운지 3.0 FO APP IA : 앱에서 Native와 WebView로 제공할 화면 정의 더라운지 3.0 FO IA(PC WEB) : PC와 Mobile Web을 포함한 반응형 웹사이트 화면 정의 두 IA는 같은 Front Office 서비스를 서로 다른 실행 환경에서 정의합니다. 따라서 화면 수를 단순 합산하지 않고, 공통 기능의 재사용 여부와 앱 전용 처리를 구분하여 산정했습니다.",
  ...
  },
  {
    ...
    section: "일정 산정 방식",
    excerpt:
      "화면 수에 고정 단가를 곱하지 않고 다음 요소를 함께 고려했습니다. 공통 Layout과 Component의 최초 구축 비용 동일 패턴 화면의 재사용 효과 회원, 주문과 결제처럼 상태가 복잡한 기능의 추가 비용 반응형 웹과 앱 WebView의 환경 차이 Backend API와 Flutter Bridge 통합 비용 자동화 테스트, 실제 기기 검증과 UAT 수정 비용 여러 개발자의 병렬 작업에서…",
    content:
  ...
  },
  {
    ...
    section: "리뷰 체크리스트",
    excerpt:
      "새 토큰이 실제로 반복되는 디자인 결정인가? Base Token과 Semantic Token의 역할이 구분되어 있는가? 컴포넌트에서 원시 색상과 임의 값을 불필요하게 사용하지 않았는가? Figma Variables와 globals.css 의 이름 및 값이 일치하는가? shadcn/ui의 기존 Semantic Token과 접근성 동작을 유지하는가? 사용하지 않는 Component Token과…",
    content:
      "9. 리뷰 체크리스트 새 토큰이 실제로 반복되는 디자인 결정인가? Base Token과 Semantic Token의 역할이 구분되어 있는가? 컴포넌트에서 원시 색상과 임의 값을 불필요하게 사용하지 않았는가? Figma Variables와 globals.css 의 이름 및 값이 일치하는가? shadcn/ui의 기존 Semantic Token과 접근성 동작을 유지하는가? 사용하지 않는 Component Token과 자동화 구조를 미리 추가하지 않았는가? 미확정된 브랜드, 상태와 다크 모드 값을 확정된 기준처럼 사용하지 않았는가? 디자인 토큰 가이드",
  ...
  },
  {
    ...
    section: "목적과 범위",
    excerpt:
      "이 문서는 디자인 값의 이름, 사용 범위와 코드 연결 방식을 정의합니다. Figma 화면을 React 코드로 변환하는 절차는 React Code Exports 가이드 를 따릅니다. 디자인 토큰은 반복되는 모든 숫자가 아니라 합의된 디자인 결정입니다. 화면 코드에서는 원시 값보다 의미 기반 토큰을 우선합니다. 실제 사용처가 없는 토큰과 컴포넌트별 토큰을 미리 만들지 않습니다. Tokens St…",
    content:
      "1. 목적과 범위 이 문서는 디자인 값의 이름, 사용 범위와 코드 연결 방식을 정의합니다. Figma 화면을 React 코드로 변환하는 절차는 React Code Exports 가이드 를 따릅니다. 디자인 토큰은 반복되는 모든 숫자가 아니라 합의된 디자인 결정입니다. 화면 코드에서는 원시 값보다 의미 기반 토큰을 우선합니다. 실제 사용처가 없는 토큰과 컴포넌트별 토큰을 미리 만들지 않습니다. Tokens Studio 무료 기능 안에서 유지할 수 있는 구조로 시작합니다.",
  ...
  },
  {
    ...
    section: "변경과 동기화",
    excerpt:
      "변경할 토큰의 사용 목적과 영향을 확인합니다. Tokens Studio의 Base Token과 Semantic Token 참조를 수정합니다. Figma Variables 또는 Styles를 다시 내보내고 주요 컴포넌트를 확인합니다. globals.css 의 실제 값과 @theme inline 연결을 갱신합니다. 기존 shadcn/ui와 기능 화면에서 대비, 상태와 반응형 결과를 확인합니다.…",
    content:
      "8. 변경과 동기화 변경할 토큰의 사용 목적과 영향을 확인합니다. Tokens Studio의 Base Token과 Semantic Token 참조를 수정합니다. Figma Variables 또는 Styles를 다시 내보내고 주요 컴포넌트를 확인합니다. globals.css 의 실제 값과 @theme inline 연결을 갱신합니다. 기존 shadcn/ui와 기능 화면에서 대비, 상태와 반응형 결과를 확인합니다. 관련 AI 요약 문서와 가이드 내용이 달라졌는지 확인합니다. 토큰 이름 변경은 단순한 값 변경보다 영향이 큽니다. 기존 Tailwind 클래스와 Figma Variable 사용처를 먼저 찾고 한 변경에서 함께 수정합니다.",
  ...
  },
  {
    ...
    section: "토큰 분류와 이름",
    excerpt:
      "4.1 Base Token Base Token은 실제 값의 재료입니다. Tokens Studio에서는 base Token Set으로 관리하고, 의미를 가진 화면 코드에서 직접 사용하는 것은 지양합니다. 분류 이름 적용 기준 Color color.neutral.* , color.brand.* 실제 팔레트가 확정된 범위만 작성 Spacing spacing.1 , spacing.2 , spacin…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "색상, 간격과 크기는 반복되는 화면별 값보다 Variable을 우선합니다. Auto Layout의 간격은 승인된 spacing 값에 맞춥니다. Semantic Token은 실제 사용 목적이 드러나는 이름으로 연결합니다. 공통 컴포넌트 인스턴스는 Variables와 함께 유지합니다. Tokens Studio에서 지원하지 않는 타입은 억지로 Variable로 변환하지 않습니다. 무료 Token…",
    content:
      "6. Figma Variables 적용 색상, 간격과 크기는 반복되는 화면별 값보다 Variable을 우선합니다. Auto Layout의 간격은 승인된 spacing 값에 맞춥니다. Semantic Token은 실제 사용 목적이 드러나는 이름으로 연결합니다. 공통 컴포넌트 인스턴스는 Variables와 함께 유지합니다. Tokens Studio에서 지원하지 않는 타입은 억지로 Variable로 변환하지 않습니다. 무료 Token Set 내보내기에서는 Mode를 만들 수 없으므로, 현재 코드의 .dark 값은 Figma와 자동 동기화되는 것으로 간주하지 않습니다. TBD : 다크 모드가 제품 요구사항으로 확정되면 Figma Mode 관리와 코드 동기화 방식을 별도로 결정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "무료 라이선스에서는 Token Set을 Figma의 Variables와 Styles로 내보낼 수 있습니다. Token Set마다 Variable Collection이 생성되며, Collection 안의 Mode 생성과 다른 Figma 파일의 Variable 참조는 지원되지 않습니다. 자세한 범위는 Tokens Studio Token Set 내보내기 를 확인합니다. base 와 semantic…",
    content:
      "5. Tokens Studio 무료 운영 무료 라이선스에서는 Token Set을 Figma의 Variables와 Styles로 내보낼 수 있습니다. Token Set마다 Variable Collection이 생성되며, Collection 안의 Mode 생성과 다른 Figma 파일의 Variable 참조는 지원되지 않습니다. 자세한 범위는 Tokens Studio Token Set 내보내기 를 확인합니다. base 와 semantic 두 Token Set으로 시작합니다. semantic 은 base 값을 참조합니다. 무료 범위에서 Theme와 여러 Mode를 자동 생성하는 흐름을 전제로 하지 않습니다. Token 이름과 타입을 바꿀 때 Figma 사용처와 코드 영향을 함께 확인합니다. 여러 Token Set과 Mode가 실제로 필요해진 뒤 유료 기능이나 자동화를 검토합니다. 관리 원칙 : 디자인 토큰은 Figma에서만 생성·수정·삭제합니다. 저장소의 토큰 파일을 직접 수정하는 것은 금지합니다.",
  ...
  },
  {
    ...
    section: "업데이트와 배포",
    excerpt:
      "변경 내용 일반적인 배포 범위 확인 사항 웹 화면의 문구·디자인·업무 흐름 웹 서비스 배포 기존 App과의 호환성 새로운 App 기능 호출 웹과 App 동시 검토 구버전 App의 처리 방법 Bridge 통신 규격 변경 웹과 App 배포 버전 호환성과 배포 순서 운영체제 권한·Native 기능 변경 App 심사와 배포 Android·iOS 정책과 사용자 안내",
    content:
      "8. 업데이트와 배포 변경 내용 일반적인 배포 범위 확인 사항 웹 화면의 문구·디자인·업무 흐름 웹 서비스 배포 기존 App과의 호환성 새로운 App 기능 호출 웹과 App 동시 검토 구버전 App의 처리 방법 Bridge 통신 규격 변경 웹과 App 배포 버전 호환성과 배포 순서 운영체제 권한·Native 기능 변경 App 심사와 배포 Android·iOS 정책과 사용자 안내",
  ...
  },
  {
    ...
    excerpt:
      "WebView가 무엇인지, 일반 브라우저와 어떤 차이가 있는지, 모바일 앱에서 어떤 역할을 담당하는지 설명합니다. 구현과 설정 방법은 Flutter WebView 설정 가이드 에서 확인합니다. 이 문서의 대상 주요 확인 내용 기획자·운영 담당자 WebView의 역할, 제약, 업데이트와 장애 대응 범위 프로젝트 관리자 App·Web·Backend의 책임과 사전 결정 항목 신규 참여자 서비스가…",
    content:
      "1. 이 문서에서 알 수 있는 것 WebView가 무엇인지, 일반 브라우저와 어떤 차이가 있는지, 모바일 앱에서 어떤 역할을 담당하는지 설명합니다. 구현과 설정 방법은 Flutter WebView 설정 가이드 에서 확인합니다. 이 문서의 대상 주요 확인 내용 기획자·운영 담당자 WebView의 역할, 제약, 업데이트와 장애 대응 범위 프로젝트 관리자 App·Web·Backend의 책임과 사전 결정 항목 신규 참여자 서비스가 모바일 앱 안에서 동작하는 전체 구조",
  ...
  },
  {
    ...
    excerpt:
      "구분 일반 브라우저 앱 내부 WebView 실행 위치 Chrome·Safari 같은 별도 앱 모바일 앱 화면 내부 주소 이동 사용자가 자유롭게 주소 입력 앱이 허용한 주소와 이동 정책 적용 기기 기능 브라우저가 제공하는 범위에서 사용 App과 연결해 카메라·위치 등 사용 가능 업데이트 브라우저와 웹 서비스가 각각 업데이트 웹 화면과 App 기능의 변경 범위에 따라 달라짐 문제 확인 웹 서비스…",
    content:
      "3. 일반 브라우저와의 차이 구분 일반 브라우저 앱 내부 WebView 실행 위치 Chrome·Safari 같은 별도 앱 모바일 앱 화면 내부 주소 이동 사용자가 자유롭게 주소 입력 앱이 허용한 주소와 이동 정책 적용 기기 기능 브라우저가 제공하는 범위에서 사용 App과 연결해 카메라·위치 등 사용 가능 업데이트 브라우저와 웹 서비스가 각각 업데이트 웹 화면과 App 기능의 변경 범위에 따라 달라짐 문제 확인 웹 서비스와 브라우저 중심으로 확인 App·Web·Backend·운영체제를 함께 확인",
  ...
  },
  {
    ...
    excerpt:
      "현상 우선 확인 영역 주요 확인 내용 웹 화면이 열리지 않음 Web·네트워크·App 서비스 URL, 서버 상태, 인터넷 권한과 인증서 로그인이 유지되지 않음 인증·Web·App 쿠키, 세션 만료와 로그아웃 처리 카메라·위치가 동작하지 않음 App·운영체제 사용자 권한, App 설정과 Bridge 요청 일부 기기에서만 화면이 다름 Web·운영체제 Android System WebView 또는…",
    content:
      "9. 장애가 발생했을 때 확인할 영역 현상 우선 확인 영역 주요 확인 내용 웹 화면이 열리지 않음 Web·네트워크·App 서비스 URL, 서버 상태, 인터넷 권한과 인증서 로그인이 유지되지 않음 인증·Web·App 쿠키, 세션 만료와 로그아웃 처리 카메라·위치가 동작하지 않음 App·운영체제 사용자 권한, App 설정과 Bridge 요청 일부 기기에서만 화면이 다름 Web·운영체제 Android System WebView 또는 iOS 버전 차이 결제·예약 결과가 다름 Backend·Web 서버의 최종 처리 결과와 화면 갱신",
  ...
  },
  {
    ...
    section: "지원 환경",
    excerpt:
      "신규 모바일 프로젝트는 다음 환경을 기준으로 검토합니다. 플랫폼 최소 환경 확인 기준 Android Android SDK 24 이상 지원 대상 System WebView에서 프로젝트 사용 CSS와 핵심 기능 iOS iOS 15 이상 iOS 15 이상에서 제공되는 WKWebView Tailwind CSS v4의 공식 최소 브라우저 버전을 WebView의 일괄 차단 기준으로 사용하지 않습니다.…",
    content:
      "6. 지원 환경 신규 모바일 프로젝트는 다음 환경을 기준으로 검토합니다. 플랫폼 최소 환경 확인 기준 Android Android SDK 24 이상 지원 대상 System WebView에서 프로젝트 사용 CSS와 핵심 기능 iOS iOS 15 이상 iOS 15 이상에서 제공되는 WKWebView Tailwind CSS v4의 공식 최소 브라우저 버전을 WebView의 일괄 차단 기준으로 사용하지 않습니다. Safari 15와 지원 대상 구형 WebView에서 프로젝트가 실제 사용하는 CSS와 주요 기능이 정상 동작하는지 확인합니다. 공통 CSS 하한은 Safari 15에서 지원되는 기능 범위로 정합니다. Android는 Safari 15에 특정 Chromium 버전을 대응시키지 않고 지원 대상 System WebView에서 실제 사용하는 Tailwind Utility와 핵심 기능을 검증합니다. 최소 기준을 정한 근거 주요 서비스 국가는 한국·중국·일본·영문이며 그 밖의 국가는 글로벌 기준으로 검토합니다. 2026년 7월 StatCounter 모바일 트래픽에서 Android와 iOS의 비중은 한국 65.39%·34.60%, 중국 70.37%·29.51%, 일본 36.41%·63.59%, 영문 40.39%·59.58%, 전 세계 68.36%·31.60%입니다. 따라서 Android 중심 국가와 iOS 중심 국가를 모두 지원해야 하며, 한 국가의 통계만으로 전체 지원 범위를 판단하지 않습니다. 한국 모바일 OS 점유율 중국 모바일 OS 점유율 일본 모바일 OS 점유율 영문 모바일 OS 점유율 전 세계 모바일 OS 점유율 Google이 2025년 12월 공개한 Android 배포 데이터에서 API 21~23의 합계는 0.8%이므로 API 24 이상은 약 99.2%입니다. 이 수치는 Google의 Android 활성 기기 분포이며 중국 시장 또는 이 서비스의 실제 고객 분포와 같다고 보지 않습니다. Google Android 버전 배포 원본 데이터 에서 조사일과 API별 비율을 확인합니다. Apple이 2026년 6월 7일 App Store에서 거래한 기기를 집계한 결과 전체 iPhone의 79%가 iOS 26을 사용합니다. 이 값만으로 iOS 15 이상 전체 비율이나 실제 서비스의 지원 도달률을 계산하지 않습니다. 자세한 내용은 Apple iOS 사용 현황 에서 확인합니다. 공개 점유율은 기준 선정의 참고 자료입니다. 출시 후에는 국가·OS·Android WebView 엔진별 실제 활성 사용자 분포와 오류율을 우선 근거로 사용합니다.",
  ...
  },
  {
    ...
    excerpt:
      "항목 결정할 내용 주요 참여자 지원 환경 최소 Android·iOS 버전과 검증 기기 기획·App·QA 접속 범위 시작 주소, 허용 사이트와 외부 앱 이동 기획·Web·App·보안 로그인 로그인 방식, 유지 시간과 로그아웃 처리 기획·Backend·Web·App 기기 기능 카메라, 위치, 파일과 알림 사용 범위 기획·Web·App 오류 처리 로딩, 실패, 재시도와 사용자 안내 방식 기획·디자…",
    content:
      "10. 프로젝트 시작 전에 결정할 항목 항목 결정할 내용 주요 참여자 지원 환경 최소 Android·iOS 버전과 검증 기기 기획·App·QA 접속 범위 시작 주소, 허용 사이트와 외부 앱 이동 기획·Web·App·보안 로그인 로그인 방식, 유지 시간과 로그아웃 처리 기획·Backend·Web·App 기기 기능 카메라, 위치, 파일과 알림 사용 범위 기획·Web·App 오류 처리 로딩, 실패, 재시도와 사용자 안내 방식 기획·디자인·Web·App 배포 웹과 App의 배포 순서 및 구버전 호환 정책 PM·Web·App·운영",
  ...
  },
  {
    ...
    excerpt:
      "구분 내용 기본 제공 가능 웹 페이지 표시, 화면 이동, 로그인 화면과 서비스 정보 표시 App 연동 필요 카메라, QR, 위치, Push, 생체인증과 기기 파일 사용 정책 결정 필요 로그인 유지, 외부 앱 이동, 다운로드와 새 창 처리 보안 검토 필요 결제, 개인정보, 인증정보, 알 수 없는 외부 사이트 표시 App 연동이 필요한 기능은 웹 화면만 수정해서 추가할 수 없습니다. App과 W…",
    content:
      "7. 할 수 있는 것과 별도 검토가 필요한 것 구분 내용 기본 제공 가능 웹 페이지 표시, 화면 이동, 로그인 화면과 서비스 정보 표시 App 연동 필요 카메라, QR, 위치, Push, 생체인증과 기기 파일 사용 정책 결정 필요 로그인 유지, 외부 앱 이동, 다운로드와 새 창 처리 보안 검토 필요 결제, 개인정보, 인증정보, 알 수 없는 외부 사이트 표시 App 연동이 필요한 기능은 웹 화면만 수정해서 추가할 수 없습니다. App과 Web 사이의 통신 규격을 정하고 Android·iOS 앱을 함께 개발·배포해야 할 수 있습니다.",
  ...
  },
  {
    ...
    excerpt:
      "WebView는 모바일 앱 화면 안에서 웹 페이지를 보여주는 기능입니다. 사용자는 별도의 브라우저 앱으로 이동하지 않고 웹으로 만든 서비스 화면을 이용할 수 있습니다. WebView는 Chrome이나 Safari 앱을 그대로 넣은 것이 아닙니다. Android와 iOS가 앱에 제공하는 전용 웹 표시 기능을 사용합니다. 환경 사용하는 기능 쉬운 설명 Android Android System W…",
    content:
      "2. WebView란 무엇인가 WebView는 모바일 앱 화면 안에서 웹 페이지를 보여주는 기능입니다. 사용자는 별도의 브라우저 앱으로 이동하지 않고 웹으로 만든 서비스 화면을 이용할 수 있습니다. WebView는 Chrome이나 Safari 앱을 그대로 넣은 것이 아닙니다. Android와 iOS가 앱에 제공하는 전용 웹 표시 기능을 사용합니다. 환경 사용하는 기능 쉬운 설명 Android Android System WebView Android가 앱 내부 웹 화면을 위해 제공하는 기능 iOS WKWebView iOS가 앱 내부 웹 화면을 위해 제공하는 기능",
  ...
  },
  {
    ...
    excerpt:
      "Android와 iOS에서 동일한 서비스 화면과 업무 흐름을 제공할 수 있습니다. 웹 화면만 변경할 때 App 전체를 다시 배포하지 않아도 될 수 있습니다. 기존 웹 기술과 개발 인력을 활용할 수 있습니다. 필요한 경우 App의 카메라·위치·파일 기능과 연결할 수 있습니다. 모든 화면을 WebView로 만드는 것이 항상 유리한 것은 아닙니다. 앱 시작, Push, 생체인증, 보안 저장소와 운…",
    content:
      "4. WebView를 사용하는 이유 Android와 iOS에서 동일한 서비스 화면과 업무 흐름을 제공할 수 있습니다. 웹 화면만 변경할 때 App 전체를 다시 배포하지 않아도 될 수 있습니다. 기존 웹 기술과 개발 인력을 활용할 수 있습니다. 필요한 경우 App의 카메라·위치·파일 기능과 연결할 수 있습니다. 모든 화면을 WebView로 만드는 것이 항상 유리한 것은 아닙니다. 앱 시작, Push, 생체인증, 보안 저장소와 운영체제 권한처럼 App이 직접 처리해야 하는 기능은 Flutter Native 영역에 둡니다.",
  ...
  },
  {
    ...
    section: "근거 갱신",
    excerpt:
      "출시 전에는 StatCounter의 최근 3개월 추세와 실제 기기 검증 결과를 함께 확인합니다. 출시 후에는 당사 서비스의 국가·기기·브라우저·버전별 활성 사용자 비율을 우선합니다. 자동 업데이트를 전제로 사용자 버전을 추정하지 않고 실제 운영 데이터에서 최소 버전 미만 사용자를 확인합니다. 최소 버전 미만 사용자 비율, 오류율과 고객 문의를 확인해 지원 기준을 조정합니다. 점유율 자료에는…",
    content:
      "8. 근거 갱신 출시 전에는 StatCounter의 최근 3개월 추세와 실제 기기 검증 결과를 함께 확인합니다. 출시 후에는 당사 서비스의 국가·기기·브라우저·버전별 활성 사용자 비율을 우선합니다. 자동 업데이트를 전제로 사용자 버전을 추정하지 않고 실제 운영 데이터에서 최소 버전 미만 사용자를 확인합니다. 최소 버전 미만 사용자 비율, 오류율과 고객 문의를 확인해 지원 기준을 조정합니다. 점유율 자료에는 출처와 기준 월을 기록하고 분기마다 다시 확인합니다. 최소 버전 변경 시 Tailwind CSS, 사용 Web API와 주요 국가 사용자 비율을 함께 검토합니다.",
  ...
  },
  {
    ...
    excerpt:
      "구분 최소 버전 검증 최신 버전 검증 국가별 추가 검증 PC Safari 15, Chrome·Edge·Firefox 지원 대상 구형 버전 각 브라우저 최신 안정 버전 한국 Whale, 중국 360 Safe·UC·QQ 구형·최신 버전 태블릿 iPadOS Safari 15, Android 주요 브라우저 지원 대상 구형 버전 iPadOS·Android 주요 브라우저 최신 안정 버전 주요 서비스 국…",
    content:
      "6. 기기 구분별 필수 검증 환경 구분 최소 버전 검증 최신 버전 검증 국가별 추가 검증 PC Safari 15, Chrome·Edge·Firefox 지원 대상 구형 버전 각 브라우저 최신 안정 버전 한국 Whale, 중국 360 Safe·UC·QQ 구형·최신 버전 태블릿 iPadOS Safari 15, Android 주요 브라우저 지원 대상 구형 버전 iPadOS·Android 주요 브라우저 최신 안정 버전 주요 서비스 국가에서 비중이 높은 실제 태블릿 1종 이상 모바일 iOS Safari 15, Android 주요 브라우저 지원 대상 구형 버전 iOS·Android 주요 브라우저 최신 안정 버전 한국·중국·일본·영문의 주요 실제 단말 반응형 검증은 최소 버전 검증과 별개로 수행합니다. 프로젝트가 확정한 breakpoint의 경계값 전후, 세로·가로 화면, 확대와 글자 크기 변화에서 레이아웃을 확인합니다. 기기 이름만으로 Desktop, Tablet, Mobile 레이아웃을 고정하지 않습니다.",
  ...
  },
  {
    ...
    excerpt:
      "이 문서는 회사 웹사이트의 PC, 태블릿, 모바일 브라우저 최소 지원 기준과 그 근거를 정리합니다. 브라우저 지원은 화면 크기가 아니라 렌더링 엔진과 브라우저 버전을 기준으로 판단합니다. PC, 태블릿, 모바일 구분은 필수 검증 환경을 정하는 데 사용합니다. 결제, 제휴카드, 인증과 외부 앱 연결의 기능별 정책은 이 문서의 범위에 포함하지 않습니다.",
    content:
      "1. 문서 목적과 적용 범위 이 문서는 회사 웹사이트의 PC, 태블릿, 모바일 브라우저 최소 지원 기준과 그 근거를 정리합니다. 브라우저 지원은 화면 크기가 아니라 렌더링 엔진과 브라우저 버전을 기준으로 판단합니다. PC, 태블릿, 모바일 구분은 필수 검증 환경을 정하는 데 사용합니다. 결제, 제휴카드, 인증과 외부 앱 연결의 기능별 정책은 이 문서의 범위에 포함하지 않습니다.",
  ...
  },
  {
    ...
    section: "우선 확정 항목",
    excerpt:
      "다음 운영 기준은 프로젝트 담당자가 합의한 뒤 확정합니다. 브라우저 최소 버전 미만 사용자를 허용할 비율 국가별 실제 검증 기기와 브라우저 목록 Whale, 360 Safe, UC와 QQ의 정식 지원 또는 호환 확인 범위 브라우저 업데이트 안내 방식 운영 데이터 기준의 재검토 주기와 승인 담당자 반응형 웹 브라우저 지원 가이드",
    content:
      "9. 우선 확정 항목 다음 운영 기준은 프로젝트 담당자가 합의한 뒤 확정합니다. 브라우저 최소 버전 미만 사용자를 허용할 비율 국가별 실제 검증 기기와 브라우저 목록 Whale, 360 Safe, UC와 QQ의 정식 지원 또는 호환 확인 범위 브라우저 업데이트 안내 방식 운영 데이터 기준의 재검토 주기와 승인 담당자 반응형 웹 브라우저 지원 가이드",
  ...
  },
  {
    ...
    excerpt:
      "아래 수치는 StatCounter의 2026년 7월 PC·모바일·태블릿 통합 웹 사용량 기준입니다. 설치 대수나 당사 고객 수가 아니며, 국가별 우선 검증 제품군을 정하는 보조 근거로 사용합니다. 지역 주요 브라우저 점유율 표시 제품군 합계 근거 한국 Chrome 57.99%, Safari 11.76%, Samsung Internet 10.40%, Edge 8.99%, Whale 8.37%,…",
    content:
      "5. 점유율을 근거로 한 브라우저 선정 아래 수치는 StatCounter의 2026년 7월 PC·모바일·태블릿 통합 웹 사용량 기준입니다. 설치 대수나 당사 고객 수가 아니며, 국가별 우선 검증 제품군을 정하는 보조 근거로 사용합니다. 지역 주요 브라우저 점유율 표시 제품군 합계 근거 한국 Chrome 57.99%, Safari 11.76%, Samsung Internet 10.40%, Edge 8.99%, Whale 8.37%, Firefox 0.83% 98.34% StatCounter 한국 중국 Chrome 51.59%, Edge 16.35%, Safari 14.44%, 360 Safe 4.88%, UC 4.85%, QQ 3.69% 95.80% StatCounter 중국 일본 Chrome 55.96%, Safari 23.21%, Edge 14.42%, Firefox 2.84%, Brave 1.12%, Samsung Internet 0.70% 98.25% StatCounter 일본 영문 Chrome 51.55%, Safari 29.83%, Edge 7.14%, Firefox 6.78%, Samsung Internet 1.69%, Brave 1.10% 98.09% StatCounter 영문 전 세계 Chrome 68.25%, Safari 16.48%, Edge 5.34%, Firefox 3.34%, Samsung Internet 2.06%, Opera 1.88% 97.35% StatCounter 전 세계 이 점유율은 브라우저 제품군의 비중이며 프로젝트 최소 버전 이상 사용자의 정확한 비율이 아닙니다. 공개된 일부 상위 버전만 합산해 최소 버전 지원률로 표현하지 않습니다. 한국은 Safari, Samsung Internet과 Whale을 함께 확인합니다. 일본과 영문은 Safari 비중이 높으므로 Apple 기기의 Safari 검증을 Chromium 검증으로 대체하지 않습니다. 중국은 360 Safe, UC와 QQ의 최신 안정 버전을 별도로 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "정식 지원: 최소 버전 이상이며 출시 전 필수 검증 환경에 포함하고 문제 발생 시 수정합니다. 호환 확인: Whale, 360 Safe, UC, QQ 등의 최신 안정 버전에서 주요 흐름을 확인하되 모든 제품 버전을 보장하지 않습니다. 미지원: 최소 버전보다 낮은 브라우저와 Internet Explorer는 정상 동작을 보장하지 않고 업데이트를 안내합니다.",
    content:
      "7. 지원과 호환 확인의 구분 정식 지원: 최소 버전 이상이며 출시 전 필수 검증 환경에 포함하고 문제 발생 시 수정합니다. 호환 확인: Whale, 360 Safe, UC, QQ 등의 최신 안정 버전에서 주요 흐름을 확인하되 모든 제품 버전을 보장하지 않습니다. 미지원: 최소 버전보다 낮은 브라우저와 Internet Explorer는 정상 동작을 보장하지 않고 업데이트를 안내합니다.",
  ...
  },
  {
    ...
    section: "문서 목적과 적용 범위",
    excerpt:
      "이 문서는 한 명의 Front-end 개발자가 여러 Backend·App 개발자와 협업하는 환경에서 AI에게 구현을 적극적으로 맡기면서도 설계, 검토와 문제 해결 역량을 점진적으로 키우기 위한 기준을 제공합니다. 모든 코드를 직접 작성하는 것을 성장 기준으로 삼지 않습니다. 개발자는 제품 요구, UI 흐름, 상태와 계약을 판단하고 AI가 만든 결과를 검증할 수 있어야 합니다. 구체적인 기술과…",
    content:
      "1. 문서 목적과 적용 범위 이 문서는 한 명의 Front-end 개발자가 여러 Backend·App 개발자와 협업하는 환경에서 AI에게 구현을 적극적으로 맡기면서도 설계, 검토와 문제 해결 역량을 점진적으로 키우기 위한 기준을 제공합니다. 모든 코드를 직접 작성하는 것을 성장 기준으로 삼지 않습니다. 개발자는 제품 요구, UI 흐름, 상태와 계약을 판단하고 AI가 만든 결과를 검증할 수 있어야 합니다. 구체적인 기술과 구현 기준은 Front-End 개발 가이드 를 따릅니다. TypeScript 작성 기준과 테스트 범위는 각각 TypeScript 가이드 와 테스트 가이드 를 함께 확인합니다. Front-end의 외부 입력 처리, 개인정보 노출 방지, Client 저장소, Bridge Adapter와 의존성 보안 및 인증·권한·CSP 연동 시 책임 구분은 Front-End 보안과 개인정보 가이드 를 따릅니다. Backend·Native App·배포·보안 정책을 Front-end가 대신 구현한다는 의미가 아닙니다. API 요청 계층과 세션·회원 경계는 기획 흐름과 Backend API 계약이 확정된 뒤 각각 API 요청 계층 , 세션과 회원 경계 가이드를 기준으로 학습합니다. 계약 확정 전에는 Backend 확인 질문과 TBD 만 정리하며, API Mock 은 승인된 계약 이후 실제 환경에서 필요한 상태를 반복 재현하기 어려울 때 Front-end 테스트 범위와 관리 책임을 정해 선택적으로 검토합니다. AI는 구현 속도를 높이는 협업 도구입니다. AI를 많이 사용하는 것 자체는 문제가 아니지만, 확인되지 않은 제품 계약과 기술 결정을 AI의 추측으로 확정하지 않습니다.",
  ...
  },
  {
    ...
    section: "성장의 기준",
    excerpt:
      "숙련도는 직접 입력한 코드의 양이 아니라 다음 판단을 스스로 내리고 설명할 수 있는지로 확인합니다. 사용자 요구를 화면과 동작으로 구체화할 수 있는가? 데이터가 React 지역 상태, URL, 폼 상태, 서버 상태 또는 공유 클라이언트 상태 중 어디에 있어야 하는지 판단할 수 있는가? 컴포넌트와 파일을 나눈 이유를 책임과 변경 범위로 설명할 수 있는가? API와 WebView Bridge의…",
    content:
      "2. 성장의 기준 숙련도는 직접 입력한 코드의 양이 아니라 다음 판단을 스스로 내리고 설명할 수 있는지로 확인합니다. 사용자 요구를 화면과 동작으로 구체화할 수 있는가? 데이터가 React 지역 상태, URL, 폼 상태, 서버 상태 또는 공유 클라이언트 상태 중 어디에 있어야 하는지 판단할 수 있는가? 컴포넌트와 파일을 나눈 이유를 책임과 변경 범위로 설명할 수 있는가? API와 WebView Bridge의 입력, 출력, 오류와 미확정 항목을 구분할 수 있는가? TypeScript 오류를 숨기지 않고 발생 원인과 안전한 처리 방법을 확인할 수 있는가? AI가 만든 코드의 과도한 상태, 추상화, 의존성과 보안 문제를 발견할 수 있는가? 변경 위험에 맞는 Typecheck, Lint, Test와 실제 화면 검증 범위를 선택할 수 있는가? 처음에는 AI의 설명과 제안에 의존할 수 있습니다. 프로젝트가 진행될수록 개발자가 요구와 제약을 먼저 정의하고 AI의 구현을 검토하는 비중을 높입니다.",
  ...
  },
  {
    ...
    section: "순차 교육 과정",
    excerpt:
      "JavaScript와 TypeScript 기초부터 React, Next.js, API, WebView, UI·브라우저·다국어 검증, 테스트, 기능 통합과 보안 리뷰까지 순서대로 학습할 수 있는 교육 문서를 제공합니다. AI 협업 Front-End 교육 과정 시작하기 AI 협업 기반 Front-End 성장 가이드",
    content:
      "11. 순차 교육 과정 JavaScript와 TypeScript 기초부터 React, Next.js, API, WebView, UI·브라우저·다국어 검증, 테스트, 기능 통합과 보안 리뷰까지 순서대로 학습할 수 있는 교육 문서를 제공합니다. AI 협업 Front-End 교육 과정 시작하기 AI 협업 기반 Front-End 성장 가이드",
  ...
  },
  {
    ...
    excerpt:
      "기능 전체를 한 번에 생성하고 실제 동작과 변경 파일을 확인하지 않는 방식 AI가 제안했다는 이유만으로 새 상태 관리 도구, package 또는 공통 계층을 추가하는 방식 TypeScript 오류를 any , as , ! 또는 검사 비활성화로만 제거하는 방식 API 문서, 실제 응답과 담당자 합의가 다른데 AI의 추측으로 계약을 확정하는 방식 UI가 Figma와 비슷하다는 이유로 접근성, 오…",
    content:
      "10. 피해야 할 AI 협업 방식 기능 전체를 한 번에 생성하고 실제 동작과 변경 파일을 확인하지 않는 방식 AI가 제안했다는 이유만으로 새 상태 관리 도구, package 또는 공통 계층을 추가하는 방식 TypeScript 오류를 any , as , ! 또는 검사 비활성화로만 제거하는 방식 API 문서, 실제 응답과 담당자 합의가 다른데 AI의 추측으로 계약을 확정하는 방식 UI가 Figma와 비슷하다는 이유로 접근성, 오류 상태와 반응형 동작을 생략하는 방식 생성된 테스트의 의미를 확인하지 않고 테스트 통과만 완료 기준으로 삼는 방식 AI 사용량을 줄이거나 모든 코드를 직접 입력하는 것을 성장 목표로 삼는 방식 AI 협업의 목표는 개발자가 반복 코드를 더 많이 작성하는 것이 아닙니다. 프로젝트가 진행될수록 더 정확한 요구와 제약을 제공하고, AI의 가정과 구현을 검토하며, 제품과 기술 결정에 책임질 수 있는 범위를 넓히는 것을 목표로 합니다.",
  ...
  },
  {
    ...
    excerpt:
      "7.1 기능 시작 요청 이 기능을 프로젝트 가이드와 기존 소스를 기준으로 검토해줘. 바로 코드를 작성하지 말고 먼저 다음을 정리해줘. 1. 재사용할 기존 컴포넌트와 코드 2. 사용자 행동과 필요한 화면 상태 3. 지역 상태, 서버 상태, 폼 상태와 공유 상태의 구분 4. 확인이 필요한 API 또는 App Bridge 계약 5. 작은 단위로 나눈 구현 계획 6. 내가 이해해야 할 TypeScr…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "AI는 계약 초안과 질문을 정리할 수 있지만 실제 제품 계약을 확정하지 않습니다. Backend API 기능 구현 전에 다음 내용을 담당자와 확인합니다. 요청과 응답 필드, 필수 여부와 nullable 여부 날짜, 시간, 통화와 금액 형식 인증과 권한 처리 주체 업무 오류 코드와 사용자 표시 가능 메시지 페이지네이션, 정렬과 중복 요청 처리 계약 변경과 하위 호환 방식 API 문서와 실제 응…",
    content:
      "8. Backend·App 개발자와의 계약 확인 AI는 계약 초안과 질문을 정리할 수 있지만 실제 제품 계약을 확정하지 않습니다. Backend API 기능 구현 전에 다음 내용을 담당자와 확인합니다. 요청과 응답 필드, 필수 여부와 nullable 여부 날짜, 시간, 통화와 금액 형식 인증과 권한 처리 주체 업무 오류 코드와 사용자 표시 가능 메시지 페이지네이션, 정렬과 중복 요청 처리 계약 변경과 하위 호환 방식 API 문서와 실제 응답이 다르면 AI가 임의로 한쪽에 맞추지 않고 Backend 담당자에게 차이를 확인합니다. App Bridge Native 기능을 연결할 때 다음 내용을 App 담당자와 확인합니다. Method와 지원 기능 입력, 성공 결과와 오류 형식 사용자 취소, 권한 거부와 미지원 상태 Bridge와 App의 최소 지원 Version Timeout, 중복 요청과 응답 식별 방식 민감정보와 로그에 포함할 수 없는 값 Bridge 계약이 확정되기 전에는 전역 객체명이나 메시지 필드를 제품 기준으로 고정하지 않습니다.",
  ...
  },
  {
    ...
    section: "공통 완료 기준",
    excerpt:
      "핵심 개념을 자신의 말로 설명할 수 있습니다. AI가 만든 코드에서 해당 단계의 위험을 찾을 수 있습니다. Lounge 프로젝트의 작은 기능에 적용할 수 있습니다. 확인된 기준과 미확정 계약을 구분할 수 있습니다. 실행한 검사와 직접 확인한 항목을 구분해 설명할 수 있습니다. AI의 조사 범위, 변경 파일과 추측을 통제하고 Diff를 근거로 승인할 수 있습니다. API가 확정되지 않았을 때…",
    content:
      "7. 공통 완료 기준 핵심 개념을 자신의 말로 설명할 수 있습니다. AI가 만든 코드에서 해당 단계의 위험을 찾을 수 있습니다. Lounge 프로젝트의 작은 기능에 적용할 수 있습니다. 확인된 기준과 미확정 계약을 구분할 수 있습니다. 실행한 검사와 직접 확인한 항목을 구분해 설명할 수 있습니다. AI의 조사 범위, 변경 파일과 추측을 통제하고 Diff를 근거로 승인할 수 있습니다. API가 확정되지 않았을 때 구현을 선행하지 않고 Backend 확인 질문과 TBD 를 정리할 수 있습니다. API 확정 뒤 화면을 다시 만들지 않고 계약 경계와 테스트를 함께 교체할 수 있습니다. 세션·회원·서버·폼·UI 상태의 소유자와 사용자 전환 시 정리 범위를 설명할 수 있습니다. 외부 입력, 인증·권한, Client 저장소, 개인정보와 Bridge의 보안 경계를 검토할 수 있습니다. UI 변경 시 Figma·실제 코드·Story의 차이와 Safari 15 호환 여부를 확인할 수 있습니다. 다국어 대상 문구와 사용자·서버 콘텐츠를 구분하고 번역 Fallback과 변수 보존을 검증할 수 있습니다. 1단계 JavaScript 기초 시작하기 AI 협업 Front-End 교육 과정",
  ...
  },
  {
    ...
    section: "과정 사용 방법",
    excerpt:
      "이 과정은 모든 코드를 직접 입력하는 교육이 아닙니다. AI가 조사와 구현을 수행하고, 개발자는 요구·상태·계약을 결정하며 결과를 검증하는 방식으로 진행합니다. 협업 원칙은 AI 협업 기반 Front-End 성장 가이드 를 따릅니다. 각 단계의 완료 기준을 설명하고 적용할 수 있을 때 다음 문서로 이동합니다. 읽은 문서 수나 학습 시간만으로 완료를 판단하지 않습니다. 쉬운 설명만 이해하는 것…",
    content:
      "1. 과정 사용 방법 이 과정은 모든 코드를 직접 입력하는 교육이 아닙니다. AI가 조사와 구현을 수행하고, 개발자는 요구·상태·계약을 결정하며 결과를 검증하는 방식으로 진행합니다. 협업 원칙은 AI 협업 기반 Front-End 성장 가이드 를 따릅니다. 각 단계의 완료 기준을 설명하고 적용할 수 있을 때 다음 문서로 이동합니다. 읽은 문서 수나 학습 시간만으로 완료를 판단하지 않습니다. 쉬운 설명만 이해하는 것을 목표로 하지 않습니다. 각 단계의 실행 원리, 실패 조건과 교체 경계를 이해하고 AI의 구현이 그 기준을 지켰는지 판단할 수 있어야 합니다. 모르는 문법은 AI에게 설명받아도 되지만 이해하지 못한 핵심 결정은 승인하지 않습니다.",
  ...
  },
  {
    ...
    section: "기초 언어",
    excerpt:
      "JavaScript 기초 — 값과 참조, Closure, 비동기 순서, 취소와 자원 정리 TypeScript 기본 — 추론, 판별 가능한 Union, 누락 없는 분기와 타입 보존 TypeScript 경계 — Props, 외부 입력, 형태·의미 검증과 생성 타입의 한계 이 구간을 마치면 AI가 만든 타입의 역할과 타입 오류의 원인을 설명할 수 있어야 합니다.",
    content:
      "2. 기초 언어 JavaScript 기초 — 값과 참조, Closure, 비동기 순서, 취소와 자원 정리 TypeScript 기본 — 추론, 판별 가능한 Union, 누락 없는 분기와 타입 보존 TypeScript 경계 — Props, 외부 입력, 형태·의미 검증과 생성 타입의 한계 이 구간을 마치면 AI가 만든 타입의 역할과 타입 오류의 원인을 설명할 수 있어야 합니다.",
  ...
  },
  {
    ...
    section: "실제 서비스 연결",
    excerpt:
      "API 계약과 오류 — 공통 요청 계층, Runtime Parser, 지연·Offline·응답·오류·취소 분류 서버·클라이언트 상태 — Query 수명, Mutation 일관성, 세션·회원과 사용자 전환 WebView Bridge와 보안 — Native 경계, 응답 검증, 요청 수명과 최소 노출 이 구간을 마치면 Backend·App 담당자에게 필요한 계약 질문을 하고, API 미확정 상태에…",
    content:
      "4. 실제 서비스 연결 API 계약과 오류 — 공통 요청 계층, Runtime Parser, 지연·Offline·응답·오류·취소 분류 서버·클라이언트 상태 — Query 수명, Mutation 일관성, 세션·회원과 사용자 전환 WebView Bridge와 보안 — Native 경계, 응답 검증, 요청 수명과 최소 노출 이 구간을 마치면 Backend·App 담당자에게 필요한 계약 질문을 하고, API 미확정 상태에서는 구현을 선행하지 않은 채 TBD 를 관리하며, 계약 확정 후 세션과 사용자별 상태의 수명을 설계할 수 있어야 합니다.",
  ...
  },
  {
    ...
    section: "품질과 독립적인 판단",
    excerpt:
      "테스트와 디버깅 — 테스트 경계, Storybook 검수, Backend 환경 우선과 증거 기반 원인 추적 기능 통합과 리뷰 — 승인된 계약에서 구현과 검증까지 하나의 기능 완성 Front-End 보안과 개인정보 — 완성한 기능의 신뢰 경계, 개인정보와 보안 위험 검토 마지막 구간에서는 AI의 구현을 그대로 승인하지 않고 기능·회귀·보안 위험과 변경 영향을 검토해 병합 여부를 판단합니다.",
    content:
      "5. 품질과 독립적인 판단 테스트와 디버깅 — 테스트 경계, Storybook 검수, Backend 환경 우선과 증거 기반 원인 추적 기능 통합과 리뷰 — 승인된 계약에서 구현과 검증까지 하나의 기능 완성 Front-End 보안과 개인정보 — 완성한 기능의 신뢰 경계, 개인정보와 보안 위험 검토 마지막 구간에서는 AI의 구현을 그대로 승인하지 않고 기능·회귀·보안 위험과 변경 영향을 검토해 병합 여부를 판단합니다.",
  ...
  },
  {
    ...
    excerpt:
      "React 컴포넌트와 상태 — 책임, 상태 소유자, 렌더링 위치·Key와 상태 보존 React Effect와 폼 — Effect 수명·경쟁 상태, 입력과 Mutation 흐름 Next.js App Router — Route, Server·Client 경계, Hydration과 오류 책임 Tailwind UI와 접근성 — 토큰, Figma·Story 연결, Safari 15 호환, 상태 전달과…",
    content:
      "3. React와 화면 구현 React 컴포넌트와 상태 — 책임, 상태 소유자, 렌더링 위치·Key와 상태 보존 React Effect와 폼 — Effect 수명·경쟁 상태, 입력과 Mutation 흐름 Next.js App Router — Route, Server·Client 경계, Hydration과 오류 책임 Tailwind UI와 접근성 — 토큰, Figma·Story 연결, Safari 15 호환, 상태 전달과 Focus 흐름 이 구간을 마치면 화면을 시각적 레이어가 아니라 사용자 동작과 상태의 조합으로 설계할 수 있어야 합니다.",
  ...
  },
  {
    ...
    excerpt:
      "아래 기능은 BO 범위 후보입니다. 실제 제공 기능과 우선순위, 운영 권한은 서비스·운영·개발 담당자가 합의한 뒤 확정합니다. TBD 기능 설명 공지 팝업 BO에서 등록·수정·삭제하고 앱 실행 시 또는 특정 화면에서 노출 이벤트 팝업 기간 설정, 이미지 및 링크 관리 인트로 이미지 및 노출 기간 관리 점검 모드 BO에서 점검 여부 설정 시 앱에서 점검 화면 표시 업데이트 안내 최신 버전 안내…",
    content:
      "6. BO에서 제공하는 APP 관리 기능 아래 기능은 BO 범위 후보입니다. 실제 제공 기능과 우선순위, 운영 권한은 서비스·운영·개발 담당자가 합의한 뒤 확정합니다. TBD 기능 설명 공지 팝업 BO에서 등록·수정·삭제하고 앱 실행 시 또는 특정 화면에서 노출 이벤트 팝업 기간 설정, 이미지 및 링크 관리 인트로 이미지 및 노출 기간 관리 점검 모드 BO에서 점검 여부 설정 시 앱에서 점검 화면 표시 업데이트 안내 최신 버전 안내 팝업 강제 업데이트 최소 지원 버전 미만은 앱 사용 차단 후 스토어로 이동 앱 버전 관리 현재 버전, 최소 지원 버전, 최신 버전 관리 배너 관리 메인 배너, 링크, 노출 기간 관리 공지사항 BO에서 등록한 공지 조회 BO 보안 및 검수 기준 관리자 계정은 역할 기반 최소 권한, MFA와 세션 만료를 적용합니다. 접근 IP 제한은 운영 환경과 위협 모델을 검토해 적용 여부를 결정합니다. 등록·수정·삭제·승인·배포 이력을 관리자와 변경 전후 값 기준으로 감사 기록합니다. HTML 입력은 원칙적으로 금지하고 불가피한 경우 서버 Allowlist Sanitizer를 적용합니다. 이미지와 파일은 MIME·확장자·크기·악성코드를 검사합니다. 링크는 승인된 HTTPS Origin과 Path만 허용하고 앱에서는 다시 Route Allowlist를 검증합니다. javascript: , data: , 임의 Custom Scheme은 차단합니다. 운영 반영 전 미리보기를 제공하고, 승인·Rollback 절차는 변경 영향도와 운영 체계를 기준으로 확정합니다. TBD",
  ...
  },
  {
    ...
    excerpt:
      "Flutter Flavor dev · qa · prod 는 기본 예시입니다. 실제 환경 수와 이름, 승격 순서는 인프라·릴리스 정책과 함께 확정합니다. TBD 환경 설명 qa 검증 dev 개발 prod 운영 환경별 관리 항목 API Endpoint, 인증 Issuer·Client ID·Redirect URI와 WebView Origin Allowlist Firebase Project, Pus…",
    content:
      "12. Build Environment Flutter Flavor dev · qa · prod 는 기본 예시입니다. 실제 환경 수와 이름, 승격 순서는 인프라·릴리스 정책과 함께 확정합니다. TBD 환경 설명 qa 검증 dev 개발 prod 운영 환경별 관리 항목 API Endpoint, 인증 Issuer·Client ID·Redirect URI와 WebView Origin Allowlist Firebase Project, Push Sender와 Crash 수집 정책 App Name 및 App Icon Signing Key와 배포 자격 증명은 CI Secret Manager에서 분리 관리 개발·QA 앱은 운영 API·운영 인증 Client·운영 Push Topic 접근 금지 운영 빌드 Debug, WebView Inspection, Test 계정과 Mock Endpoint 비활성화 WebView Front-end 환경 WebView Front-end는 dev · qa · prod 환경과 배포 권한을 분리합니다. Host, Cookie 범위, CORS·CSP, 인증 Redirect URI와 Origin Allowlist는 서비스·환경별 승인 값으로 관리합니다. 브라우저에 노출되는 공개 설정과 서버·CI에서만 사용하는 Secret을 구분하고, Artifact·환경 설정·승인 이력을 애플리케이션별로 추적합니다.",
  ...
  },
  {
    ...
    excerpt:
      "네이밍 대상 규칙 예시 Class PascalCase UserProfile Method lowerCamelCase loadUserProfile() Variable lowerCamelCase userProfile Constant lowerCamelCase defaultTimeout 개발 규칙 Effective Dart 준수 Null Safety 적용 const 적극 사용 Widget 단일 책임…",
    content:
      "8. Coding Convention 네이밍 대상 규칙 예시 Class PascalCase UserProfile Method lowerCamelCase loadUserProfile() Variable lowerCamelCase userProfile Constant lowerCamelCase defaultTimeout 개발 규칙 Effective Dart 준수 Null Safety 적용 const 적극 사용 Widget 단일 책임 원칙 적용 공통 Widget 재사용 외부 입력은 타입·길이·범위·형식을 경계 계층에서 검증 dynamic , 강제 형변환과 Null 강제 해제는 경계 코드에서 지양 Repository Interface는 Domain에, 구현체와 DataSource는 Data에 배치 Token·Cookie·인증 Code·개인정보·카드정보를 로그와 예외 메시지에 포함 금지 보안 관련 상수와 허용 목록은 단일 정책 모듈에서 관리하고 테스트로 고정",
  ...
  },
  {
    ...
    excerpt:
      "API Dio Interceptor에서 Trace ID, Timeout과 표준 오류 형식을 공통 처리 401의 비로그인·만료·갱신 실패 의미를 API 계약에 따라 구분하고 승인된 인증 흐름으로 이동 403은 권한 부족으로 처리하며 세션 갱신을 반복하지 않음 429는 Retry-After 를 존중하고 Exponential Backoff 적용 조회 요청만 제한적으로 자동 재시도하고 Mutatio…",
    content:
      "10. Error Handling API Dio Interceptor에서 Trace ID, Timeout과 표준 오류 형식을 공통 처리 401의 비로그인·만료·갱신 실패 의미를 API 계약에 따라 구분하고 승인된 인증 흐름으로 이동 403은 권한 부족으로 처리하며 세션 갱신을 반복하지 않음 429는 Retry-After 를 존중하고 Exponential Backoff 적용 조회 요청만 제한적으로 자동 재시도하고 Mutation은 Idempotency 보장 시에만 재시도 서버 오류 원문과 Stack Trace를 사용자 또는 WebView에 노출하지 않음 UI 사용자 친화적인 오류 메시지 제공 오프라인·Timeout·권한 거부·사용자 취소·인증 만료를 구분 안전한 작업만 명시적인 사용자 동작으로 재시도 중복 Tap과 중복 결제를 방지하고 처리 중 상태를 명확히 표시 진단용 Trace ID만 제공하고 내부 Endpoint·Payload·예외 내용은 숨김 Bridge 승인된 오류 코드를 사용하고 알 수 없는 예외는 INTERNAL_ERROR 로 변환 Timeout 후 도착한 응답은 폐기하고 완료된 Request ID를 다시 실행하지 않음 오류 로그에는 Method 분류와 Trace ID만 남기고 Params 원문은 기록하지 않음",
  ...
  },
  {
    ...
    excerpt:
      "아래 Branch 구성은 Git Flow 기반의 EXAMPLE 입니다. 릴리스 빈도와 팀 규모에 따라 GitHub Flow 또는 Trunk-based 방식을 함께 비교하고 표준을 확정합니다. TBD Branch 설명 main 운영 develop 개발 feature/* 기능 개발 release/* QA hotfix/* 긴급 수정 Merge 기준 main 과 develop 직접 Push 금지 및…",
    content:
      "9. Git Strategy 아래 Branch 구성은 Git Flow 기반의 EXAMPLE 입니다. 릴리스 빈도와 팀 규모에 따라 GitHub Flow 또는 Trunk-based 방식을 함께 비교하고 표준을 확정합니다. TBD Branch 설명 main 운영 develop 개발 feature/* 기능 개발 release/* QA hotfix/* 긴급 수정 Merge 기준 main 과 develop 직접 Push 금지 및 Branch 보호 적용 Pull Request 검토, 필수 CI, 보안 점검과 승인 완료 후 Merge 운영 배포는 <app-name>/v<version> 형식의 앱별 Tag와 Release Note를 남기고 해당 Artifact와 연결 Secret 노출 Commit은 삭제만 하지 않고 즉시 자격 증명을 폐기·교체",
  ...
  },
  {
    ...
    section: "Security",
    excerpt:
      "인증 및 세션 승인된 인증 시스템이 자격 증명·계정 상태와 권한을 최종 검증 Cookie 기반 세션을 채택하면 HttpOnly·Secure를 적용하고 SameSite·Domain·Path는 사이트와 Redirect 구조에 맞춰 결정 채택한 인증 상태의 갱신·폐기와 완전한 로그아웃을 지원하고 사용자 전환 시 실제 사용한 Cookie, token과 관련 WebView 데이터 정리 Cookie 기…",
    content:
      "11. Security 인증 및 세션 승인된 인증 시스템이 자격 증명·계정 상태와 권한을 최종 검증 Cookie 기반 세션을 채택하면 HttpOnly·Secure를 적용하고 SameSite·Domain·Path는 사이트와 Redirect 구조에 맞춰 결정 채택한 인증 상태의 갱신·폐기와 완전한 로그아웃을 지원하고 사용자 전환 시 실제 사용한 Cookie, token과 관련 WebView 데이터 정리 Cookie 기반 인증의 상태 변경 요청은 CSRF Token과 Origin/Referer 검증 등 승인된 CSRF 방어 적용 Native와 WebView에 인증 상태가 함께 존재하면 서버 수명주기와 로그아웃을 하나의 기준으로 통합 비밀번호 변경, 계정 복구, 고위험 결제 등 중요 작업 전 재인증 통신 및 WebView TLS 기반 HTTPS만 사용하고 Android Cleartext Traffic 비활성화 Scheme·Host·Port로 구성된 정확한 Origin Allowlist와 허용 Path 정책 적용 서버에서 CSP, HSTS, 안전한 Cookie와 민감 응답 no-store 적용 Bridge Origin·Main Frame·입력 Schema·권한·Rate Limit·Replay 검증 불필요한 File/Content Access와 운영 WebView Debugging 비활성화 데이터 및 운영 환경별 Secret 분리 및 소스 코드·앱 Bundle·Web JavaScript 하드코딩 금지 Signing Key와 CI Secret은 전용 Secret Manager에서 최소 권한으로 관리 개인정보 최소 수집, 보존 기간과 삭제 정책 적용 및 로그 Redaction Root/Jailbreak 탐지는 위협 모델상 필요한 경우 보조 신호로만 사용하고 서버 권한 검증을 대체하지 않음 정기 위협 모델링, Dependency 점검, 침투 테스트와 사고 대응 절차 운영 참고 보안 기준 OWASP Mobile Application Security Android WebView Unsafe URI Loading",
  ...
  },
  {
    ...
    section: "디버깅과 호환성 검증",
    excerpt:
      "디버깅 Android는 Chrome DevTools의 원격 디버깅, iOS는 Safari Web Inspector로 WebView 콘텐츠를 확인합니다. WebView 디버깅 기능은 개발 빌드에서만 활성화하고 운영 빌드에서는 비활성화합니다. 플랫폼 검증 환경 플랫폼 필수 검증 환경 주요 확인 항목 Android 주요 국가의 Android SDK 24 이상에서 구형·대표·최신 System Web…",
    content:
      "12. 디버깅과 호환성 검증 디버깅 Android는 Chrome DevTools의 원격 디버깅, iOS는 Safari Web Inspector로 WebView 콘텐츠를 확인합니다. WebView 디버깅 기능은 개발 빌드에서만 활성화하고 운영 빌드에서는 비활성화합니다. 플랫폼 검증 환경 플랫폼 필수 검증 환경 주요 확인 항목 Android 주요 국가의 Android SDK 24 이상에서 구형·대표·최신 System WebView 화면 복귀, 키보드, 권한, 파일 선택, Tailwind 렌더링과 엔진별 차이 iOS iOS 15와 최신 iOS의 WKWebView 화면 복귀, 키보드, 권한, 파일 선택과 WebKit 차이 공통 검증 항목 페이지 로딩, 재시도와 네트워크 단절·지연 로그인, 로그아웃, 쿠키와 세션 유지 JavaScript Bridge와 허용되지 않은 요청 차단 외부 URL, Custom Scheme와 뒤로 가기 HTTP·SSL 오류와 사용자 안내 Safe Area, 화면 회전, 접근성 글자 크기와 다크 모드 메모리 사용량과 화면 종료 후 리소스 해제 한국·중국·일본·영문의 주요 단말과 중국 제조사별 WebView 차이 선택한 최신 Tailwind Utility의 기능 저하와 대체 표현",
  ...
  },
  {
    ...
    section: "문서 목적과 적용 범위",
    excerpt:
      "이 문서는 Flutter의 공식 WebView 플러그인인 webview_flutter 를 사용하는 Android 및 iOS 애플리케이션에 적용합니다. WebView 시작 URL, 인증 방식, JavaScript Bridge 계약, 허용 도메인과 외부 앱 호출 정책은 프로젝트 담당자가 합의한 뒤 확정합니다. WebView의 개념과 담당 영역을 먼저 확인하려면 모바일 앱 WebView 개요 를…",
    content:
      "1. 문서 목적과 적용 범위 이 문서는 Flutter의 공식 WebView 플러그인인 webview_flutter 를 사용하는 Android 및 iOS 애플리케이션에 적용합니다. WebView 시작 URL, 인증 방식, JavaScript Bridge 계약, 허용 도메인과 외부 앱 호출 정책은 프로젝트 담당자가 합의한 뒤 확정합니다. WebView의 개념과 담당 영역을 먼저 확인하려면 모바일 앱 WebView 개요 를 참고합니다. Native와 WebView의 역할, 인증·세션 및 Bridge의 상위 기준은 APP 개발 표준 을 함께 적용합니다.",
  ...
  },
  {
    ...
    section: "세션, 오류와 보안",
    excerpt:
      "쿠키와 세션 로그인 쿠키의 생성 주체, Domain·Path·Secure·SameSite 속성, 로그아웃 시 삭제 범위와 앱 재실행 시 세션 유지 정책을 사전에 합의합니다. 인증 토큰은 URL Query Parameter에 포함하지 않습니다. 오류 상태 최초 로딩과 정상 표시 네트워크 연결 실패와 HTTP 오류 SSL 또는 인증서 오류 허용되지 않은 URL 이동 세션 만료와 콘텐츠 렌더링 실…",
    content:
  ...
  },
  {
    ...
    section: "우선 결정 항목",
    excerpt:
      "다음 항목은 실제 애플리케이션 요구사항과 담당자 합의 후 확정합니다. 항목 결정할 내용 SDK와 패키지 Flutter·Dart 버전과 고정할 webview_flutter 버전 접속 범위 시작 URL, 허용 도메인과 외부 앱 호출 정책 인증 쿠키와 세션 생성·유지·삭제 정책 Bridge 채널명, 요청·응답·오류 계약과 허용 기능 기기 기능 파일, 카메라, 위치, 마이크와 다운로드 범위 사용자…",
    content:
      "13. 우선 결정 항목 다음 항목은 실제 애플리케이션 요구사항과 담당자 합의 후 확정합니다. 항목 결정할 내용 SDK와 패키지 Flutter·Dart 버전과 고정할 webview_flutter 버전 접속 범위 시작 URL, 허용 도메인과 외부 앱 호출 정책 인증 쿠키와 세션 생성·유지·삭제 정책 Bridge 채널명, 요청·응답·오류 계약과 허용 기능 기기 기능 파일, 카메라, 위치, 마이크와 다운로드 범위 사용자 경험 로딩, 오류, 재시도, 세션 만료와 종료 화면 지원 범위 핵심 국가별 허용할 미지원 사용자 비율과 기준 재검토 주기 Flutter WebView 설정 가이드",
  ...
  },
  {
    ...
    excerpt:
      "Flutter 공식 플러그인인 webview_flutter 를 사용합니다. 문서 작성 시 확인한 최신 안정 버전은 4.14.1 입니다. dependencies : webview_flutter : ^4.14.1 최신 버전을 자동으로 사용하는 규칙으로 해석하지 않습니다. 실제 프로젝트의 Flutter·Dart SDK와 호환성을 확인하고 승인된 버전을 pubspec.lock 에 고정합니다. 패키지…",
    content:
      "2. 패키지 선정과 버전 기준 Flutter 공식 플러그인인 webview_flutter 를 사용합니다. 문서 작성 시 확인한 최신 안정 버전은 4.14.1 입니다. dependencies : webview_flutter : ^4.14.1 최신 버전을 자동으로 사용하는 규칙으로 해석하지 않습니다. 실제 프로젝트의 Flutter·Dart SDK와 호환성을 확인하고 승인된 버전을 pubspec.lock 에 고정합니다. 패키지 버전과 플랫폼별 요구사항은 webview_flutter 공식 패키지 문서 와 공식 버전 목록 에서 확인합니다. 패키지 역할 추가 방법 webview_flutter 공통 WebView Controller와 Widget API 기본 의존성으로 추가 webview_flutter_android Android 전용 WebView 구현과 기능 기본적으로 자동 포함, 전용 API를 직접 Import할 때 명시 webview_flutter_wkwebview iOS 전용 WKWebView 구현과 기능 기본적으로 자동 포함, 전용 API를 직접 Import할 때 명시",
  ...
  },
  {
    ...
    section: "패키지 설치",
    excerpt:
      "프로젝트 루트에서 패키지를 추가하고 선택된 버전을 확인합니다. flutter pub add webview_flutter flutter pub get 설치 결과는 pubspec.yaml 과 pubspec.lock 을 함께 확인합니다. 플랫폼 전용 API를 직접 사용하지 않으면 구현 패키지를 별도로 추가하지 않습니다.",
    content:
      "6. 패키지 설치 프로젝트 루트에서 패키지를 추가하고 선택된 버전을 확인합니다. flutter pub add webview_flutter flutter pub get 설치 결과는 pubspec.yaml 과 pubspec.lock 을 함께 확인합니다. 플랫폼 전용 API를 직접 사용하지 않으면 구현 패키지를 별도로 추가하지 않습니다.",
  ...
  },
  {
    ...
    section: "iOS 설정",
    excerpt:
      "Deployment Target Xcode의 Runner Target과 ios/Podfile 을 모두 iOS 15 이상으로 설정합니다. platform :ios , '15.0' App Transport Security HTTPS 사용을 원칙으로 합니다. HTTP 연결이 필요해도 NSAllowsArbitraryLoads 로 모든 연결을 허용하지 않고 필요한 개발 도메인만 예외로 등록합니다. 카…",
    content:
      "8. iOS 설정 Deployment Target Xcode의 Runner Target과 ios/Podfile 을 모두 iOS 15 이상으로 설정합니다. platform :ios , '15.0' App Transport Security HTTPS 사용을 원칙으로 합니다. HTTP 연결이 필요해도 NSAllowsArbitraryLoads 로 모든 연결을 허용하지 않고 필요한 개발 도메인만 예외로 등록합니다. 카메라, 마이크 또는 위치정보를 사용하는 경우에는 실제 사용하는 기능에 해당하는 목적 문구를 Info.plist 에 등록합니다.",
  ...
  },
  {
    ...
    excerpt:
      'flowchart LR accTitle: Native와 WebView의 역할 및 통신 흐름 accDescr: 사용자는 WebView의 서비스 화면을 이용하며, 서버 데이터는 Backend API에서 처리하고 기기 기능은 Bridge를 거쳐 Flutter Native와 OS에서 처리합니다. U([사용자]) --> W["Next.js WebView · 서비스 UI와 업무 흐름"] W -->|"서…',
    content:
  ...
  },
  {
    ...
    section: "접근성 기본",
    excerpt:
      "현재 목표는 인증 획득 자체가 아니라, 추후 접근성 검수를 확장할 수 있는 기본 구조를 유지하는 것입니다. 동작 요소는 가능한 한 의미에 맞는 HTML 요소를 사용합니다. 클릭 동작에 div 대신 button 또는 a 를 사용합니다. 입력 요소에는 연결된 label 또는 동등한 접근 가능한 이름을 제공합니다. 주요 기능은 키보드로 사용할 수 있어야 합니다. 포커스 표시를 이유 없이 제거하지…",
    content:
      "10. 접근성 기본 현재 목표는 인증 획득 자체가 아니라, 추후 접근성 검수를 확장할 수 있는 기본 구조를 유지하는 것입니다. 동작 요소는 가능한 한 의미에 맞는 HTML 요소를 사용합니다. 클릭 동작에 div 대신 button 또는 a 를 사용합니다. 입력 요소에는 연결된 label 또는 동등한 접근 가능한 이름을 제공합니다. 주요 기능은 키보드로 사용할 수 있어야 합니다. 포커스 표시를 이유 없이 제거하지 않습니다. 의미 있는 이미지에는 적절한 대체 텍스트를 제공하고, 장식 이미지는 보조기술이 무시하도록 처리합니다. 색상만으로 상태나 오류를 전달하지 않습니다. shadcn/ui가 제공하는 키보드, 포커스 및 ARIA 구조를 임의로 깨뜨리지 않습니다. Dialog, Sheet 등 레이어 UI는 열림·닫힘 시 포커스 이동과 복귀를 확인합니다. TBD WCAG 목표 수준, 색상 대비 수치 검수, 스크린리더 테스트 범위와 접근성 인증 일정은 별도 품질 계획에서 확정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "Flutter Native 앱과 앱 내 WebView Front-end는 하나의 모노레포에서 관리합니다. Native 앱은 apps/mobile , WebView Front-end는 apps/app-webview , 구현·검토 문서는 docs 에 둡니다. 이 문서에서 Front-end 또는 Web은 apps/app-webview 구현 영역을 의미합니다. 전체 저장소 구조와 운영 원칙은 Fro…",
    content:
      "4. 프로젝트 및 파일 구조 Flutter Native 앱과 앱 내 WebView Front-end는 하나의 모노레포에서 관리합니다. Native 앱은 apps/mobile , WebView Front-end는 apps/app-webview , 구현·검토 문서는 docs 에 둡니다. 이 문서에서 Front-end 또는 Web은 apps/app-webview 구현 영역을 의미합니다. 전체 저장소 구조와 운영 원칙은 Front-End 저장소 구조 기준 을 단일 기준으로 사용합니다. 이 절은 apps/app-webview 의 상세 배치 기준을 보완하며, 내용이 충돌하면 공통 기준을 우선합니다. 4.1 배치 원칙 라우트, 레이아웃 및 Next.js 예약 파일은 apps/app-webview/src/app 에서 관리합니다. 특정 기능에서만 쓰는 UI, API 연동, 폼, 훅과 타입은 해당 기능 가까이에 둡니다. 여러 기능에서 실제로 공유하는 UI 원형과 WebView 전용 조합은 apps/app-webview/src/components/ui 에서 관리합니다. 비슷해 보인다는 이유만으로 코드를 공통 영역이나 공통 패키지로 이동하지 않습니다. 사용하지 않는 빈 폴더, store, hook, wrapper 또는 패키지를 미리 만들지 않습니다. <repository-root>/ ├── AGENTS.md # 저장소 전체 구현 지침 ├── apps/ │ ├── mobile/ # Flutter Native 앱 │ └── app-webview/ # 앱 내 Next.js WebView │ └── src/ │ ├── app/ # Routes, Layouts, Providers │ ├── components/ # 공통 UI와 WebView 전용 UI 조합 │ ├── features/ # 서비스 기능별 구현 │ └── lib/ # API·Bridge 등 앱 기반 코드 ├── docs/ # 상세 가이드와 AI 작업 기준 └── README.md 하나의 기능이 커진 뒤에만 다음처럼 내부 책임을 나눌 수 있습니다. src/features/ticket/ ├── api/ ├── components/ ├── forms/ └── types.ts 4.2 컴포넌트와 파일 분리 단순한 컴포넌트는 한 파일로 시작합니다. 파일 길이 자체보다 책임의 분리 필요성, 변경 빈도, 재사용 범위를 기준으로 파일을 나눕니다. 타입, variant, hook 또는 테스트 파일은 실제 분리 이유가 있을 때만 만듭니다. 모든 컴포넌트에 동일한 폴더·파일 세트를 강제하지 않습니다. Button/ ├── Button.tsx ├── Button.types.ts # 외부 공개 타입이 커졌을 때 ├── Button.variants.ts # variant 조합이 복잡해졌을 때 └── Button.test.tsx # 위험도상 테스트가 필요할 때 4.3 공통화 판단 공통화는 코드 모양이 아니라 책임과 변경 이유를 기준으로 합니다. 다음 질문에 모두 답할 수 있을 때 공통화를 검토합니다. 둘 이상의 실제 사용처가 있는가? 사용처들의 의미와 변경 이유가 같은가? 공통화 후 API가 원래 구현보다 이해하기 쉬운가? 특정 기능의 예외를 props나 조건문으로 계속 추가하지 않아도 되는가? 4.4 저장소 운영 기준 Flutter Native 앱과 WebView Front-end는 Pipeline, 환경 설정, Version과 Rollback 단위를 분리합니다. 변경 경로와 의존 관계를 기준으로 영향받는 영역을 빌드·테스트합니다. Bridge 계약 변경 시 Native 앱과 WebView Front-end의 회귀 테스트를 함께 수행합니다. Web Workspace와 Flutter는 Toolchain별 Lock File과 빌드 도구를 각각 관리합니다. 환경 변수, 인증 Client, Origin Allowlist와 배포 Secret은 애플리케이션·제휴사·환경별로 분리합니다. 운영 배포는 <app-name>/v<version> 형식의 앱별 Tag와 Release Note로 Artifact를 식별합니다.",
  ...
  },
  {
    ...
    excerpt:
      "아래 기술 스택은 apps/app-webview 에 적용합니다. Flutter Native 앱은 별도의 Dart·Flutter Toolchain과 구현 기준을 사용합니다. 정확한 패키지 버전은 Web Workspace와 Flutter 등 각 Toolchain의 Lock File을 기준으로 합니다. 구분 기술 기본 책임 Framework Next.js 16.x App Router 라우팅, 레이…",
    content:
      "3. 확정 기술 스택과 책임 아래 기술 스택은 apps/app-webview 에 적용합니다. Flutter Native 앱은 별도의 Dart·Flutter Toolchain과 구현 기준을 사용합니다. 정확한 패키지 버전은 Web Workspace와 Flutter 등 각 Toolchain의 Lock File을 기준으로 합니다. 구분 기술 기본 책임 Framework Next.js 16.x App Router 라우팅, 레이아웃, 렌더링과 웹 애플리케이션 구성 UI React 19.x 컴포넌트 기반 UI 구성 Language TypeScript 6.x 컴포넌트, API, 폼과 외부 경계의 타입 안전성 Styling Tailwind CSS 4.x 기본 스타일과 디자인 토큰 적용 Server State TanStack Query 5.x 서버 데이터 조회, 캐시, 동기화와 비동기 상태 Client State Zustand 필요한 공유 UI·클라이언트 상태 Form React Hook Form 7.x 폼 값, 입력 상태, 제출 상태와 오류 연결 UI Component shadcn/ui 프로젝트가 소유하고 수정하는 UI 원형의 출발점 각 기술은 위 책임 안에서 필요한 경우에만 사용합니다. 서버에서 받은 데이터를 Zustand에 복제하지 않습니다. 한 컴포넌트 또는 가까운 하위 트리에서만 쓰는 UI 상태는 우선 React 지역 상태로 관리합니다. 설치된 버전과 호환성을 확인하지 않은 코드 또는 AI 생성 코드를 그대로 병합하지 않습니다. TBD Server Component와 Client Component의 세부 경계, SSR·SSG·ISR, 캐싱 및 데이터 패칭 정책은 화면과 API 구조가 확정된 후 결정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "5.1 TypeScript 애플리케이션 코드는 TypeScript로 작성합니다. 컴포넌트 props, API 경계, 폼 입력, Bridge 입출력처럼 모듈 경계를 통과하는 값은 타입을 정의합니다. 외부 입력은 타입 선언만 신뢰하지 않고 실제 실행 시점에도 필요한 검증을 거칩니다. any , 과도한 타입 단언, 불필요한 제네릭 추상화는 피합니다. 불가피한 경우 범위를 경계 모듈 안으로 제한합니…",
    content:
      "5. TypeScript 및 Import 5.1 TypeScript 애플리케이션 코드는 TypeScript로 작성합니다. 컴포넌트 props, API 경계, 폼 입력, Bridge 입출력처럼 모듈 경계를 통과하는 값은 타입을 정의합니다. 외부 입력은 타입 선언만 신뢰하지 않고 실제 실행 시점에도 필요한 검증을 거칩니다. any , 과도한 타입 단언, 불필요한 제네릭 추상화는 피합니다. 불가피한 경우 범위를 경계 모듈 안으로 제한합니다. API 타입과 화면 전용 타입을 무조건 하나로 통합하지 않습니다. 의미와 변경 주기가 다르면 분리합니다. 한 파일에서만 쓰는 짧은 props 타입은 같은 파일에 두고, 여러 모듈이 사용하는 공개 타입만 별도 파일로 분리합니다. 타입의 배치, React Props와 공통 타입의 구체적인 작성 기준은 TypeScript 가이드 를 따릅니다. 5.2 Import alias와 배럴 파일 @ 는 src 경로를 가리키는 import alias로 사용합니다. 순환 참조를 만들거나 내부 구현을 무분별하게 노출하는 import 구조를 만들지 않습니다. 가까운 파일은 상대 경로를, 기능 또는 공통 영역을 가로지르는 import는 @/ alias를 사용할 수 있습니다. import { cn } from '@/lib/utils' ; import { TicketCard } from './TicketCard' ; index.ts 는 모든 폴더에 만들지 않습니다. 외부에 제공할 공개 API가 안정된 기능 또는 컴포넌트 폴더에서만 선택적으로 사용하고, 저장소 전체를 다시 내보내는 대형 배럴 파일은 피합니다. TBD 배럴 파일의 허용 범위와 lint 기반 import 경계 검사는 실제 모듈 구조가 생긴 후 확정합니다.",
  ...
  },
  {
    ...
    section: "관련 기준",
    excerpt:
      "API 요청, 오류 본문, 응답 검증과 취소 신호 연결은 API 요청 기반 구현 예시 를 따릅니다. 서버 데이터와 Query 상태 관리는 Front-End 개발 가이드 를 따릅니다. API 지연과 오류 재현에 별도 도구가 필요하면 API Mock 도입 판단 기준 을 확인합니다. 승인된 API 계약을 기준으로 Front-end 책임자 또는 프로젝트 담당자와 테스트 범위·관리 책임을 정하고 Ba…",
    content:
      "16. 관련 기준 API 요청, 오류 본문, 응답 검증과 취소 신호 연결은 API 요청 기반 구현 예시 를 따릅니다. 서버 데이터와 Query 상태 관리는 Front-End 개발 가이드 를 따릅니다. API 지연과 오류 재현에 별도 도구가 필요하면 API Mock 도입 판단 기준 을 확인합니다. 승인된 API 계약을 기준으로 Front-end 책임자 또는 프로젝트 담당자와 테스트 범위·관리 책임을 정하고 Backend에는 계약을 대신하지 않는 도구임을 공유합니다. 사용자 관점의 상태 전환 검증은 테스트 공통 설정과 구현 예시 와 테스트 가이드 를 따릅니다. Skeleton, Spinner, Progress와 Toast UI는 shadcn/ui Components 를 참고하되 실제 프로젝트에 설치된 소스와 디자인 토큰을 우선합니다. Front-End 네트워크 지연 대응 및 성능 검증 가이드",
  ...
  },
  {
    ...
    section: "리뷰 체크리스트",
    excerpt:
      "상태와 UI 최초 로딩과 기존 데이터 갱신을 구분했는가? 로딩, Empty, Timeout, Offline과 오류가 같은 상태로 처리되지 않는가? Skeleton, Spinner, Progress와 Toast를 목적에 맞게 선택했는가? 기존 shadcn/ui와 프로젝트 컴포넌트를 우선 사용했는가? 로딩과 Motion이 접근성과 사용자 입력을 방해하지 않는가? 요청 안전성 필요 없어진 요청의…",
    content:
  ...
  },
  {
    ...
    section: "작업 및 검증 흐름",
    excerpt:
      "기능에서 필요한 조회와 변경 요청을 구분합니다. 최초 로딩, 갱신, 성공, Empty, Timeout, Offline과 오류 상태를 정의합니다. 설치된 shadcn/ui와 기존 프로젝트 컴포넌트에서 표시 방식을 선택합니다. 해당 위험이 있는 기능에서 취소, 순서 역전, 중복 실행과 입력 보존을 검토하고 필요한 항목만 구현합니다. 계약 확정 후 사용할 수 있는 Backend 환경과 필요한 경우…",
    content:
      "14. 작업 및 검증 흐름 기능에서 필요한 조회와 변경 요청을 구분합니다. 최초 로딩, 갱신, 성공, Empty, Timeout, Offline과 오류 상태를 정의합니다. 설치된 shadcn/ui와 기존 프로젝트 컴포넌트에서 표시 방식을 선택합니다. 해당 위험이 있는 기능에서 취소, 순서 역전, 중복 실행과 입력 보존을 검토하고 필요한 항목만 구현합니다. 계약 확정 후 사용할 수 있는 Backend 환경과 필요한 경우 승인된 계약 기반의 Front-end 재현 도구에서 정상 응답과 API 지연을 확인합니다. 실제 Network Throttling과 연결 중단 환경에서 대표 화면을 확인합니다. 변경 전후 성능과 실패 복구 결과를 기록합니다. Lint, Type Check, Test와 Build 결과를 함께 확인한 뒤 사람이 승인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "네트워크 지연을 UI로 가리는 것만으로 완료하지 않습니다. 초기 화면에 필요하지 않은 전송과 실행을 줄여 사용자가 기다리는 원인도 함께 확인합니다. Bundle은 브라우저에 전달되는 JavaScript 묶음이고, 지연 로딩(Lazy Loading)은 화면에 당장 필요하지 않은 코드나 이미지를 나중에 불러오는 방식입니다. CDN(Content Delivery Network)은 이미지와 정적 파…",
    content:
      "10. 지연 예방과 전송 성능 네트워크 지연을 UI로 가리는 것만으로 완료하지 않습니다. 초기 화면에 필요하지 않은 전송과 실행을 줄여 사용자가 기다리는 원인도 함께 확인합니다. Bundle은 브라우저에 전달되는 JavaScript 묶음이고, 지연 로딩(Lazy Loading)은 화면에 당장 필요하지 않은 코드나 이미지를 나중에 불러오는 방식입니다. CDN(Content Delivery Network)은 이미지와 정적 파일을 사용자와 가까운 서버에서 전달하는 구성입니다. 실제 CDN과 API 구조는 이 Front-end 가이드에서 확정하지 않습니다. 초기 화면의 필수 데이터와 후순위 데이터를 구분합니다. 요청 수, 전송량과 같은 데이터의 중복 요청을 확인합니다. 화면에 필요한 필드와 리소스만 요청하되 API 방식을 Front-end 문서에서 임의로 변경하지 않습니다. 이미지 크기와 표시 크기를 맞추고 화면 밖 리소스의 지연 로딩을 검토합니다. 폰트와 외부 Script가 핵심 콘텐츠 표시를 막는지 확인합니다. 새 package를 추가할 때 초기 Bundle과 Client 실행 비용을 확인합니다. Cache는 데이터 수명, 사용자 경계와 무효화 조건이 확인된 데이터에만 적용합니다.",
  ...
  },
  {
    ...
    excerpt:
      "7.1 필요 없어진 요청 취소 화면 이동, 검색어 변경과 필터 변경으로 결과가 더 이상 필요하지 않으면 요청 취소 신호를 실제 fetch 까지 전달합니다. 공통 요청 함수의 기본 연결은 API 요청 기반 구현 예시 를 따릅니다. 요청 취소는 사용자에게 보여줄 서버 오류와 구분합니다. 취소된 이전 요청 때문에 현재 화면에 오류 메시지를 표시하지 않습니다. 7.2 응답 순서 역전 방지 검색어 A…",
    content:
  ...
  },
  {
    ...
    section: "핵심 원칙",
    excerpt:
      "사용자의 네트워크가 항상 빠르고 안정적이라고 가정하지 않습니다. 로딩 때문에 기존 데이터, 사용자 입력과 완료된 작업을 불필요하게 제거하지 않습니다. 최초 로딩과 기존 데이터를 유지한 갱신을 같은 화면으로 처리하지 않습니다. 로딩, 성공, 빈 결과, 오류, Timeout과 Offline을 서로 다른 상태로 구분합니다. 사용자가 더 이상 필요로 하지 않는 요청은 가능한 범위에서 취소합니다. 늦…",
    content:
      "3. 핵심 원칙 사용자의 네트워크가 항상 빠르고 안정적이라고 가정하지 않습니다. 로딩 때문에 기존 데이터, 사용자 입력과 완료된 작업을 불필요하게 제거하지 않습니다. 최초 로딩과 기존 데이터를 유지한 갱신을 같은 화면으로 처리하지 않습니다. 로딩, 성공, 빈 결과, 오류, Timeout과 Offline을 서로 다른 상태로 구분합니다. 사용자가 더 이상 필요로 하지 않는 요청은 가능한 범위에서 취소합니다. 늦게 도착한 이전 응답이 최신 화면을 덮어쓰지 않게 합니다. 조회와 변경 요청의 재시도 정책을 구분합니다. 정상 환경뿐 아니라 저속과 연결 중단 환경의 검증 결과를 작업 근거로 남깁니다.",
  ...
  },
  {
    ...
    excerpt:
      "아래 항목은 모든 화면의 확정 요구가 아닙니다. 실제 기능과 위험에 해당하는 시나리오만 선택해 검증합니다. 조회 화면 저속 네트워크에서 최초 화면에 진입합니다. 기존 데이터가 있는 상태에서 재조회합니다. 검색어와 필터를 빠르게 연속 변경합니다. 이전 요청이 최신 요청보다 늦게 도착하게 합니다. 결과 없음과 응답 지연을 구분합니다. 요청 도중 화면을 이동합니다. 변경 화면 저장 또는 제출 버튼…",
    content:
      "12. 화면 유형별 검증 후보 아래 항목은 모든 화면의 확정 요구가 아닙니다. 실제 기능과 위험에 해당하는 시나리오만 선택해 검증합니다. 조회 화면 저속 네트워크에서 최초 화면에 진입합니다. 기존 데이터가 있는 상태에서 재조회합니다. 검색어와 필터를 빠르게 연속 변경합니다. 이전 요청이 최신 요청보다 늦게 도착하게 합니다. 결과 없음과 응답 지연을 구분합니다. 요청 도중 화면을 이동합니다. 변경 화면 저장 또는 제출 버튼을 연속으로 실행합니다. 작성 중인 상태에서 요청을 실패시킵니다. 요청 도중 연결을 끊습니다. Timeout 뒤 같은 요청을 다시 실행합니다. 서버 처리가 완료되었지만 응답만 받지 못한 상황을 검토합니다. 연결 복구 화면 진입 전부터 Offline인 상황을 확인합니다. 화면 사용 중 Offline으로 전환합니다. 연결 복구 후 현재 화면의 데이터가 필요한 범위에서 갱신되는지 확인합니다. 다른 사용자로 전환한 뒤 이전 Cache가 노출되지 않는지 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "Offline은 브라우저가 연결 없음으로 판단했거나 실제 요청이 네트워크 문제로 실패한 상태입니다. navigator.onLine 같은 연결 표시만으로 서버 접속 가능 여부를 확정하지 않고 실제 요청 결과도 함께 봅니다. Offline 전환 시 현재 화면과 작성 중인 입력을 가능한 범위에서 보존합니다. 서버 확인이 필요한 기능은 사용할 수 없는 이유와 필요한 복구 동작을 안내합니다. 승인된…",
    content:
      "9. Offline과 연결 복구 Offline은 브라우저가 연결 없음으로 판단했거나 실제 요청이 네트워크 문제로 실패한 상태입니다. navigator.onLine 같은 연결 표시만으로 서버 접속 가능 여부를 확정하지 않고 실제 요청 결과도 함께 봅니다. Offline 전환 시 현재 화면과 작성 중인 입력을 가능한 범위에서 보존합니다. 서버 확인이 필요한 기능은 사용할 수 없는 이유와 필요한 복구 동작을 안내합니다. 승인된 Cache가 있으면 오래된 정보일 수 있음을 구분하여 표시합니다. 연결 복구 후 모든 요청을 동시에 다시 실행하지 않고 현재 화면에 필요한 데이터부터 갱신합니다. Offline 중 변경 요청을 Queue(나중에 처리할 작업 대기열)에 저장해 연결 복구 후 전송하는 기능은 중복, 순서, 보안과 충돌 해결 정책이 확정된 경우에만 도입합니다. Offline에서 제공할 화면, 저장 대상, 데이터 수명, 재연결 동기화와 충돌 처리 정책은 실제 서비스 및 보안 기준 확정 후 결정합니다.",
  ...
  },
  {
    ...
    section: "검증 기준",
    excerpt:
      "번역 완료 여부는 JSON 파싱 성공만으로 판단하지 않습니다. 카탈로그 구조 모든 대상 언어에 원본과 같은 메시지 ID가 존재합니다. translation 은 문자열이며 예상하지 않은 객체나 배열을 허용하지 않습니다. 원본 카탈로그에 없는 ID를 대상 카탈로그에 임의로 추가하지 않습니다. 생성 중 오류가 발생하면 일부 결과만 저장하지 않습니다. 메시지 보존 {name} , {count} 등…",
    content:
  ...
  },
  {
    ...
    section: "실행 언어 결정",
    excerpt:
      "실행 언어는 다음 우선순위로 결정합니다. 사용자가 앱 설정에서 직접 선택하고 저장한 언어 Flutter 앱이 WebView 초기 요청에 전달한 앱 또는 디바이스 언어 HTTP Accept-Language 브라우저의 navigator.languages 영어 언어 코드는 BCP 47 형식으로 받은 뒤 프로젝트가 지원하는 언어로 정규화합니다. const supportedLocales = [ 'ko…",
    content:
  ...
  },
  {
    ...
    section: "적용 전 결정 항목",
    excerpt:
      "아래 항목은 실제 애플리케이션과 제품 요구가 확인된 후 확정합니다. 중국어 번체를 별도 지원할지 여부 Lingui 도입 여부와 실제 설치 버전 JSON 카탈로그의 정확한 경로와 컴파일 결과 위치 로컬 LLM 제품, 모델 태그, 장비별 실행 기준과 배치 크기 서비스 용어집의 소유자, 파일 형식과 변경 승인 절차 고위험 문구의 검토 담당자와 승인 기록 방식 Flutter가 WebView에 loc…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "CSP(Content Security Policy)는 브라우저에서 실행하거나 불러올 수 있는 Script, Image, iframe과 연결 대상을 제한하는 정책입니다. 실제 허용 값은 배포 구조와 외부 서비스가 정해진 뒤 Backend·배포·보안 담당 기준과 함께 확정합니다. Mixed Content는 HTTPS 페이지에서 HTTP 리소스를 불러오는 상태입니다. Nonce는 요청마다 발급하는…",
    content:
      "6. 네트워크, CSP: 콘텐츠 보안 정책과 외부 리소스 CSP(Content Security Policy)는 브라우저에서 실행하거나 불러올 수 있는 Script, Image, iframe과 연결 대상을 제한하는 정책입니다. 실제 허용 값은 배포 구조와 외부 서비스가 정해진 뒤 Backend·배포·보안 담당 기준과 함께 확정합니다. Mixed Content는 HTTPS 페이지에서 HTTP 리소스를 불러오는 상태입니다. Nonce는 요청마다 발급하는 일회성 허용값이고 Hash는 Script 내용의 해시가 일치할 때만 실행을 허용하는 방식입니다. Content-Security-Policy-Report-Only 는 차단하지 않고 위반 내용만 보고하여 적용 영향을 먼저 확인하는 설정입니다. Production 통신은 HTTPS를 사용하고 Mixed Content를 허용하지 않습니다. API Base URL, 인증 Header와 Cookie 전달 방식은 공통 요청 경계에서 관리하되 기능별 권한 판단을 공통 Wrapper가 추측하지 않습니다. 외부 Script, iframe, Image와 연결 Origin은 실제 사용처를 기준으로 최소화합니다. CSP는 XSS의 유일한 방어가 아니라 추가 방어 계층입니다. 실제 CSP는 배포 구조와 Third-party Script가 확인된 뒤 Nonce 또는 Hash 기반 정책을 검토하고 Content-Security-Policy-Report-Only 로 영향부터 확인합니다. unsafe-inline , unsafe-eval 과 넓은 Origin 허용을 오류 해결 목적으로 추가하지 않습니다. Directive(지시어)는 CSP에서 리소스 종류별 허용 범위를 정하는 항목입니다. frame-ancestors 는 이 화면을 iframe으로 포함할 수 있는 출처, connect-src 는 API 연결 대상, img-src 는 이미지 출처 등을 제한합니다. 필요한 Directive는 Hosting·Backend 기준과 함께 결정합니다. TBD : CSP, CORS, Cookie, 보안 Header와 허용 Origin의 실제 값은 배포·인증 구성이 확정된 뒤 결정합니다.",
  ...
  },
  {
    ...
    section: "문서 목적과 적용 범위",
    excerpt:
      "이 문서는 Lounge Front-end와 WebView 화면을 구현하고 검토할 때 적용할 보안·개인정보 기준 원본입니다. Front-end가 직접 구현할 방어적 처리뿐 아니라 Backend·Native App·배포·보안 담당자에게 받아야 할 계약과 Front-end의 연동 검증 범위도 함께 구분합니다. UI에서 값을 숨기는 수준이 아니라 외부 입력이 화면, 네트워크, 저장소, 로그와 Nat…",
    content:
      "1. 문서 목적과 적용 범위 이 문서는 Lounge Front-end와 WebView 화면을 구현하고 검토할 때 적용할 보안·개인정보 기준 원본입니다. Front-end가 직접 구현할 방어적 처리뿐 아니라 Backend·Native App·배포·보안 담당자에게 받아야 할 계약과 Front-end의 연동 검증 범위도 함께 구분합니다. UI에서 값을 숨기는 수준이 아니라 외부 입력이 화면, 네트워크, 저장소, 로그와 Native Bridge를 통과하는 전체 흐름을 확인합니다. 구체적인 인증 제공자, Cookie 또는 token 방식, CSP 값, 허용 Origin, Redirect URI, 개인정보 분류와 보존 기간은 아직 확정하지 않습니다. 해당 결정은 Backend·App·배포·보안 담당 기준과 함께 확정합니다. 브라우저와 WebView의 코드는 사용자가 확인하고 변조할 수 있습니다. Client 검증은 사용자 경험과 조기 오류 발견을 위한 것이며 인증, 권한과 업무 규칙의 최종 통제를 대신하지 않습니다. 이 문서를 읽는 기준 “신뢰하지 않는다”는 해당 값을 사용하지 말라는 뜻이 아닙니다. 값이 저장하거나 전달한 당시와 같다고 단정하지 말고, 사용 목적과 위험도에 맞게 확인한다는 뜻입니다. 화면 테마나 접힘 상태처럼 영향이 작은 값은 허용 값과 기본값만 확인해도 됩니다. 권한, 금액, 사용자 식별자와 이동 URL처럼 영향이 큰 값은 형식뿐 아니라 허용 범위와 서버 계약도 확인합니다. “확인합니다”와 “검토합니다”는 모든 기능에 같은 보안 도구를 적용하라는 뜻이 아닙니다. 해당 기능에 실제로 존재하는 입력·저장·통신 경계만 확인합니다. Backend·Native App·배포 영역의 내용은 Front-end가 연동 전에 확인할 조건입니다. 현재 프로젝트에서 이미 구현됐다는 의미가 아니며 실제 방식과 제공 범위는 TBD 입니다.",
  ...
  },
  {
    ...
    section: "의존성과 공급망",
    excerpt:
      "공급망 보안(Supply Chain Security)은 직접 작성한 코드뿐 아니라 npm package, 설치 Script와 외부에서 가져온 코드가 안전한지도 확인하는 활동입니다. Lock File은 설치할 package의 정확한 Version과 의존 관계를 기록합니다. Severity는 취약점의 심각도, Breaking Change는 기존 코드와 호환되지 않는 변경, Owner는 해당 의존…",
    content:
  ...
  },
  {
    ...
    section: "참고 기준",
    excerpt:
      "OWASP XSS Prevention Cheat Sheet OWASP CSRF Prevention Cheat Sheet OWASP Logging Cheat Sheet MDN Content Security Policy MDN Secure Cookie Configuration Next.js Environment Variables React Security Advisories Next.js Sec…",
    content:
      "11. 참고 기준 OWASP XSS Prevention Cheat Sheet OWASP CSRF Prevention Cheat Sheet OWASP Logging Cheat Sheet MDN Content Security Policy MDN Secure Cookie Configuration Next.js Environment Variables React Security Advisories Next.js Security Advisories npm Security Documentation Front-End 보안과 개인정보 가이드",
  ...
  },
  {
    ...
    excerpt:
      "구조 변경 기준 새 애플리케이션을 추가할 때 Owner, 배포 단위, 환경, 권한과 Rollback 책임을 함께 정의합니다. 계약을 변경할 때 영향을 받는 TypeScript·Dart 생성 결과, Adapter와 출시된 앱 Version을 함께 검토합니다. 우선 결정 항목 실제 package에 사용할 패키지 관리자와 실행 Script 승인된 Backend API와 Native Bridge 계…",
    content:
      "8. 변경 기준과 미확정 항목 구조 변경 기준 새 애플리케이션을 추가할 때 Owner, 배포 단위, 환경, 권한과 Rollback 책임을 함께 정의합니다. 계약을 변경할 때 영향을 받는 TypeScript·Dart 생성 결과, Adapter와 출시된 앱 Version을 함께 검토합니다. 우선 결정 항목 실제 package에 사용할 패키지 관리자와 실행 Script 승인된 Backend API와 Native Bridge 계약 배포 플랫폼과 CI/CD 품질 Gate Front-End 저장소 구조 기준",
  ...
  },
  {
    ...
    section: "저장소 구조",
    excerpt:
      "<repository-root>/ ├── AGENTS.md # 저장소 전체 구현 지침 ├── README.md # 프로젝트 진입점 ├── apps/ │ ├── mobile/ # Flutter Native 앱 │ └── app-webview/ # 앱 내 Next.js WebView Front-end │ ├── AGENTS.md # 앱 전용 구현 지침 │ ├── .storybook/ # Stor…",
    content:
      "2. 저장소 구조 <repository-root>/ ├── AGENTS.md # 저장소 전체 구현 지침 ├── README.md # 프로젝트 진입점 ├── apps/ │ ├── mobile/ # Flutter Native 앱 │ └── app-webview/ # 앱 내 Next.js WebView Front-end │ ├── AGENTS.md # 앱 전용 구현 지침 │ ├── .storybook/ # Storybook 설정 │ └── src/ # 애플리케이션 source └── docs/ ├── AGENTS.md # 문서 작성 지침 ├── guides/ # 상세 가이드 ├── ai/ # AI 작업별 요약 └── common-source/ # 파일 단위 구현 기준 애플리케이션 범위 Directory 역할 기술·분리 상태 apps/mobile Flutter Native 앱과 기기 기능 Flutter·Dart apps/app-webview 앱 내 Front-end 서비스 화면과 업무 흐름 Next.js·React·TypeScript",
  ...
  },
  {
    ...
    section: "적용 범위와 표기",
    excerpt:
      "이 저장소의 WebView Front-end 애플리케이션과 구현 문서를 함께 관리합니다. 이 문서의 <repository-root> 는 현재 저장소 루트를 의미합니다. 표시 의미 적용 방법 MUST 확정 원칙 구현, 검수와 배포에서 반드시 준수 TBD 결정 필요 담당자 합의와 승인 후 이 문서에 반영 EXAMPLE 참고 예시 프로젝트 조건에 맞게 변경 가능",
    content:
      "1. 적용 범위와 표기 이 저장소의 WebView Front-end 애플리케이션과 구현 문서를 함께 관리합니다. 이 문서의 <repository-root> 는 현재 저장소 루트를 의미합니다. 표시 의미 적용 방법 MUST 확정 원칙 구현, 검수와 배포에서 반드시 준수 TBD 결정 필요 담당자 합의와 승인 후 이 문서에 반영 EXAMPLE 참고 예시 프로젝트 조건에 맞게 변경 가능",
  ...
  },
  {
    ...
    excerpt:
      "영역 책임 금지 사항 apps 독립적으로 빌드·테스트·배포 가능한 애플리케이션 다른 앱의 내부 소스 경로를 직접 Import하지 않음 docs 상세 가이드, AI 요약과 파일 단위 구현 기준 애플리케이션 source와 package 설정을 포함하지 않음",
    content:
      "3. Directory 책임 영역 책임 금지 사항 apps 독립적으로 빌드·테스트·배포 가능한 애플리케이션 다른 앱의 내부 소스 경로를 직접 Import하지 않음 docs 상세 가이드, AI 요약과 파일 단위 구현 기준 애플리케이션 source와 package 설정을 포함하지 않음",
  ...
  },
  {
    ...
    excerpt:
      "Web 프로젝트는 하나의 Workspace에서 공통 작업과 의존성 정책을 관리합니다. Flutter는 같은 저장소에 두되 Dart 의존성, Lock File과 빌드 도구를 Flutter Toolchain 기준으로 관리합니다. Web Workspace와 Flutter 등 각 Toolchain의 Lock File을 변경 사항과 함께 검토하고 CI에서 고정 설치합니다. Workspace, 패키지…",
    content:
      "5. Workspace와 의존성 Web 프로젝트는 하나의 Workspace에서 공통 작업과 의존성 정책을 관리합니다. Flutter는 같은 저장소에 두되 Dart 의존성, Lock File과 빌드 도구를 Flutter Toolchain 기준으로 관리합니다. Web Workspace와 Flutter 등 각 Toolchain의 Lock File을 변경 사항과 함께 검토하고 CI에서 고정 설치합니다. Workspace, 패키지 관리자와 Task Runner 제품은 CI·배포 플랫폼을 확인한 뒤 확정합니다. TBD",
  ...
  },
  {
    ...
    section: "도구와 명령 후보",
    excerpt:
      "현재 프로젝트에 다음 도구는 설치되어 있지 않습니다. 단위·컴포넌트: Vitest, React Testing Library, @testing-library/user-event , @testing-library/jest-dom E2E: Playwright API Mock: 프로젝트 기본 도구가 아니며 승인된 계약 이후 Front-end 테스트 필요에 따라 후보를 결정 npm run test n…",
    content:
      "7. 도구와 명령 후보 현재 프로젝트에 다음 도구는 설치되어 있지 않습니다. 단위·컴포넌트: Vitest, React Testing Library, @testing-library/user-event , @testing-library/jest-dom E2E: Playwright API Mock: 프로젝트 기본 도구가 아니며 승인된 계약 이후 Front-end 테스트 필요에 따라 후보를 결정 npm run test npm run test:watch npm run test:e2e 도구와 명령은 후보이며 한꺼번에 모두 도입하지 않습니다. 단위·컴포넌트 테스트부터 시작하고 실제 필요가 생길 때 E2E와 API Mock을 추가합니다.",
  ...
  },
  {
    ...
    section: "리뷰 체크리스트",
    excerpt:
      "변경 위험에 맞는 테스트 수준을 선택했는가? 사용자에게 보이는 동작과 결과를 검증했는가? 영향이 큰 오류·취소·중복 실행을 확인했는가? 테스트가 실행 순서와 외부 상태에 의존하지 않는가? Mock이 실제 계약 오류를 숨기지 않는가? 실제 개인정보와 운영 Secret을 사용하지 않았는가? 테스트하지 못한 위험을 변경 설명에 남겼는가? 도입 순서 단위·컴포넌트 테스트 도구를 비교하고 프로젝트 기…",
    content:
      "9. 리뷰 체크리스트 변경 위험에 맞는 테스트 수준을 선택했는가? 사용자에게 보이는 동작과 결과를 검증했는가? 영향이 큰 오류·취소·중복 실행을 확인했는가? 테스트가 실행 순서와 외부 상태에 의존하지 않는가? Mock이 실제 계약 오류를 숨기지 않는가? 실제 개인정보와 운영 Secret을 사용하지 않았는가? 테스트하지 못한 위험을 변경 설명에 남겼는가? 도입 순서 단위·컴포넌트 테스트 도구를 비교하고 프로젝트 기준을 확정합니다. 오류 영향이 큰 순수 로직이나 핵심 컴포넌트부터 시작합니다. 반복되는 설정이 확인된 경우에만 공통 테스트 도구를 만듭니다. 승인된 API 계약과 사용할 수 있는 Backend 환경으로 재현하기 어려운 상태가 확인되면 Front-end 책임자 또는 프로젝트 담당자가 Front-end 테스트 범위와 관리 책임을 정해 API Mock을 검토하고, 여러 시스템 경계를 통과하는 핵심 흐름이 생기면 Playwright를 검토합니다. 실행 시간과 안정성을 확인한 뒤 CI 범위를 확정합니다. Front-End 테스트 가이드",
  ...
  },
  {
    ...
    section: "목적과 현재 상태",
    excerpt:
      "이 문서는 Front-End 변경으로 발생할 수 있는 주요 회귀를 효율적으로 확인하기 위한 테스트 기준을 정리합니다. 테스트 개수나 커버리지 수치 자체를 목표로 삼지 않습니다. 오류가 발생했을 때 사용자, 금액, 권한과 데이터에 미치는 영향이 큰 동작부터 검증합니다. 현재 프로젝트에는 테스트 도구와 test 스크립트가 없습니다. 도구, 명령과 CI 기준은 실제 테스트 환경을 구성할 때 확정합…",
    content:
      "1. 목적과 현재 상태 이 문서는 Front-End 변경으로 발생할 수 있는 주요 회귀를 효율적으로 확인하기 위한 테스트 기준을 정리합니다. 테스트 개수나 커버리지 수치 자체를 목표로 삼지 않습니다. 오류가 발생했을 때 사용자, 금액, 권한과 데이터에 미치는 영향이 큰 동작부터 검증합니다. 현재 프로젝트에는 테스트 도구와 test 스크립트가 없습니다. 도구, 명령과 CI 기준은 실제 테스트 환경을 구성할 때 확정합니다.",
  ...
  },
  {
    ...
    section: "우선 테스트 대상",
    excerpt:
      "금액, 이용 조건과 유효기간 등 핵심 계산 사용자 권한과 상태에 따른 기능 분기 결제, 예약과 이용권 사용의 중복 실행 방지 폼의 주요 검증, 제출과 서버 오류 연결 TanStack Query의 로딩, 빈 결과와 오류 상태 Bridge의 요청·응답 변환과 미지원 환경 처리 여러 화면에서 사용하는 공통 UI의 주요 동작 서비스 흐름이 확정된 핵심 E2E 시나리오 단순 표시, Tailwind 클…",
    content:
      "3. 우선 테스트 대상 금액, 이용 조건과 유효기간 등 핵심 계산 사용자 권한과 상태에 따른 기능 분기 결제, 예약과 이용권 사용의 중복 실행 방지 폼의 주요 검증, 제출과 서버 오류 연결 TanStack Query의 로딩, 빈 결과와 오류 상태 Bridge의 요청·응답 변환과 미지원 환경 처리 여러 화면에서 사용하는 공통 UI의 주요 동작 서비스 흐름이 확정된 핵심 E2E 시나리오 단순 표시, Tailwind 클래스 문자열과 라이브러리 내부 동작은 기본 테스트 대상이 아닙니다.",
  ...
  },
  {
    ...
    section: "작성 기준",
    excerpt:
      '테스트는 대상 코드 가까이에 둡니다. 조건과 기대 결과가 드러나는 테스트 이름을 사용합니다. 반복되는 설정이 확인된 경우에만 공통 wrapper와 fixture를 만듭니다. Zustand store나 QueryClient는 테스트별 상태가 섞이지 않도록 초기화합니다. role과 label로 찾기 어려울 때만 data-testid 를 사용합니다. it ( "필수 약관에 동의하지 않으면 예약 요…',
    content:
      '6. 작성 기준 테스트는 대상 코드 가까이에 둡니다. 조건과 기대 결과가 드러나는 테스트 이름을 사용합니다. 반복되는 설정이 확인된 경우에만 공통 wrapper와 fixture를 만듭니다. Zustand store나 QueryClient는 테스트별 상태가 섞이지 않도록 초기화합니다. role과 label로 찾기 어려울 때만 data-testid 를 사용합니다. it ( "필수 약관에 동의하지 않으면 예약 요청을 보내지 않는다" , async () => { // 사용자 동작과 화면 결과를 검증합니다. }); TBD 테스트 파일명과 배치 경로는 실제 테스트 환경을 구성할 때 확정합니다.',
  ...
  },
  {
    ...
    section: "적용 원칙",
    excerpt:
      "구현 세부사항보다 사용자가 확인할 수 있는 결과를 검증합니다. 정상 흐름과 함께 영향이 큰 오류·취소·중복 실행을 확인합니다. 라이브러리 자체 동작보다 프로젝트가 추가한 로직과 연결을 검증합니다. 버그를 수정할 때는 가능한 범위에서 회귀 테스트를 추가합니다. 모든 컴포넌트와 Hook에 형식적인 테스트를 만들지 않습니다.",
    content:
      "2. 적용 원칙 구현 세부사항보다 사용자가 확인할 수 있는 결과를 검증합니다. 정상 흐름과 함께 영향이 큰 오류·취소·중복 실행을 확인합니다. 라이브러리 자체 동작보다 프로젝트가 추가한 로직과 연결을 검증합니다. 버그를 수정할 때는 가능한 범위에서 회귀 테스트를 추가합니다. 모든 컴포넌트와 Hook에 형식적인 테스트를 만들지 않습니다.",
  ...
  },
  {
    ...
    section: "CI와 완료 기준",
    excerpt:
      "테스트 환경과 CI 구성이 확정되면 관련 테스트를 PR 필수 검사 후보로 적용합니다. 공통 계약이나 패키지 변경 시 영향받는 사용처를 함께 확인합니다. 실패한 테스트를 skip하거나 재시도 횟수만 늘려 우회하지 않습니다. 커버리지는 누락 지점을 찾는 참고 자료로 사용하며 근거 없는 전체 백분율을 목표로 삼지 않습니다. TBD CI 실행 범위, E2E 환경과 커버리지 기준은 배포 환경이 정해진…",
    content:
      "8. CI와 완료 기준 테스트 환경과 CI 구성이 확정되면 관련 테스트를 PR 필수 검사 후보로 적용합니다. 공통 계약이나 패키지 변경 시 영향받는 사용처를 함께 확인합니다. 실패한 테스트를 skip하거나 재시도 횟수만 늘려 우회하지 않습니다. 커버리지는 누락 지점을 찾는 참고 자료로 사용하며 근거 없는 전체 백분율을 목표로 삼지 않습니다. TBD CI 실행 범위, E2E 환경과 커버리지 기준은 배포 환경이 정해진 뒤 확정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "네트워크와 Bridge는 필요한 경계에서만 대체합니다. 성공뿐 아니라 기능상 중요한 업무 오류, 취소와 미지원 결과를 준비합니다. 테스트는 실행 순서와 이전 테스트 결과에 의존하지 않습니다. 토큰, 카드번호와 개인정보 같은 실제 운영 데이터를 사용하지 않습니다. 과도한 Mock으로 실제 연결 오류를 숨기지 않습니다. 기획과 Backend API 계약이 확정되기 전에는 API Mock, fix…",
    content:
      "5. Mock과 테스트 데이터 네트워크와 Bridge는 필요한 경계에서만 대체합니다. 성공뿐 아니라 기능상 중요한 업무 오류, 취소와 미지원 결과를 준비합니다. 테스트는 실행 순서와 이전 테스트 결과에 의존하지 않습니다. 토큰, 카드번호와 개인정보 같은 실제 운영 데이터를 사용하지 않습니다. 과도한 Mock으로 실제 연결 오류를 숨기지 않습니다. 기획과 Backend API 계약이 확정되기 전에는 API Mock, fixture와 handler를 만들지 않습니다. 계약 확정 후 사용할 수 있는 Backend 환경에서 필요한 상태를 재현하기 어렵다면 Front-end 책임자 또는 프로젝트 담당자와 테스트 범위와 관리 책임을 정한 뒤 API Mock 도입 판단 기준 을 확인합니다. TBD Bridge Mock의 공통 fixture와 실제 API 계약 기반 fixture의 공유 범위는 구현 대상이 확인된 뒤 결정합니다.",
  ...
  },
  {
    ...
    section: "디자인 토큰",
    excerpt:
      '컴포넌트에서는 원시 색상이나 임의 색상값보다 프로젝트의 의미 기반 Tailwind 토큰을 사용합니다. // 권장 < div className = "border-border bg-card text-card-foreground" /> // 지양 < div className = "border-gray-200 bg-[#ffffff] text-gray-900" /> 현재는 코드 리뷰에서 확인합니다.…',
    content:
      '5. 디자인 토큰 컴포넌트에서는 원시 색상이나 임의 색상값보다 프로젝트의 의미 기반 Tailwind 토큰을 사용합니다. // 권장 < div className = "border-border bg-card text-card-foreground" /> // 지양 < div className = "border-gray-200 bg-[#ffffff] text-gray-900" /> 현재는 코드 리뷰에서 확인합니다. 동적 class와 예외를 안정적으로 구분할 수 없는 단순 문자열 검색은 CI 차단 규칙으로 사용하지 않습니다. TBD 같은 위반이 반복되면 허용 토큰과 예외 정책을 먼저 정한 뒤 자동 검사를 검토합니다.',
  ...
  },
  {
    ...
    section: "리뷰 체크리스트",
    excerpt:
      "npm run typecheck 와 npm run lint 를 통과했는가? lint 오류를 disable 주석으로 우회하지 않았는가? Promise의 성공과 실패 처리 방식이 명확한가? label, 키보드 조작과 포커스 구조가 올바른가? 프로젝트 디자인 토큰을 사용했는가? 자동 검사로 확인하지 못한 위험을 변경 설명에 남겼는가? 도입 순서 현재 Next.js 설정의 실제 오류와 경고를 확인합…",
    content:
      "8. 리뷰 체크리스트 npm run typecheck 와 npm run lint 를 통과했는가? lint 오류를 disable 주석으로 우회하지 않았는가? Promise의 성공과 실패 처리 방식이 명확한가? label, 키보드 조작과 포커스 구조가 올바른가? 프로젝트 디자인 토큰을 사용했는가? 자동 검사로 확인하지 못한 위험을 변경 설명에 남겼는가? 도입 순서 현재 Next.js 설정의 실제 오류와 경고를 확인합니다. 필요한 접근성 규칙만 오류로 승격합니다. no-floating-promises 의 실행 시간과 기존 코드 영향을 확인합니다. 합의된 규칙만 CI 필수 검사로 연결합니다.",
  ...
  },
  {
    ...
    section: "적용 범위와 예외",
    excerpt:
      "애플리케이션의 TypeScript와 TSX 파일을 기본 검사 대상으로 둡니다. 테스트, 설정과 생성 코드에는 같은 규칙을 일괄 적용하지 않습니다. TBD 파일 유형별 검사 범위와 완화 규칙은 실제 파일이 생긴 뒤 확정합니다. eslint-disable 이 꼭 필요한 경우 규칙 이름과 사유를 남기고 최소 범위에만 적용합니다. 파일 전체 비활성화와 규칙 이름이 없는 비활성화는 사용하지 않습니다.",
    content:
      "6. 적용 범위와 예외 애플리케이션의 TypeScript와 TSX 파일을 기본 검사 대상으로 둡니다. 테스트, 설정과 생성 코드에는 같은 규칙을 일괄 적용하지 않습니다. TBD 파일 유형별 검사 범위와 완화 규칙은 실제 파일이 생긴 뒤 확정합니다. eslint-disable 이 꼭 필요한 경우 규칙 이름과 사유를 남기고 최소 범위에만 적용합니다. 파일 전체 비활성화와 규칙 이름이 없는 비활성화는 사용하지 않습니다.",
  ...
  },
  {
    ...
    section: "적용 원칙",
    excerpt:
      "코드 오류를 정적으로 확실하게 판단할 수 있는 규칙만 오류로 둡니다. 새 규칙은 기존 코드 영향과 실행 시간을 확인한 뒤 적용합니다. eslint-disable 로 오류를 먼저 우회하지 않습니다. 자동 수정 후에는 Diff를 확인하고 typecheck 와 lint 를 다시 실행합니다. 사용되지 않는 disable 지시문은 경고 이상으로 유지합니다.",
    content:
      "2. 적용 원칙 코드 오류를 정적으로 확실하게 판단할 수 있는 규칙만 오류로 둡니다. 새 규칙은 기존 코드 영향과 실행 시간을 확인한 뒤 적용합니다. eslint-disable 로 오류를 먼저 우회하지 않습니다. 자동 수정 후에는 Diff를 확인하고 typecheck 와 lint 를 다시 실행합니다. 사용되지 않는 disable 지시문은 경고 이상으로 유지합니다.",
  ...
  },
  {
    ...
    section: "참고 자료",
    excerpt:
      "Next.js ESLint 설정 typescript-eslint no-floating-promises eslint-plugin-jsx-a11y UI 접근성 준수 가이드 Front-End Lint 가이드",
    content:
      "9. 참고 자료 Next.js ESLint 설정 typescript-eslint no-floating-promises eslint-plugin-jsx-a11y UI 접근성 준수 가이드 Front-End Lint 가이드",
  ...
  },
  {
    ...
    section: "CI 적용 기준",
    excerpt:
      "CI 구성이 확정되면 typecheck 와 lint 를 PR 필수 검사 후보로 적용합니다. 새 규칙은 기존 오류 수와 실행 시간을 확인한 뒤 CI 차단 조건으로 전환합니다. warning 허용 여부와 저장소 검사 범위는 실제 CI 설정을 기준으로 확인합니다. LLM이 생성한 코드도 같은 기준을 적용합니다. 모델의 설명을 검증 결과로 간주하지 않고 실제 명령 결과를 확인합니다.",
    content:
      "7. CI 적용 기준 CI 구성이 확정되면 typecheck 와 lint 를 PR 필수 검사 후보로 적용합니다. 새 규칙은 기존 오류 수와 실행 시간을 확인한 뒤 CI 차단 조건으로 전환합니다. warning 허용 여부와 저장소 검사 범위는 실제 CI 설정을 기준으로 확인합니다. LLM이 생성한 코드도 같은 기준을 적용합니다. 모델의 설명을 검증 결과로 간주하지 않고 실제 명령 결과를 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "apps/ 에는 실제 앱 코드, docs/ 에는 구현·검토 기준을 둡니다. 앱을 빌드할 때 문서는 포함하지 않습니다. PROJECT STRUCTURE Root/ ├─ AGENTS.md # AI 최상위 작업 원칙 ├─ apps/ # 앱 소스 │ ├─ mobile/ # Flutter Native │ └─ app-webview/ # WebView Front-end │ ├─ .storybook/ #…",
    content:
      "코드와 가이드를 같은 저장소에 apps/ 에는 실제 앱 코드, docs/ 에는 구현·검토 기준을 둡니다. 앱을 빌드할 때 문서는 포함하지 않습니다. PROJECT STRUCTURE Root/ ├─ AGENTS.md # AI 최상위 작업 원칙 ├─ apps/ # 앱 소스 │ ├─ mobile/ # Flutter Native │ └─ app-webview/ # WebView Front-end │ ├─ .storybook/ # UI 확인 │ └─ src/ # Source · Story · Test └─ docs/ # 가이드 ├─ AGENTS.md # AI 가이드 작성 기준 ├─ guides/ # 사람과 AI가 확인하는 상세 HTML 가이드 ├─ ai/ # AI 작업별 요약 ├─ common-source/ # 파일 단위 적용 예시 └─ assets/ · search/ · templates/ # 가이드 공통 자산 ROOT RULES AGENTS.md 저장소 전체에서 AI가 따라야 할 작업·편집·검증 원칙 APP SOURCE apps/ app-webview/ 은 Front-end, mobile/ 은 별도 앱 개발자가 담당 GUIDES docs/ 구현·검토 기준과 예시를 제공하며 앱에는 포함하지 않음 03 · DOCS LAYERS",
  ...
  },
  {
    ...
    excerpt:
      "01 · DECIDE 사람이 방향과 기준을 결정 화면 목적·업무 규칙·연동 방식·최종 품질 가이드 02 · BUILD AI와 사람이 같이 구현 가이드와 기존 코드를 확인해 UI·상태·Story·Test 작성 검토 03 · COMPLETE 사람의 최종 확인으로 완료 Source·Story·검사·빌드 최종 결과 확인 UPDATE RULE 정책과 기준이 바뀌면 가이드도 함께 갱신됩니다. 기존 템플…",
    content:
      "AI와 함께 구현하고 최종 확인은 사람이 합니다. 01 · DECIDE 사람이 방향과 기준을 결정 화면 목적·업무 규칙·연동 방식·최종 품질 가이드 02 · BUILD AI와 사람이 같이 구현 가이드와 기존 코드를 확인해 UI·상태·Story·Test 작성 검토 03 · COMPLETE 사람의 최종 확인으로 완료 Source·Story·검사·빌드 최종 결과 확인 UPDATE RULE 정책과 기준이 바뀌면 가이드도 함께 갱신됩니다. 기존 템플릿으로 브리핑 살펴보기 ⧉ ↑ ↓",
  ...
  },
  {
    ...
    excerpt:
      "승인된 API 계약을 받은 뒤 공통 요청 함수와 기능별 응답 검증 경계를 연결합니다. Front-end는 API 응답을 화면에 사용하기 전에 필요한 형태를 방어적으로 확인합니다. 요청 검증, 권한과 업무 규칙은 Backend가 다시 검증하며 Front-end Type과 검증 결과를 신뢰 근거로 사용하지 않습니다. 최초 조회 Loading과 기존 데이터를 유지한 갱신 상태 결과가 없는 Empt…",
    content:
      "7. 서버 통신과 비동기 화면 승인된 API 계약을 받은 뒤 공통 요청 함수와 기능별 응답 검증 경계를 연결합니다. Front-end는 API 응답을 화면에 사용하기 전에 필요한 형태를 방어적으로 확인합니다. 요청 검증, 권한과 업무 규칙은 Backend가 다시 검증하며 Front-end Type과 검증 결과를 신뢰 근거로 사용하지 않습니다. 최초 조회 Loading과 기존 데이터를 유지한 갱신 상태 결과가 없는 Empty 상태 사용자 재시도가 가능한 Error 상태 느린 응답 중 중복 제출 방지와 진행 안내 연결 끊김과 복구 후 갱신 모든 요청에 같은 Timeout과 Retry를 적용하지 않습니다. 조회와 변경 요청, 멱등성과 업무 위험도를 기준으로 결정합니다. 결제·예약처럼 중복 실행 위험이 있는 작업은 Front-end 재시도만으로 안전하다고 판단하지 않습니다. 승인된 계약이 있고 실제 환경에서 오류 상태를 반복 재현하기 어렵다면 MSW를 선택적으로 검토합니다. MSW는 API 계약을 임의로 만드는 도구가 아닙니다. 상세 기준: 느린 Network, Loading, Timeout, Retry, Offline과 측정 방법은 네트워크 지연 대응 및 성능 검증 가이드 에서 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "WebView 서비스 화면과 업무 흐름은 apps/app-webview 에 구현합니다. apps/ ├── app-webview/ # Front-end가 구현하는 WebView 화면 └── mobile/ # 별도 앱 개발자가 담당하는 Flutter 소스 docs/ # 구현·검토 기준과 예시 Application 가까이에서 시작합니다 apps/app-webview 안에서는 Routing과 화면…",
    content:
      "2. 작업 대상과 저장소 구조 WebView 서비스 화면과 업무 흐름은 apps/app-webview 에 구현합니다. apps/ ├── app-webview/ # Front-end가 구현하는 WebView 화면 └── mobile/ # 별도 앱 개발자가 담당하는 Flutter 소스 docs/ # 구현·검토 기준과 예시 Application 가까이에서 시작합니다 apps/app-webview 안에서는 Routing과 화면 진입점, 기능 코드와 WebView 전용 UI를 가까운 위치에서 관리합니다. App-WebView 한 곳에서만 사용하는 shadcn/ui 원형과 Wrapper는 apps/app-webview/src/components/ui 에 둡니다. 검증된 구현만 공통으로 관리합니다 여러 기능에서 실제로 재사용되고 의미와 변경 이유가 같은 UI만 src/components/ui 에서 공통으로 관리합니다. 한 화면에서만 사용하는 Wrapper, 공통화 가능성을 예상한 Type과 Adapter는 미리 만들지 않습니다. apps/ 와 docs/ 의 최상위 분리는 실제 프로젝트 기준입니다. 앱 내부 Package 경계와 Build 구성은 실제 사용처와 현재 설정을 기준으로 판단합니다. 상세 기준: 저장소 경계와 공통화 기준은 Front-End 저장소 구조 기준 에서 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "구현 중 함께 확인할 접근성 Semantic HTML과 Heading 구조 Keyboard 사용과 식별 가능한 Focus Input Label, 오류 메시지와 접근 가능한 이름 Dialog와 동적 UI의 Focus 이동 색상에만 의존하지 않는 상태 표현 긴 문구, 확대와 다국어에서 유지되는 Layout 기본 검증 순서는 TypeScript 검사 → Lint → Test 입니다. Story 변…",
    content:
      "12. 접근성, 테스트와 품질 Gate 구현 중 함께 확인할 접근성 Semantic HTML과 Heading 구조 Keyboard 사용과 식별 가능한 Focus Input Label, 오류 메시지와 접근 가능한 이름 Dialog와 동적 UI의 Focus 이동 색상에만 의존하지 않는 상태 표현 긴 문구, 확대와 다국어에서 유지되는 Layout 기본 검증 순서는 TypeScript 검사 → Lint → Test 입니다. Story 변경에는 Storybook 정적 Build를 추가하고 실제 구성에 Build가 있으면 Application Build도 확인합니다. 우선 Test 대상 중요한 Form 검증과 제출 서버 성공·실패와 재시도 상태 권한 거부와 Bridge 오류 중요한 업무 분기와 회귀 위험 Interaction 작업 완료 Checklist 정상·예외 상태를 구현했는가? 기존 Component와 Token을 우선 사용했는가? 새 공통 Component와 독립 재현 가치가 있는 기능·화면 상태에 Story가 있는가? Keyboard, Focus와 접근 가능한 이름을 확인했는가? 지원 Browser와 필요한 Viewport에서 확인했는가? 실제로 구성된 검사가 통과했는가? 확인하지 못한 환경과 미확정 계약을 보고했는가? 상세 기준: Type 경계는 TypeScript 가이드 , 정적 검사는 Lint 가이드 , Test 대상과 제외 기준은 테스트 가이드 에서 확인합니다.",
  ...
  },
  {
    ...
    excerpt:
      "Browser 기본 검토 기준 Chrome·Edge 지원 대상 구형 버전 Safari 15 이상 Firefox 지원 대상 구형 버전 Samsung Internet 지원 대상 구형 버전 Tailwind CSS v4를 유지하고 공통 CSS 하한은 Safari 15에서 지원되는 기능 범위로 정합니다. Android와 다른 Browser는 특정 제품 버전을 기계적으로 대응시키지 않고 주요 구형·최신…",
    content:
  ...
  },
  {
    ...
    section: "역할과 책임",
    excerpt:
      "담당 주요 책임 디자이너 공통 컴포넌트, Auto Layout, Variables와 Styles를 우선 사용합니다. 코드 구조를 고려해 모든 레이어 이름을 정리할 책임은 지지 않습니다. AI 코드 작업자 Figma와 프로젝트 소스를 함께 분석하고, 기존 컴포넌트와 토큰을 우선한 구현 초안을 제공합니다. 프론트엔드 개발자 변환 범위를 정하고 구조, 접근성, 반응형 동작과 중복을 검수하며 예외와…",
    content:
      "3. 역할과 책임 담당 주요 책임 디자이너 공통 컴포넌트, Auto Layout, Variables와 Styles를 우선 사용합니다. 코드 구조를 고려해 모든 레이어 이름을 정리할 책임은 지지 않습니다. AI 코드 작업자 Figma와 프로젝트 소스를 함께 분석하고, 기존 컴포넌트와 토큰을 우선한 구현 초안을 제공합니다. 프론트엔드 개발자 변환 범위를 정하고 구조, 접근성, 반응형 동작과 중복을 검수하며 예외와 최종 결과를 승인합니다.",
  ...
  },
  {
    ...
    section: "Figma 해석 기준",
    excerpt:
      "의미 있는 레이어 이름은 코드 생성 정확도를 높이기 위한 권장사항이며 필수 조건은 아닙니다. 기본 이름이 남아 있더라도 시각적 구조, 텍스트, 컴포넌트 정보와 프로젝트 소스를 함께 분석해 구현합니다. 디자이너에게 개발 구조에 맞춘 전체 레이어 정리를 요구하지 않습니다. 색상, 간격과 크기는 레이어 이름보다 Variables와 Auto Layout을 우선 기준으로 사용합니다. 공통 컴포넌트 인…",
    content:
      "6. Figma 해석 기준 의미 있는 레이어 이름은 코드 생성 정확도를 높이기 위한 권장사항이며 필수 조건은 아닙니다. 기본 이름이 남아 있더라도 시각적 구조, 텍스트, 컴포넌트 정보와 프로젝트 소스를 함께 분석해 구현합니다. 디자이너에게 개발 구조에 맞춘 전체 레이어 정리를 요구하지 않습니다. 색상, 간격과 크기는 레이어 이름보다 Variables와 Auto Layout을 우선 기준으로 사용합니다. 공통 컴포넌트 인스턴스는 가능한 한 유지하고 같은 요소를 새로 만들지 않습니다. 동작, 데이터 의미 또는 반응형 규칙이 불명확하면 프론트엔드 개발자가 설명을 추가하거나 담당자에게 확인합니다.",
  ...
  },
  {
    ...
    section: "기본 구성",
    excerpt:
      "Storybook은 먼저 apps/app-webview 에서 작은 범위로 도입합니다. 여러 애플리케이션에서 실제 공유되는 UI와 운영 필요성이 확인된 뒤 별도 Storybook Application 또는 공통 구성을 검토합니다. Next.js 프로젝트에서는 설치된 Next.js, React, Node.js와 Build 설정을 확인한 뒤 호환되는 Storybook Framework를 선택합니다…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "Storybook은 실제 애플리케이션과 같은 기준으로 컴포넌트와 화면 UI를 렌더링해야 합니다. globals.css 와 Tailwind CSS 4 설정을 연결합니다. Semantic Token과 CSS Variable을 애플리케이션과 동일하게 사용합니다. 프로젝트의 import alias를 유지합니다. 실제 렌더링에 필요한 Font와 전역 Provider만 연결합니다. Router, Que…",
    content:
      "5. 애플리케이션 스타일 연결 Storybook은 실제 애플리케이션과 같은 기준으로 컴포넌트와 화면 UI를 렌더링해야 합니다. globals.css 와 Tailwind CSS 4 설정을 연결합니다. Semantic Token과 CSS Variable을 애플리케이션과 동일하게 사용합니다. 프로젝트의 import alias를 유지합니다. 실제 렌더링에 필요한 Font와 전역 Provider만 연결합니다. Router, Query, Theme 또는 Form Context는 해당 컴포넌트나 화면에 필요한 최소 범위로 제공합니다. Page 또는 Screen Story의 Route Parameter와 화면 입력값은 Story 안에서 재현 가능하게 고정합니다. API 응답을 재현해야 한다면 승인된 계약이 전달된 이후에만 해당 화면에 필요한 최소 Fixture 또는 Mock을 사용합니다. 전체 애플리케이션 환경을 재현하기 위한 과도한 Mock과 Provider를 만들지 않습니다. Storybook과 실제 화면의 스타일이 다르면 Story를 임의로 보정하지 않고 전역 스타일, Token, Provider와 Build 설정의 차이를 먼저 확인합니다.",
  ...
  },
  {
    ...
    section: "완료 기준",
    excerpt:
      "Story가 실제 컴포넌트의 export, Props와 variant를 사용하는가? 사용자에게 노출되는 화면을 라우터 주소 없이 Storybook 사이드바에서 찾을 수 있는가? 각 화면의 기본 상태와 실제로 지원하는 주요 상태 및 Viewport를 확인할 수 있는가? 기본 상태와 제품에서 실제로 지원하는 주요 상태가 포함되어 있는가? Story를 위해 불필요한 제품 Props, variant…",
    content:
      "11. 완료 기준 Story가 실제 컴포넌트의 export, Props와 variant를 사용하는가? 사용자에게 노출되는 화면을 라우터 주소 없이 Storybook 사이드바에서 찾을 수 있는가? 각 화면의 기본 상태와 실제로 지원하는 주요 상태 및 Viewport를 확인할 수 있는가? 기본 상태와 제품에서 실제로 지원하는 주요 상태가 포함되어 있는가? Story를 위해 불필요한 제품 Props, variant 또는 Mock을 추가하지 않았는가? Tailwind CSS, Semantic Token, Font와 필요한 Provider가 실제 애플리케이션과 일치하는가? Autodocs에서 공개 API를 확인할 수 있는가? Storybook 정적 Build와 합의된 검사를 통과하는가? Figma, Story와 실제 코드가 충돌하는 경우 차이를 확인하고 함께 갱신했는가? AI가 만든 결과를 프론트엔드 개발자가 검수했는가? Storybook과 AI 코딩 도구는 컴포넌트와 화면 UI의 발견, 검수와 유지보수 비용을 줄이는 작업 도구입니다. 생성된 Story는 사람이 작성한 코드와 같은 리뷰 및 검증 기준을 적용합니다. Storybook 운영 가이드",
  ...
  },
  {
    ...
    excerpt:
      "Autodocs를 사용해 Story와 TypeScript 정보에서 컴포넌트 문서를 생성합니다. Props 전체 목록을 별도 Markdown 문서에 중복 작성하지 않습니다. Storybook은 다음 질문의 기준으로 사용합니다. 어떤 공통 컴포넌트가 있는가? 컴포넌트가 어떤 Props와 variant를 제공하는가? 기본 상태와 주요 예외 상태가 어떻게 보이는가? 컴포넌트를 독립적으로 렌더링하고…",
    content:
      "7. Autodocs와 UI 카탈로그 Autodocs를 사용해 Story와 TypeScript 정보에서 컴포넌트 문서를 생성합니다. Props 전체 목록을 별도 Markdown 문서에 중복 작성하지 않습니다. Storybook은 다음 질문의 기준으로 사용합니다. 어떤 공통 컴포넌트가 있는가? 컴포넌트가 어떤 Props와 variant를 제공하는가? 기본 상태와 주요 예외 상태가 어떻게 보이는가? 컴포넌트를 독립적으로 렌더링하고 조작할 수 있는가? 어떤 사용자 화면이 있으며 화면별 대표 상태는 무엇인가? 라우터 주소를 몰라도 화면 이름과 서비스 영역으로 UI를 찾을 수 있는가? 화면이 Desktop, Tablet과 Mobile에서 어떻게 보이는가? Autodocs는 주로 컴포넌트의 공개 API를 설명하는 데 사용합니다. Page와 Screen Story는 화면 이름, 대표 상태, Viewport와 필요한 짧은 설명을 중심으로 구성합니다. 업무 목적, 사용 금지 조건이나 접근성 제약처럼 코드와 타입에서 알 수 없는 내용만 추가하고, 자세한 동작은 실제 코드와 사용처를 기준으로 합니다. Autodocs 설정은 Storybook Autodocs 문서 를 따릅니다.",
  ...
  },
  {
    ...
    excerpt:
      "Figma는 컴포넌트와 화면의 시각적 기준 및 디자인 의도를 제공하고 Storybook은 실제 코드로 구현된 컴포넌트와 화면의 렌더링 상태를 제공합니다. @storybook/addon-designs 공식 Embed Frame 데모 의 Story와 Design 패널 검수자는 라우터 주소나 개발용 진입 조건을 알지 못해도 Storybook 사이드바에서 서비스 영역, 화면 이름과 상태를 선택하고…",
    content:
      "8. Figma와 Storybook의 역할 Figma는 컴포넌트와 화면의 시각적 기준 및 디자인 의도를 제공하고 Storybook은 실제 코드로 구현된 컴포넌트와 화면의 렌더링 상태를 제공합니다. @storybook/addon-designs 공식 Embed Frame 데모 의 Story와 Design 패널 검수자는 라우터 주소나 개발용 진입 조건을 알지 못해도 Storybook 사이드바에서 서비스 영역, 화면 이름과 상태를 선택하고 실제 구현과 디자인 기준을 함께 확인할 수 있습니다. Figma Main Component의 Description에는 코드 컴포넌트 이름, import 경로와 주요 Props를 기록합니다. 실제 코드 또는 Storybook에 접근할 수 있는 URL이 있으면 Main Component의 Dev resource로 연결합니다. 외부 Storybook URL이 없으면 컴포넌트 이름과 Story title을 기록하고 실제 소스 링크를 우선 연결합니다. Figma Description, Story와 실제 코드가 다르면 실제 코드와 변경 의도를 확인한 뒤 함께 갱신합니다. Figma의 주요 Screen에는 연결되는 Story 이름을 기록하고, 접근 가능한 Storybook URL이 있으면 Dev resource로 연결합니다. Story에는 Figma 파일, Frame 또는 Prototype URL을 연결할 수 있습니다. 특정 Frame을 표시하려면 Figma에서 선택한 Frame의 링크를 복사해 사용합니다. 이 URL에는 해당 Frame의 node-id 가 포함됩니다. Storybook의 Design 탭에서 연결된 Figma Embed를 확인합니다. export const PaymentFailed = { parameters: { design: { type: 'figma', url: 'https://www.figma.com/design/FILE_KEY/...?node-id=120-480', }, }, }; 위 화면은 @storybook/addon-designs 공식 데모를 직접 캡처한 예시입니다. 실제 표시 결과는 Storybook과 Addon 버전, 패널 크기, Figma 파일 권한에 따라 달라질 수 있습니다. Storybook은 Code Connect를 대체하지 않지만 AI 코딩 도구와 개발자가 기존 컴포넌트의 API와 전체 화면 상태를 찾는 실행 가능한 UI 기준으로 사용합니다.",
  ...
  },
  {
    ...
    section: "Story 작성 기준",
    excerpt:
      "Story 파일은 컴포넌트 또는 화면 UI 구현 가까이에 둡니다. button.tsx button.stories.tsx 컴포넌트 Story는 실제 컴포넌트의 공개 API를 기준으로 작성합니다. 기본 상태를 작성합니다. 실제로 지원하는 주요 variant와 size를 작성합니다. disabled , loading , error , empty 처럼 컴포넌트가 실제로 소유하는 상태만 작성합니다.…",
    content:
      "6. Story 작성 기준 Story 파일은 컴포넌트 또는 화면 UI 구현 가까이에 둡니다. button.tsx button.stories.tsx 컴포넌트 Story는 실제 컴포넌트의 공개 API를 기준으로 작성합니다. 기본 상태를 작성합니다. 실제로 지원하는 주요 variant와 size를 작성합니다. disabled , loading , error , empty 처럼 컴포넌트가 실제로 소유하는 상태만 작성합니다. 중요한 사용자 상호작용은 재현 가능한 경우에만 작성합니다. 존재하지 않는 Props와 상태를 추측하지 않습니다. 내부 구현 세부사항보다 사용자가 선택할 수 있는 공개 API를 보여줍니다. 테스트를 통과시키기 위해 Story를 삭제하거나 검사 대상에서 임의로 제외하지 않습니다. Page와 Screen Story는 다음 기준으로 작성합니다. 라우터 주소를 몰라도 화면의 목적을 알 수 있는 이름을 사용합니다. 기본 화면을 작성하고 실제 제품이 지원하는 loading, empty, error, 권한과 완료 상태를 필요한 만큼 분리합니다. 실제 지원 범위에 맞춰 Desktop, Tablet과 Mobile Viewport에서 레이아웃을 확인합니다. 데이터, Route Parameter와 Provider는 같은 상태를 반복 재현할 수 있는 최소 범위로 제공합니다. 실제 결제, 인증, Backend 처리와 Native App 기능을 Storybook 안에서 완료된 것처럼 모사하지 않습니다. 여러 Route가 동일한 화면을 사용하면 Story를 중복하지 않고 화면 상태 또는 입력값으로 구분합니다. Story의 title , 이름과 분류는 UI의 실제 사용 범위를 드러내야 합니다. UI 원형, 애플리케이션 공통 조합, feature 컴포넌트와 화면이 같은 분류에 무질서하게 섞이지 않도록 합니다.",
  ...
  },
  {
    ...
    section: "문서 목적",
    excerpt:
      "이 문서는 Front-End 프로젝트에서 TypeScript와 React를 간결하고 일관되게 사용하는 기준을 제공합니다. 문법이나 상세 설계를 모두 설명하지 않고, 코드 작성과 리뷰에 자주 필요한 규칙을 예시 중심으로 정리합니다. 전체 구조는 Front-End 저장소 구조 기준 , Front-End 기술과 책임은 Front-End 개발 가이드 를 참고합니다. 현재 폴더 구조는 확정안이 아니므…",
    content:
      "1. 문서 목적 이 문서는 Front-End 프로젝트에서 TypeScript와 React를 간결하고 일관되게 사용하는 기준을 제공합니다. 문법이나 상세 설계를 모두 설명하지 않고, 코드 작성과 리뷰에 자주 필요한 규칙을 예시 중심으로 정리합니다. 전체 구조는 Front-End 저장소 구조 기준 , Front-End 기술과 책임은 Front-End 개발 가이드 를 참고합니다. 현재 폴더 구조는 확정안이 아니므로 이 문서에서 별도 경로를 고정하지 않습니다. API 계약, 데이터 모델 계층, Runtime Schema와 상태 관리 도구의 상세 규칙은 현재 범위에 포함하지 않으며, 각 설계가 확정된 후 기준을 정합니다.",
  ...
  },
  {
    ...
    section: "추후 확정 항목",
    excerpt:
      "공통 tsconfig 의 세부 Compiler Option 파일명과 타입명 세부 Convention 배럴 파일과 모듈 경계의 자동 검사 범위 TypeScript 가이드",
    content:
      "9. 추후 확정 항목 공통 tsconfig 의 세부 Compiler Option 파일명과 타입명 세부 Convention 배럴 파일과 모듈 경계의 자동 검사 범위 TypeScript 가이드",
  ...
  },
  {
    ...
    section: "타입 작성과 배치",
    excerpt:
      '3.1 추론 우선 값과 반환 타입이 명확하면 중복해서 작성하지 않습니다. // 권장 const title = "라운지 이용권" ; function formatCount ( count : number ) { return ` ${count} 건` ; } // 불필요한 중복 const title : string = "라운지 이용권" ; function formatCount ( count : num…',
    content:
  ...
  },
  {
    ...
    section: "핵심 원칙",
    excerpt:
      "추론 가능한 타입은 TypeScript에 맡깁니다. Props, 함수 입력과 모듈 경계처럼 중요한 계약은 타입으로 표현합니다. 타입은 사용하는 코드 가까이에서 시작합니다. 의미와 변경 이유가 같은 사용처가 생겼을 때만 공통화합니다. 작은 타입을 조합하고, 사용처가 없는 추상화는 만들지 않습니다. any , 근거 없는 타입 단언과 불필요한 Generic을 피합니다.",
    content:
      "2. 핵심 원칙 추론 가능한 타입은 TypeScript에 맡깁니다. Props, 함수 입력과 모듈 경계처럼 중요한 계약은 타입으로 표현합니다. 타입은 사용하는 코드 가까이에서 시작합니다. 의미와 변경 이유가 같은 사용처가 생겼을 때만 공통화합니다. 작은 타입을 조합하고, 사용처가 없는 추상화는 만들지 않습니다. any , 근거 없는 타입 단언과 불필요한 Generic을 피합니다.",
  ...
  },
  {
    ...
    excerpt:
      '타입만 가져올 때는 import type 을 사용합니다. import type { Ticket } from "./ticket.types" ; 가까운 파일은 상대 경로를 사용하고, 기능 또는 공통 영역을 가로지를 때 @/ alias를 사용할 수 있습니다. index.ts 는 안정된 공개 API가 있는 영역에서만 선택적으로 사용합니다. 모든 타입을 다시 내보내는 대형 배럴 파일은 만들지 않습니다…',
    content:
      '7. Import와 공개 타입 타입만 가져올 때는 import type 을 사용합니다. import type { Ticket } from "./ticket.types" ; 가까운 파일은 상대 경로를 사용하고, 기능 또는 공통 영역을 가로지를 때 @/ alias를 사용할 수 있습니다. index.ts 는 안정된 공개 API가 있는 영역에서만 선택적으로 사용합니다. 모든 타입을 다시 내보내는 대형 배럴 파일은 만들지 않습니다.',
  ...
  },
  {
    ...
    section: "애니메이션과 운동감",
    excerpt:
      "화면 이동, Dialog 등장, 로딩 표시처럼 요소가 위치·크기·투명도를 바꾸는 모든 Motion은 prefers-reduced-motion 설정을 존중합니다. 이 설정은 전정운동질환 사용자에게 중요합니다. 핵심 규칙 @media (prefers-reduced-motion: reduce) 안에서 모든 장식적 애니메이션을 animation: none 과 transition: none 으로 제거…",
    content:
      '9. 애니메이션과 운동감 화면 이동, Dialog 등장, 로딩 표시처럼 요소가 위치·크기·투명도를 바꾸는 모든 Motion은 prefers-reduced-motion 설정을 존중합니다. 이 설정은 전정운동질환 사용자에게 중요합니다. 핵심 규칙 @media (prefers-reduced-motion: reduce) 안에서 모든 장식적 애니메이션을 animation: none 과 transition: none 으로 제거합니다. 기능적 애니메이션(로딩 표시자, 오류 강조)은 유지하되 강도를 낮춥니다. 3초 이상 지속되는 반복 애니메이션은 prefers-reduced-motion 에서 중단합니다. 3초 이내에 3회 이상 깜빡이는 콘텐츠는 사용하지 않습니다. (WCAG 2.3.1) Tailwind CSS 적용 < div className = "animate-spin motion-reduce:animate-none" > 로딩 중... </ div > 확장 검토 prefers-reduced-motion 활성화 시 모든 라우트 전환, Dialog, Toast 동작을 검증',
  ...
  },
  {
    ...
    section: "참고 문서",
    excerpt:
      "WCAG 2.1 Quick Reference WAI-ARIA Authoring Practices Guide MDN ARIA Apple HIG Accessibility web.dev Learn Accessibility Lint 가이드 디자인 토큰 가이드 React Code Exports 가이드 UI 접근성 준수 가이드",
    content:
      "12. 참고 문서 WCAG 2.1 Quick Reference WAI-ARIA Authoring Practices Guide MDN ARIA Apple HIG Accessibility web.dev Learn Accessibility Lint 가이드 디자인 토큰 가이드 React Code Exports 가이드 UI 접근성 준수 가이드",
  ...
  },
  {
    ...
    section: "키보드 접근성",
    excerpt:
      "모든 상호작용 기능을 키보드만으로 수행할 수 있어야 합니다. 이는 스크린 리더 사용자가 VoiceOver의 두 손가락 탭, TalkBack의 두 손가락 탭으로 상호작용하는 것과 동일한 경로를 보장합니다. 핵심 규칙 모든 Button, Link, Input, Select, Tab을 Tab 키로 순차적으로 도달할 수 있습니다. 포커스 순서가 화면의 시각적 읽기 순서(위에서 아래, 왼쪽에서 오른쪽…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "화면과 플랫폼 Callback을 단순 success · failure Boolean으로 연결하지 않습니다. 아래 상태는 복귀 화면 설계를 위한 후보이며, 실제 상태 이름, 판정 주체와 Backend 연동은 모두 TBD 입니다. 상태 의미 사용자 처리 준비 중 Web 결제 절차를 준비하는 중이며 세부 처리 주체는 TBD 중복 입력 차단과 진행 표시 외부 결제 진행 중 Web이 구성한 요청에 따…",
    content:
      "6. 결제 중단·복귀 문제의 화면 처리 화면과 플랫폼 Callback을 단순 success · failure Boolean으로 연결하지 않습니다. 아래 상태는 복귀 화면 설계를 위한 후보이며, 실제 상태 이름, 판정 주체와 Backend 연동은 모두 TBD 입니다. 상태 의미 사용자 처리 준비 중 Web 결제 절차를 준비하는 중이며 세부 처리 주체는 TBD 중복 입력 차단과 진행 표시 외부 결제 진행 중 Web이 구성한 요청에 따라 Native가 외부 앱을 열었음 앱 복귀 안내와 중복 실행 차단 결과 확인 중 앱 복귀 또는 재진입 후 협의된 방식으로 결과를 확인함 결제 성공으로 미리 표시하지 않음 완료 협의된 결과 계약에서 성공으로 확인됨 예약·이용권 등 후속 결과 표시 실패 협의된 결과 계약에서 실패로 확인됨 안전한 재시도 또는 결제수단 변경 제공 취소 사용자가 결제 흐름을 명시적으로 취소함 오류로 과장하지 않고 기존 화면 복귀 미확정 복귀 유실, Timeout 또는 결과 확인 지연 자동 재결제 금지, 협의된 확인 수단 제공 앱이 백그라운드로 이동한 것만으로 취소 처리하지 않습니다. 앱이 활성화되어도 Web Return URL 처리가 완료되지 않았을 수 있습니다. 이때 결과를 확인하고 미확정 상태를 해소하는 방법은 Backend 협의 후 확정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "Flutter에서 외부 결제를 처리할 수 없는 것은 아닙니다. 다만 Web 결제 중 발생하는 플랫폼 전용 URL과 앱 전환을 Dart 코드 또는 범용 플러그인만으로 처리하면 Android·iOS의 실제 동작 차이가 추상화에 가려져 장애 원인과 책임 경계를 찾기 어려워질 수 있습니다. 문제 영역 발생 가능한 문제 이 프로젝트의 대응 Android Intent 해석 intent: URL의 Sch…",
    content:
  ...
  },
  {
    ...
    excerpt:
      "sequenceDiagram accTitle: WebView 외부 결제 Intent 실행과 복귀 흐름 accDescr: WebView가 결제를 구성하고 Intent 또는 Scheme을 Native에 전달하면 Native가 외부 앱을 열며, 서비스 앱 복귀 뒤 WebView가 협의된 후속 처리를 수행합니다. actor U as 사용자 participant W as Web 콘텐츠 particip…",
    content:
  ...
  },
  {
    ...
    section: "문서 목적",
    excerpt:
      "이 문서는 여러 UI 영역에서 공유하는 클라이언트 상태를 Zustand로 간결하게 관리하는 기준을 제공합니다. Zustand를 모든 상태의 기본 도구로 사용하지 않고, React 지역 상태만으로 관리하기 어려운 UI 상태에 선택적으로 사용합니다. 서버 상태는 TanStack Query, 폼 입력과 제출 상태는 React Hook Form, 주소로 공유하거나 복원할 상태는 URL을 우선 검토합…",
    content:
      "1. 문서 목적 이 문서는 여러 UI 영역에서 공유하는 클라이언트 상태를 Zustand로 간결하게 관리하는 기준을 제공합니다. Zustand를 모든 상태의 기본 도구로 사용하지 않고, React 지역 상태만으로 관리하기 어려운 UI 상태에 선택적으로 사용합니다. 서버 상태는 TanStack Query, 폼 입력과 제출 상태는 React Hook Form, 주소로 공유하거나 복원할 상태는 URL을 우선 검토합니다. 전체 상태 도구 선택 기준은 Front-End 개발 가이드 를 따릅니다. Store와 React Props의 타입 작성은 TypeScript 가이드 를 따릅니다.",
  ...
  },
  {
    ...
    section: "참고 문서",
    excerpt:
      "Zustand 공식 소개 Zustand TypeScript 가이드 Zustand Next.js 가이드 useShallow 가이드 Store Reset 가이드 Persist Middleware Zustand UI 상태 관리 가이드",
    content:
      "13. 참고 문서 Zustand 공식 소개 Zustand TypeScript 가이드 Zustand Next.js 가이드 useShallow 가이드 Store Reset 가이드 Persist Middleware Zustand UI 상태 관리 가이드",
  ...
  },
  {
    ...
    section: "추후 확정 항목",
    excerpt:
      "Store 분리 단위와 파일명 Convention App Router의 Provider 및 요청별 Store 적용 범위 Persist 허용 대상과 Hydration 처리 방식 Devtools 적용 환경과 Middleware 사용 기준 Selector 공통화와 테스트 범위",
    content:
      "12. 추후 확정 항목 Store 분리 단위와 파일명 Convention App Router의 Provider 및 요청별 Store 적용 범위 Persist 허용 대상과 Hydration 처리 방식 Devtools 적용 환경과 Middleware 사용 기준 Selector 공통화와 테스트 범위",
  ...
  },
  {
    ...
    section: "코드 리뷰 체크리스트",
    excerpt:
      "React 지역 상태로 충분한데 Zustand를 사용하지 않았는가? 서버·폼·URL 상태를 Store에 복제하지 않았는가? 컴포넌트가 필요한 값과 Action만 Selector로 구독하는가? 상태 변경 Action의 이름이 의도를 표현하는가? 기존 상태에서 계산할 수 있는 값을 중복 저장하지 않았는가? 하나의 Store에 변경 이유가 다른 상태가 섞이지 않았는가? 사용자 전환이나 기능 종료…",
    content:
      "11. 코드 리뷰 체크리스트 React 지역 상태로 충분한데 Zustand를 사용하지 않았는가? 서버·폼·URL 상태를 Store에 복제하지 않았는가? 컴포넌트가 필요한 값과 Action만 Selector로 구독하는가? 상태 변경 Action의 이름이 의도를 표현하는가? 기존 상태에서 계산할 수 있는 값을 중복 저장하지 않았는가? 하나의 Store에 변경 이유가 다른 상태가 섞이지 않았는가? 사용자 전환이나 기능 종료 시 필요한 상태를 초기화하는가? Persist가 실제 요구보다 넓게 적용되지 않았는가? shadcn/ui의 접근성과 기본 동작을 유지하는가?",
  ...
  },
  {
    ...
    section: "피해야 할 패턴",
    excerpt:
      "// 서버 데이터를 Zustand에 복제 const useTicketStore = create ( () => ({ tickets : [], isLoading : false , })); // 모든 UI 상태를 하나의 Store에 추가 type GlobalUiStore = { menuOpen : boolean ; dialogOpen : boolean ; formValues : Record < s…",
    content:
  ...
  },
  {
    ...
    section: "핵심 원칙",
    excerpt:
      "React 지역 상태로 해결되지 않을 때 Store를 만듭니다. Store는 UI 책임과 수명이 같은 상태만 포함합니다. 컴포넌트는 필요한 상태와 Action만 Selector로 구독합니다. 상태 변경은 의미가 드러나는 Action으로 표현합니다. 서버 상태와 폼 상태를 Zustand에 복제하지 않습니다. 사용자 전환이나 화면 종료 시 남으면 안 되는 상태는 초기화합니다. Persist와 M…",
    content:
      "3. 핵심 원칙 React 지역 상태로 해결되지 않을 때 Store를 만듭니다. Store는 UI 책임과 수명이 같은 상태만 포함합니다. 컴포넌트는 필요한 상태와 Action만 Selector로 구독합니다. 상태 변경은 의미가 드러나는 Action으로 표현합니다. 서버 상태와 폼 상태를 Zustand에 복제하지 않습니다. 사용자 전환이나 화면 종료 시 남으면 안 되는 상태는 초기화합니다. Persist와 Middleware는 실제 요구가 확인된 후 추가합니다.",
  ...
  },
  {
    ...
    excerpt:
      "Zustand Store를 사용하는 컴포넌트는 Client Component로 작성합니다. React Server Component에서 Store를 읽거나 수정하지 않습니다. 초기 상태는 서버와 클라이언트에서 다르게 계산하지 않습니다. 요청 또는 사용자별 데이터를 전역 Module Store에 저장하지 않습니다. TBD App Router에서 Store를 Module Hook으로 사용할 범위…",
    content:
      "8. Next.js App Router 기준 Zustand Store를 사용하는 컴포넌트는 Client Component로 작성합니다. React Server Component에서 Store를 읽거나 수정하지 않습니다. 초기 상태는 서버와 클라이언트에서 다르게 계산하지 않습니다. 요청 또는 사용자별 데이터를 전역 Module Store에 저장하지 않습니다. TBD App Router에서 Store를 Module Hook으로 사용할 범위와 요청별 Provider를 적용할 범위는 SSR·Hydration 및 Layout 경계가 확정된 후 결정합니다.",
  ...
  },
  {
    ...
    section: "Persist 사용",
    excerpt:
      "UI 상태는 새로고침 시 초기화되는 것을 기본으로 합니다. 단순한 메뉴·Dialog·Toast 상태를 Persist하지 않습니다. 인증 토큰, 결제 정보와 개인정보를 Persist Store에 저장하지 않습니다. Persist가 필요하면 저장할 필드만 선택합니다. 저장 구조가 변경될 가능성이 있으면 Version과 Migration을 함께 검토합니다. Hydration 전후의 화면 차이로 인…",
    content:
      "9. Persist 사용 UI 상태는 새로고침 시 초기화되는 것을 기본으로 합니다. 단순한 메뉴·Dialog·Toast 상태를 Persist하지 않습니다. 인증 토큰, 결제 정보와 개인정보를 Persist Store에 저장하지 않습니다. Persist가 필요하면 저장할 필드만 선택합니다. 저장 구조가 변경될 가능성이 있으면 Version과 Migration을 함께 검토합니다. Hydration 전후의 화면 차이로 인한 UI 깜빡임과 오류를 확인합니다. TBD Persist 허용 대상, Storage, Key 이름, Version, Migration과 Hydration 처리 기준은 실제 유지 요구가 확인된 후 결정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "모든 UI 상태를 하나의 거대한 Store에 넣지 않습니다. Store 이름은 useNavigationUiStore , useOverlayStore 처럼 책임을 드러냅니다. 상태의 사용처와 수명이 같을 때만 같은 Store에 둡니다. 사용처가 생기기 전에 Store 파일이나 폴더를 미리 만들지 않습니다. 로그아웃, 사용자 전환 또는 기능 종료 시 남으면 안 되는 상태는 Reset Action…",
    content:
      "7. Store 분리와 초기화 모든 UI 상태를 하나의 거대한 Store에 넣지 않습니다. Store 이름은 useNavigationUiStore , useOverlayStore 처럼 책임을 드러냅니다. 상태의 사용처와 수명이 같을 때만 같은 Store에 둡니다. 사용처가 생기기 전에 Store 파일이나 폴더를 미리 만들지 않습니다. 로그아웃, 사용자 전환 또는 기능 종료 시 남으면 안 되는 상태는 Reset Action으로 초기화합니다. TBD Store의 최종 분리 단위와 파일 배치는 실제 UI 구조와 공유 상태가 확인된 후 결정합니다.",
  ...
  },
  {
    ...
    excerpt:
      "상태 도구보다 상태의 소유자와 수명을 먼저 판단합니다. 상태 기본 선택 한 컴포넌트 또는 가까운 트리의 UI 상태 React 지역 상태 주소로 공유·복원할 상태 URL 상태 검토 폼 입력과 제출 상태 React Hook Form 서버에서 조회·변경하는 데이터 TanStack Query 서로 떨어진 여러 UI가 공유하는 클라이언트 상태 Zustand Zustand를 검토하는 경우 Header와…",
    content:
