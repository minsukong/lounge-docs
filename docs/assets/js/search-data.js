window.LOUNGE_GUIDE_SEARCH_INDEX = [
  ...
  },
  {
    "document": "Front-End 개발 가이드",
    "section": "미확정 항목 목록",
    "excerpt": "아래 항목은 합의 전까지 구현 표준으로 간주하지 않습니다. 서비스와 화면 IA, 주요 사용자 흐름과 라우트 그룹 Flutter Native 앱과 WebView Front-end의 배포 연계 범위 WebView 화면의 일반 모바일 브라우저 직접 접근 지원 여부 일반 사용자·직원 역할과 세부 권한 정책 렌더링과 API Server Component와 Client Component의 구분 기준 S…",
  ...
  },
  {
    ...
    "excerpt": "구조 변경 기준 새 애플리케이션을 추가할 때 Owner, 배포 단위, 환경, 권한과 Rollback 책임을 함께 정의합니다. 계약을 변경할 때 영향을 받는 TypeScript·Dart 생성 결과, Adapter와 출시된 앱 Version을 함께 검토합니다. 우선 결정 항목 실제 package에 사용할 패키지 관리자와 실행 Script 승인된 Backend API와 Native Bridge 계…",
    "content": "8. 변경 기준과 미확정 항목 구조 변경 기준 새 애플리케이션을 추가할 때 Owner, 배포 단위, 환경, 권한과 Rollback 책임을 함께 정의합니다. 계약을 변경할 때 영향을 받는 TypeScript·Dart 생성 결과, Adapter와 출시된 앱 Version을 함께 검토합니다. 우선 결정 항목 실제 package에 사용할 패키지 관리자와 실행 Script 승인된 Backend API와 Native Bridge 계약 배포 플랫폼과 CI/CD 품질 Gate Front-End 저장소 구조 기준",
    "url": "./../guides/architecture/index.html#section-8"
  },
  {
    "document": "Front-End 저장소 구조 기준",
    "section": "저장소 구조",
    "excerpt": "<repository-root>/ ├── AGENTS.md # 저장소 전체 구현 지침 ├── README.md # 프로젝트 진입점 ├── apps/ │ ├── mobile/ # Flutter Native 앱 │ └── app-webview/ # 앱 내 Next.js WebView Front-end │ ├── AGENTS.md # 앱 전용 구현 지침 │ ├── .storybook/ # Stor…",
    "content": "2. 저장소 구조 <repository-root>/ ├── AGENTS.md # 저장소 전체 구현 지침 ├── README.md # 프로젝트 진입점 ├── apps/ │ ├── mobile/ # Flutter Native 앱 │ └── app-webview/ # 앱 내 Next.js WebView Front-end │ ├── AGENTS.md # 앱 전용 구현 지침 │ ├── .storybook/ # Storybook 설정 │ └── src/ # 애플리케이션 source └── docs/ ├── AGENTS.md # 문서 작성 지침 ├── guides/ # 상세 가이드 ├── ai/ # AI 작업별 요약 └── common-source/ # 파일 단위 구현 기준 애플리케이션 범위 Directory 역할 기술·분리 상태 apps/mobile Flutter Native 앱과 기기 기능 Flutter·Dart apps/app-webview 앱 내 Front-end 서비스 화면과 업무 흐름 Next.js·React·TypeScript",
