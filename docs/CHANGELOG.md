...
- 애플리케이션 소스의 상위 경로를 `apps/`로 정의하고 Flutter Native는 `apps/mobile/`, WebView Front-end는 `apps/app-webview/`에서 관리하도록 저장소 지침과 가이드의 상위 구조 설명을 통일했습니다.
- 저장소 구조를 설명하는 문장과 흐름에서는 `apps/`를 사용하고, Next.js·TypeScript·Storybook 등 WebView 구현을 직접 가리키는 경로는 `apps/app-webview/`로 유지했습니다.
- 두 애플리케이션은 담당자별로 Toolchain, Build, Test와 배포 범위를 각각 관리하며, Front-end의 기본 구현 범위는 `apps/app-webview/`, `apps/mobile/`은 별도 앱 개발자 책임임을 명확히 했습니다. `docs/`는 어느 애플리케이션의 제품 Bundle이나 Build 입력에도 포함하지 않습니다.
- 검증: 변경된 Markdown과 HTML의 경로 표현을 다시 검색하고 `git diff --check`로 공백 오류와 관련 없는 전체 파일 변동이 없는지 확인했습니다.
...
- 한국·중국·일본·영문과 전 세계의 2026년 7월 StatCounter 브라우저 제품군 점유율을 브라우저 선정의 보조 근거로 추가하고, 제품군 점유율을 특정 최소 버전 이상의 정확한 사용자 비율로 해석하지 않도록 제한을 명시했습니다.
