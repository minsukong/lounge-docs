...
다음 항목은 화면 기획과 관계없이 현재 기술 기준으로 검토할 수 있는 참고 구현입니다. 실제 앱의 기존 구조, 사용처와 설치 버전을 확인하고 필요한 항목만 적용합니다.

| 참고 문서 | 권장 대상 경로 | 책임 | 적용 전 확인 |
| --- | --- | --- | --- |
...
`TBD`: 패키지 관리자, 정확한 패키지 버전과 설치 명령은 실제 package와 잠금 파일에서 확인합니다. 다음 표는 각 구현 예시에 필요한 패키지만 보여 줍니다.
...
예시 코드를 실제 앱에 반영하고 검증한 뒤 다음 내용을 PR이나 작업 기록에 남깁니다.
