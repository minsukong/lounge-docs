...
## 공통 아이콘 컴포넌트(Icon) 사용 규칙

- **프로젝트 공통 컴포넌트**: `Icon`은 프로젝트 전반에서 사용하는 공통 컴포넌트입니다.
- **Inline SVG 사용 금지**: 모든 아이콘 작업 시 `<svg>` 태그를 직접 작성하는 것은 엄격히 금지됩니다.
- **`Icon` 컴포넌트 활용**: 반드시 프로젝트의 공통 `Icon` 컴포넌트를 사용하여 구현합니다.
  - 사용법: `<Icon name="arrow" className="text-primary size-5" />`
- **스타일링**: 색상은 `text-{color}` 클래스를, 크기는 `size-{n}` 클래스를 통해 Tailwind CSS로 제어합니다.
