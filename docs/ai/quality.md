...
- Lint가 발견하지 못하는 접근성 항목(스크린 리더 낭독, 포커스 이동, 대비)은 `accessibility.md`에서 수작업 검증 기준을 확인합니다.
- Test는 코드를 실행해 사용자 동작과 실제 결과를 확인합니다.
- Storybook은 Component, Feature와 사용자에게 노출되는 Screen의 실제 렌더링, 공개 API, 주요 상태와 Viewport를 애플리케이션 전체 실행과 분리해 확인합니다.
