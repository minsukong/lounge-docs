...
기획과 Backend API 계약이 확정되기 전에는 API Mock 설정을 추가하지 않습니다. 계약 확정 후 사용할 수 있는 Backend 환경에서 필요한 상태를 재현하기 어렵다면 Front-end 책임자 또는 프로젝트 담당자와 테스트 범위·관리 책임을 정한 뒤 [API Mock 도입 판단 기준](./api-mocking.md)을 확인해 기존 setup에 필요한 lifecycle을 병합합니다.
...
API 연동 테스트는 승인된 계약과 Backend 개발·테스트 환경을 우선 사용합니다. 단순 폼처럼 network 경계가 책임이 아닌 테스트는 함수를 prop으로 전달해 더 작게 검증합니다.
